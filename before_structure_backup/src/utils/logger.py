...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if event and hasattr(event, '__str__') and str(event) in LOG_EMOJIS:
            emoji = LOG_EMOJIS[str(event)]
        ...
        if extra_data and isinstance(extra_data, dict):
            formatted += self._format_extra_data(extra_data)
    ...
    def _format_extra_data(self, extra_data):
        """Format extra data vertically for better readability."""
        if not extra_data:
            return ""
        ...
        if formatted_lines:
            # Replace the last ├─ with └─ for better visual hierarchy
...
class FileFormatter(logging.Formatter):
    """Custom formatter for file output with vertical extra data formatting."""
    ...
    def _format_extra_data(self, extra_data):
        """Format extra data vertically for better readability in files."""
        if not extra_data:
            return ""
