            ...
            if quiz_state_file.exists():
                try:
                    ...
                    if previous_interval is None:
                        # First check
                        print(f"[{current_time.strftime('%H:%M:%S')}] Initial interval: {current_interval} hours")
                        print(f"                    Last updated: {last_updated}")
                    elif current_interval != previous_interval:
                        # Interval changed!
                        print(f"\n🚨 INTERVAL CHANGE DETECTED!")
                        print(f"[{current_time.strftime('%H:%M:%S')}] Changed from {previous_interval}h to {current_interval}h")
                        print(f"                    Last updated: {last_updated}")
                        ...
                        print(f"[{current_time.strftime('%H:%M:%S')}] File updated (interval unchanged: {current_interval}h)")
                        print(f"                    New update time: {last_updated}")
                    elif check_count % 12 == 0:  # Every 2 minutes
                        # Periodic status update
                        print(f"[{current_time.strftime('%H:%M:%S')}] Status: {current_interval}h (no changes)")
                    
                    previous_interval = current_interval
                    previous_update_time = last_updated
                    
                except json.JSONDecodeError as e:
                    print(f"[{current_time.strftime('%H:%M:%S')}] ❌ Error reading quiz state file: {e}")
                except Exception as e:
                    print(f"[{current_time.strftime('%H:%M:%S')}] ❌ Unexpected error: {e}")
            else:
                print(f"[{current_time.strftime('%H:%M:%S')}] ❌ Quiz state file not found!")
