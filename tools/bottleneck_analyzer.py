...
class BottleneckAnalyzer:
    ...
    async def _analyze_system_performance(self) -> Dict[str, Any]:
        ...
        cpu_status = "excellent" if cpu_score > 80 else "good" if cpu_score > 60 else "poor"
        
        # Analyze memory usage
        memory_score = 100.0 - system_metrics["memory_percent"]
        memory_status = "excellent" if memory_score > 80 else "good" if memory_score > 60 else "poor"
        
        # Analyze thread count
        thread_count = system_metrics["num_threads"]
        thread_status = "good" if thread_count < 50 else "warning" if thread_count < 100 else "critical"
        
        # Analyze open files
        open_files = system_metrics["open_files"]
        file_status = "good" if open_files < 100 else "warning" if open_files < 500 else "critical"
        ...
        for op in top_operations:
            if op["avg_time_ms"] > 100:  # Operations taking > 100ms
                slow_operations.append({
                    ...
                    "severity": "critical" if op["avg_time_ms"] > 1000 else "high" if op["avg_time_ms"] > 500 else "medium"
                })
    ...
    async def _analyze_memory_usage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "growth_rate_mb_per_minute": memory_growth / max(len(self.profiler.memory_snapshots), 1),
            "issues": [
    ...
    async def _identify_critical_issues(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def generate_report(self, results: Dict[str, Any]) -> str:
        ...
        if "system_analysis" in results:
            ...
            report.append("-" * 30)
            report.append(f"CPU Usage: {system['cpu']['usage_percent']:.1f}% ({system['cpu']['status']})")
            report.append(f"Memory Usage: {system['memory']['usage_percent']:.1f}% ({system['memory']['status']})")
            report.append(f"Threads: {system['threads']['count']} ({system['threads']['status']})")
            report.append(f"Open Files: {system['files']['count']} ({system['files']['status']})")
            report.append("")
    ...
    async def export_results(self, results: Dict[str, Any], filename: str = None) -> str:
