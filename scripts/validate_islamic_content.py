...
HADITH_PATTERN = re.compile(r"Hadith|Bukhari|Muslim|Tirmidhi|Abu Dawud|Nasa'i|Ibn Majah")
ISLAMIC_TERMS = {
...
SENSITIVE_TERMS = [
    "jihad", "kafir", "infidel", "crusade", "terrorism", 
    "extremist", "radical", "fundamentalist"
]

# Valid Islamic content sources
VALID_SOURCES = {
    "quran.com", "sunnah.com", "islamqa.info", "islamhouse.com",
    "kalamullah.com", "islamweb.net", "dar-us-salam.com"
}
...
class IslamicContentValidator:
    ...
    def _validate_quiz_questions(self, file_path: Path, questions) -> bool:
        ...
        for i, question in enumerate(questions):
            ...
            if 'explanation' in question:
                valid &= self._validate_explanation(file_path, question['explanation'], i)
    ...
    def _validate_text_content(self, file_path: Path, text: str, context: str = "") -> bool:
        ...
        for surah, ayah in quran_matches:
            valid &= self._validate_verse_reference(file_path, int(surah), int(ayah), context)
        ...
        for correct_term, variations in ISLAMIC_TERMS.items():
            for variation in variations:
                if variation.lower() in text_lower and correct_term.lower() not in text_lower:
                    self._add_warning(
                        file_path,
                        f"Consider using '{correct_term}' instead of '{variation}' in {context}",
                        {"suggested": correct_term, "found": variation, "context": context}
                    )
        
        return valid
    
    def _validate_verse_reference(self, file_path: Path, surah: int, ayah: int, context: str = "") -> bool:
    ...
    def _validate_quiz_answer(self, file_path: Path, question: dict, index: int) -> bool:
    ...
    def _validate_explanation(self, file_path: Path, explanation: str, index: int) -> bool:
        ...
        return self._validate_text_content(file_path, explanation, f"explanation[{index}]")
    ...
    def _validate_arabic_text(self, file_path: Path, arabic_text: str) -> bool:
        ...
        if not arabic_pattern.search(arabic_text):
            self._add_warning(
                ...
                {"text": arabic_text[:50] + "..." if len(arabic_text) > 50 else arabic_text}
            )
    ...
    def _validate_hadith_content(self, file_path: Path, hadith_data) -> bool:
        ...
        if isinstance(hadith_data, dict):
            ...
            missing_fields = [field for field in required_fields if field not in hadith_data]
            ...
            if 'source' in hadith_data:
                ...
                if not any(valid_source in source for valid_source in ['bukhari', 'muslim', 'tirmidhi', 'abu dawud', 'nasai', 'ibn majah']):
                    self._add_warning(
                        file_path,
...
def main():
    ...
    if all_valid:
        ...
        print(f"الحمد لله (Alhamdulillahi Rabbil Alameen)")
    else:
