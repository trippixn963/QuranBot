...
def check_existing_instances():
    ...
    try:
                            ...
                            try:
                                ...
                                if 'QuranBot' in proc_cwd:
                                    ...
                                    if os.path.normpath(proc_cwd) == os.path.normpath(current_cwd):
                                        log_tree_branch("venv_detected", f"Virtual environment process detected")
        ...
        log_tree_branch("error", f"Failed to check processes: {e}")
    ...
    for i, proc in enumerate(bot_processes, 1):
        try:
            log_tree_branch(f"instance_{i}", f"PID {proc.pid} - {' '.join(proc.cmdline())}")
        except (psutil.NoSuchProcess, psutil.AccessDenied):
