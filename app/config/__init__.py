...
try:
    ...
    _config_components["main_config"] = {"status": "❌ Import failed", "error": str(e)}
...
_config_package_metadata.update(
    {
        ...
        "success_rate": (
            f"{(_config_package_metadata['imports_successful'] / _config_package_metadata['imports_attempted'] * 100):.1f}%"
            if _config_package_metadata["imports_attempted"] > 0
            else "0%"
        ),
...
try:
    ...
    config_status = {
        ...
        "env_file": (
            "✅ Found" if _config_package_metadata["env_file_exists"] else "❌ Missing"
        ),
        "env_file_size": (
            f"{_config_package_metadata['env_file_size']} bytes"
            if _config_package_metadata["env_file_exists"]
            else "N/A"
        ),
        "main_config": _config_components.get("main_config", {}).get(
            "status", "Unknown"
        ),
        "enums": _config_components.get("enums", {}).get("status", "Unknown"),
        "config_instance": _config_components.get("config_instance", {}).get(
            "status", "Unknown"
        ),
        "critical_validation": _config_components.get("critical_validation", {}).get(
            "status", "Unknown"
        ),
    }
    ...
    if (
        "config_instance" in _config_components
        and _config_components["config_instance"]["status"] == "✅ Loaded successfully"
    ):
        config_status.update(
            {
                ...
                "guild_configured": (
                    "✅ Yes"
                    if _config_components["config_instance"]["guild_configured"]
                    else "❌ No"
                ),
                "audio_folder": (
                    "✅ Exists"
                    if _config_components["config_instance"]["audio_folder_exists"]
                    else "❌ Missing"
                ),
                "ffmpeg": (
                    "✅ Available"
                    if _config_components["config_instance"]["ffmpeg_available"]
                    else "❌ Missing"
                ),
            }
        )
    ...
    if _config_import_warnings:
        log_event(
            ...
            {
                f"warning_{i+1}": warning
                for i, warning in enumerate(_config_import_warnings)
            },
        )
    ...
    sys.stderr.write(
        f"Configuration package loaded in {_config_package_metadata['total_import_time_ms']}ms\n"
    )
    if _config_import_errors:
...
if (
    "main_config" in _config_components
    and _config_components["main_config"]["status"] == "✅ Success"
):
    config_exports = [
        "QuranBotConfig",
        "get_config",
    ...
    for export_name in config_exports:
        _exported_config_components[export_name] = _config_components["main_config"][
            export_name
        ]

if (
    "enums" in _config_components
    and _config_components["enums"]["status"] == "✅ Success"
):
    enum_exports = ["ReciterName", "Environment", "LogLevel"]
    __all__.extend(enum_exports)

    for export_name in enum_exports:
        _exported_config_components[export_name] = _config_components["enums"][
            export_name
        ]
...
def validate_config_package() -> dict[str, Any]:
    ...
    validation_results = {
        ...
        "component_status": {
            ...
            for name, comp in _config_components.items()
        },
...
def get_config_package_info() -> dict[str, Any]:
    ...
    return {
        ...
        "configuration_status": _config_components.get("config_instance", {}).get(
            "status", "Unknown"
        ),
