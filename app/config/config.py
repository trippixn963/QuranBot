...
_config_metadata = {
    ...
    "env_file_path": str(Path(".env").absolute()) if Path(".env").exists() else "Not found",
    "total_env_vars": len(os.environ),
    "bot_env_vars": len([k for k in os.environ.keys() if k.startswith(('DISCORD_', 'GUILD_', 'VOICE_', 'PANEL_', 'QURAN_VC_', 'DEVELOPER_', 'AUDIO_', 'FFMPEG_', 'OPENAI_', 'ENVIRONMENT', 'LOG_'))]),
...
class QuranBotConfig(BaseSettings):
    ...
    def validate_audio_folder(cls, v: Path) -> Path:
        ...
        if not v.exists():
            raise ValueError(f"Audio folder not found: {v}. Please ensure audio files are present.")
        if not v.is_dir():
    ...
    def validate_ffmpeg_path(cls, v: Path) -> Path:
        ...
        def log_detection_info():
            ...
            try:
                ...
                TreeLogger.info("🎵 FFmpeg Detection & Verification", {
                        ...
                        "Path Exists": "✅ Yes" if Path(detection_info['final_path']).exists() else "❌ No",
                        "Executable": "✅ Yes" if os.access(detection_info['final_path'], os.X_OK) else "❌ No"
                    }
                )
                
                if detection_info['tested_paths']:
                    TreeLogger.info("📂 Tested Paths", {f"path_{i+1}": f"{path} ({'✅ Found' if found else '❌ Not found'})" 
                         for i, (path, found) in enumerate(detection_info['tested_paths'])}, service="Config")
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        TreeLogger.info("✅ FFmpeg Functionality Verified", {
                        ...
                        TreeLogger.warning("⚠️ FFmpeg Execution Warning", {
                    ...
                    TreeLogger.error("❌ FFmpeg Test Failed", None, {
                            "Path": detection_info['final_path'],
                ...
                import sys
                sys.stderr.write(f"FFmpeg detected: {detection_info['final_path']} (Method: {detection_info['detection_method']})\\n")
                sys.stderr.flush()
        ...
        if detected_ffmpeg:
            ...
            if os.access(detected_path, os.X_OK):
                ...
                detection_info['path_verification'][str(detected_path)] = "found_in_path_but_not_executable"
        ...
        for path in common_paths:
            ...
            if path_exists:
                if os.access(path, os.X_OK):
                    ...
                    detection_info['path_verification'][str(path)] = "exists_but_not_executable"
            else:
                ...
                for root, dirs, files in os.walk(search_dir):
                    if 'ffmpeg' in files:
                        ...
                        if os.access(candidate, os.X_OK):
                            detection_info['detection_method'] = f"recursive_search_{search_dir}"
                            detection_info['final_path'] = str(candidate)
                            detection_info['tested_paths'].append((str(candidate), True))
                            log_detection_info()
                            return candidate
        ...
        try:
            ...
            TreeLogger.error("❌ FFmpeg Detection Failed", None, {
                    "Operating System": f"{detection_info['os_system']} {detection_info['os_release']}",
    ...
    def validate_developer_id(cls, v: Optional[int]) -> Optional[int]:
        """Ensure developer_id is valid if provided."""
        if v is not None and v <= 0:
            raise ValueError(
                "DEVELOPER_ID must be a positive integer if provided."
            )
        return v
                ...
                if path_value.exists():
                    ...
                    if path_value.is_dir():
                        ...
                        try:
                            test_file.touch()
                            test_file.unlink()
                            validation_results["path_validations"][f"{path_name}_writable"] = "✅ Writable"
                        except Exception:
                            validation_results["path_validations"][f"{path_name}_writable"] = "❌ Not writable"
                            validation_results["warnings"].append(f"Directory {path_name} is not writable")
                    elif path_value.is_file():
                        # Check if file is executable (for ffmpeg)
                        if path_name == "ffmpeg_path" and os.access(path_value, os.X_OK):
                            validation_results["path_validations"][f"{path_name}_executable"] = "✅ Executable"
                        elif path_name == "ffmpeg_path":
                            validation_results["path_validations"][f"{path_name}_executable"] = "❌ Not executable"
                            validation_results["warnings"].append("FFmpeg is not executable")
                else:
                    validation_results["path_validations"][path_name] = "❌ Missing"
                    if path_name in ["audio_folder", "ffmpeg_path"]:
                        validation_results["errors"].append(f"Critical path {path_name} does not exist")
                    else:
                        validation_results["warnings"].append(f"Path {path_name} will be created automatically")
                        
            except Exception as e:
                validation_results["path_validations"][path_name] = f"❌ Error: {e}"
                validation_results["errors"].append(f"Path validation failed for {path_name}: {e}")
        
        # Security checks
        if config.discord_token:
            if len(config.discord_token) < 50:
                validation_results["security_checks"]["discord_token_length"] = "⚠️ Suspiciously short"
                validation_results["warnings"].append("Discord token appears to be too short")
            else:
                validation_results["security_checks"]["discord_token_length"] = "✅ Appropriate length"
        
        if config.openai_api_key:
            if config.openai_api_key.startswith("sk-"):
                validation_results["security_checks"]["openai_key_format"] = "✅ Valid format"
            else:
                validation_results["security_checks"]["openai_key_format"] = "⚠️ Invalid format"
                validation_results["warnings"].append("OpenAI API key format appears invalid")
        
        # Environment-specific validations
        if config.is_production:
            if config.log_level == LogLevel.DEBUG:
                validation_results["warnings"].append("DEBUG logging enabled in production environment")
            
            validation_results["security_checks"]["production_ready"] = "✅ Production mode"
        else:
            validation_results["security_checks"]["production_ready"] = "⚠️ Development mode"
        
        # Audio system validation
        if config.audio_folder.exists():
            reciter_folders = [f for f in config.audio_folder.iterdir() if f.is_dir()]
            validation_results["field_details"]["available_reciters"] = f"✅ {len(reciter_folders)} found"
            
            if len(reciter_folders) == 0:
                validation_results["errors"].append("No reciter folders found in audio directory")
...
_config_instance: Optional[QuranBotConfig] = None
