...
from ..ui.islamic.response_embeds import create_ai_response_embed, create_rate_limit_embed, create_ai_error_embed
...
class MentionHandler:
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            TreeLogger.debug("Services retrieved", {
            ...
            if not all([self.ai_service, self.islamic_service, self.rate_limiter]):
                TreeLogger.warning("Some AI services not available - mention handler limited", {
                    "ai_service": bool(self.ai_service),
                    "islamic_service": bool(self.islamic_service),
                    "rate_limiter": bool(self.rate_limiter),
                    "token_tracker": bool(self.token_tracker)
            ...
            TreeLogger.info("Mention handler setup complete", {
                "services_available": sum([bool(self.ai_service), bool(self.islamic_service), 
                                          bool(self.rate_limiter), bool(self.token_tracker)])
            })
            
        except Exception as e:
            TreeLogger.error("Failed to setup mention handler", e, {
                "error_type": type(e).__name__
            })
            await self.error_handler.handle_error(
                e,
                {"operation": "mention_handler_setup"}
            )
    ...
    async def handle_message(self, message: discord.Message) -> None:
        ...
        if message.reference and message.reference.message_id:
            try:
                ...
                if referenced_msg.author.id == self.bot.user.id:
                    TreeLogger.debug("Skipping reply to bot message", {
        ...
        TreeLogger.debug("Bot mention detected", {
            "user_id": message.author.id,
        ...
        if not self.ai_service or not self.config.openai_api_key:
            TreeLogger.warning("AI services not available for mention", {
        ...
        try:
            ...
            if message.author.id == self.config.developer_id:
                if message_text.lower() in ['usage', 'ai usage', 'openai usage', 'budget']:
                    await self._handle_usage_command(message)
                    return
            
            # Log mention
            TreeLogger.info("Bot mentioned for AI interaction", {
            ...
            if self.rate_limiter:
                TreeLogger.debug("Checking rate limit", {
                    "user_id": message.author.id
                })
                
                allowed, time_remaining = await self.rate_limiter.check_user(message.author.id)
                
                if not allowed:
                    TreeLogger.info("Rate limit hit for user", {
                        "user_id": message.author.id,
                        "time_remaining": str(time_remaining)
                    })
                    
                    # Send rate limit message
                    cooldown_message = self.rate_limiter.format_cooldown_message(time_remaining)
            ...
            if self.token_tracker and self.token_tracker.is_budget_exceeded():
                TreeLogger.warning("AI budget exceeded, blocking request", {
                    "user_id": message.author.id
                })
                ...
                embed.set_footer(
                    text="Developed by حَـــــنَّـــــا",
            ...
            async with message.channel.typing():
                ...
                if self.islamic_service:
                    greeting_response = self.islamic_service.detect_greeting(message_text)
                    if greeting_response:
                        TreeLogger.debug("Greeting detected", {
                            "greeting_type": "islamic" if "Assalam" in greeting_response else "regular"
                        })
                ...
                if self.islamic_service:
                    ...
                    TreeLogger.debug("Context prepared", {
                ...
                if not success:
                    TreeLogger.warning("AI response generation failed", {
                ...
                if self.ai_service:
                    try:
                        ...
                        success, arabic_resp, _ = await self.ai_service.generate_response(arabic_prompt)
                        if success:
                ...
                TreeLogger.info("AI response sent successfully", {
                    "user_id": message.author.id,
                    "response_length": len(response),
                    "tokens_used": metadata.get("total_tokens", 0) if metadata else 0
                })
                ...
                if self.token_tracker and metadata:
                    TreeLogger.debug("Tracking token usage", {
            ...
            TreeLogger.error("Error handling mention", e, {
                "user_id": message.author.id,
                "guild_id": message.guild.id if message.guild else None,
                "error_type": type(e).__name__,
                "channel_id": message.channel.id
    ...
    def _extract_question(self, content: str) -> str:
        ...
        """
        TreeLogger.debug("Extracting message content", {
        ...
        TreeLogger.debug("Message extracted", {
    ...
    async def _handle_usage_command(self, message: discord.Message) -> None:
        ...
        try:
            ...
            async with message.channel.typing():
                ...
                TreeLogger.info("Usage command executed via mention", {
                    "user": message.author.name,
