...
class BaseValidator(ABC):
    ...
    async def validate_with_logging(self) -> bool:
        ...
        """
        try:
            TreeLogger.debug(f"Starting validation for {self.service_name}", 
            ...
            for error in result.errors:
                TreeLogger.error(f"Validation error: {error}", 
                               service=self.service_name)
            
            # Log warnings
            for warning in result.warnings:
                TreeLogger.warning(f"Validation warning: {warning}", 
                                 service=self.service_name)
            
            # Log result
            if result.is_valid:
                TreeLogger.success(f"Validation passed for {self.service_name}", {
                    "warnings_count": len(result.warnings),
                    "metadata": result.metadata
                }, service=self.service_name)
            else:
                TreeLogger.error(f"Validation failed for {self.service_name}", {
            ...
            TreeLogger.error(f"Error during validation: {e}", {
...
class FileSystemValidator(BaseValidator):
    ...
    def add_path(self, path: Union[str, Path], 
                 ...
                 required_space_mb: Optional[float] = None) -> None:
        """
            ...
            if not path.exists():
                if path_type == "directory" and path_config["create_if_missing"]:
                    try:
                        ...
                        TreeLogger.info(f"Created directory: {path}", 
                                      service=self.service_name)
                    except Exception as e:
...
class DependencyValidator(BaseValidator):
    ...
    async def _validate_executable(self, dep: Dict[str, Any], 
    ...
    async def _validate_module(self, dep: Dict[str, Any], 
                             result: ValidationResult) -> None:
...
class ConfigurationValidator(BaseValidator):
    ...
    def add_check(self, check_func: Callable[[Any, ValidationResult], None], 
    ...
    async def validate(self) -> ValidationResult:
        ...
        for check_func, description in self.checks:
            try:
                TreeLogger.debug(f"Running config check: {description}", 
                               service=self.service_name)
...
async def validate_service_requirements(
    ...
    config_checks: Optional[List[Callable]] = None,
