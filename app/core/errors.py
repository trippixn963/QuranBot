...
class ErrorContext:
    ...
    def to_dict(self, service: str | None = None) -> dict[str, Any]:
...
class ErrorHandler:
    ...
    def _create_error_context(
        ...
        error_context = ErrorContext(
            operation=op, severity=severity, category=category, **valid_context_params
        )
    ...
    def _categorize_error(
        ...
        if (
            ...
            and "cannot write to closing transport" in error_message
        ):
        ...
        if any(
            network_error in error_message
        ...
        if any(
            discord_error in error_message
            for discord_error in [
                "discord",
                "api",
                "rate limit",
                "forbidden",
        ...
        if any(
            ...
            for db_error in ["database", "sql", "state", "file", "json", "corruption"]
        ...
        if any(
            validation_error in error_message
        ...
        if any(
            resource_error in error_message
            for resource_error in ["memory", "disk", "space", "resource", "file", "io"]
        ...
        if any(
            service_error in error_message
    ...
    async def _log_error(self, error: Exception, error_context: ErrorContext) -> None:
        ...
        if error_context.severity == ErrorSeverity.DEBUG:
            TreeLogger.debug(
                f"Expected error in {error_context.operation}: {error!s}",
                context={k: v for k, v in log_context.items() if k != "traceback"},
            )
        else:
            # Use TreeLogger.print_error for robust error logging
            TreeLogger.error(
                f"Error in {error_context.operation}: {error!s}", context=log_context
            )
        ...
        TreeLogger.error(
            ...
            context={
                **error_context.to_dict(service="ErrorHandler"),
                ...
                "bot_restart_recommended": error_context.severity
                == ErrorSeverity.FATAL,
            },
        )
