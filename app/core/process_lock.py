...
class ProcessLockManager:
    ...
    def __init__(self, lock_file_path: Optional[str] = None):
        ...
        TreeLogger.info("Process lock manager initialized", {
    ...
    def acquire_lock(self, force: bool = False, timeout: int = 30) -> bool:
        ...
        """
        try:
            TreeLogger.info("Attempting to acquire process lock", {
                "force": force,
                "timeout": timeout,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            ...
            while time.time() - start_time < timeout:
                ...
                if existing_instance:
                    if force:
                        TreeLogger.warning("Existing instance detected, attempting termination", {
                            "existing_pid": existing_instance["pid"],
                            "existing_start_time": existing_instance.get("start_time"),
                            "force_mode": True
                        }, service="ProcessLockManager")
                        
                        if self._terminate_existing_instance(existing_instance):
                            TreeLogger.info("Existing instance terminated successfully", {
                            ...
                            TreeLogger.error("Failed to terminate existing instance", None, {
                                "existing_pid": existing_instance["pid"]
                            }, service="ProcessLockManager")
                            return False
                    else:
                        TreeLogger.error("Another bot instance is already running", None, {
                            "existing_pid": existing_instance["pid"],
                            "existing_start_time": existing_instance.get("start_time"),
                            "force_mode": False
                        }, service="ProcessLockManager")
                ...
                if self._create_lock_file():
                    ...
                    TreeLogger.info("Process lock acquired successfully", {
                        "lock_file_path": self.lock_file_path,
                        "current_pid": self.current_pid,
            ...
            TreeLogger.error("Failed to acquire process lock within timeout", None, {
                "timeout": timeout,
            ...
            TreeLogger.error(f"Error acquiring process lock: {e}", None, {
                "lock_file_path": self.lock_file_path,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            return False
    ...
    def release_lock(self) -> bool:
        ...
        try:
            if not self.lock_acquired:
                TreeLogger.warning("No lock to release", {
                    "current_pid": self.current_pid
                }, service="ProcessLockManager")
                return True
            
            TreeLogger.info("Releasing process lock", {
                "lock_file_path": self.lock_file_path,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            ...
            if success:
                self.lock_acquired = False
                TreeLogger.info("Process lock released successfully", {
                    "lock_file_path": self.lock_file_path,
                    "current_pid": self.current_pid
                }, service="ProcessLockManager")
            else:
                TreeLogger.error("Failed to release process lock", None, {
                    "lock_file_path": self.lock_file_path,
                    "current_pid": self.current_pid
                }, service="ProcessLockManager")
            ...
            TreeLogger.error(f"Error releasing process lock: {e}", None, {
                "lock_file_path": self.lock_file_path,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            return False
    
    def _check_existing_instance(self) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if len(lock_data) < 2:
                TreeLogger.warning("Invalid lock file format, removing", {
                    "lock_file_path": self.lock_file_path
                }, service="ProcessLockManager")
            ...
            if self._is_process_running(pid):
                ...
                TreeLogger.warning("Stale lock file detected, removing", {
                    "stale_pid": pid,
                    "lock_file_path": self.lock_file_path
                }, service="ProcessLockManager")
                os.remove(self.lock_file_path)
            ...
            TreeLogger.warning(f"Error checking existing instance: {e}", {
                "lock_file_path": self.lock_file_path
            }, service="ProcessLockManager")
            ...
            TreeLogger.error(f"Unexpected error checking existing instance: {e}", None, {
                "lock_file_path": self.lock_file_path
            }, service="ProcessLockManager")
            return None
        ...
        try:
            ...
            if psutil.pid_exists(pid):
                ...
                try:
                    ...
                    if any('python' in arg.lower() or 'quranbot' in arg.lower() for arg in cmdline):
                        return True
            ...
            TreeLogger.warning(f"Error checking if process {pid} is running: {e}", {
                "pid": pid
            }, service="ProcessLockManager")
            return False
    ...
    def _terminate_existing_instance(self, instance_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            TreeLogger.warning("Terminating existing bot instance", {
                "target_pid": pid,
                "start_time": instance_info.get("start_time", "unknown")
            }, service="ProcessLockManager")
            
            if not self._is_process_running(pid):
                TreeLogger.info("Target process no longer running", {
                    "target_pid": pid
                }, service="ProcessLockManager")
                return True
            ...
            TreeLogger.info("Sending SIGTERM to existing instance", {
                "target_pid": pid
            }, service="ProcessLockManager")
            ...
            try:
                ...
                TreeLogger.info("Existing instance terminated gracefully", {
                    "target_pid": pid
                }, service="ProcessLockManager")
                return True
            except psutil.TimeoutExpired:
                TreeLogger.warning("Graceful termination timed out, using SIGKILL", {
                    "target_pid": pid
                }, service="ProcessLockManager")
                ...
                try:
                    ...
                    TreeLogger.info("Existing instance force-killed", {
                        "target_pid": pid
                    }, service="ProcessLockManager")
                    return True
                except psutil.TimeoutExpired:
                    TreeLogger.error("Failed to force-kill existing instance", None, {
                        "target_pid": pid
                    }, service="ProcessLockManager")
                    return False
                    
        except psutil.NoSuchProcess:
            TreeLogger.info("Target process no longer exists", {
                "target_pid": instance_info["pid"]
            }, service="ProcessLockManager")
            return True
        except psutil.AccessDenied:
            TreeLogger.error("Permission denied when terminating existing instance", None, {
                "target_pid": instance_info["pid"]
            }, service="ProcessLockManager")
            return False
        except Exception as e:
            TreeLogger.error(f"Error terminating existing instance: {e}", None, {
                "target_pid": instance_info["pid"]
            }, service="ProcessLockManager")
            return False
    ...
    def _create_lock_file(self) -> bool:
        ...
        try:
            ...
            self.lock_file = os.open(self.lock_file_path, os.O_CREAT | os.O_WRONLY | os.O_TRUNC)
            
            # Try to acquire exclusive lock (non-blocking)
            fcntl.flock(self.lock_file, fcntl.LOCK_EX | fcntl.LOCK_NB)
            
            # Write process information
            lock_content = f"{self.current_pid}\n{datetime.now(APP_TIMEZONE).isoformat()}\n"
            ...
            if "Resource temporarily unavailable" in str(e) or "Operation would block" in str(e):
                return False
            
            TreeLogger.error(f"Error creating lock file: {e}", None, {
                "lock_file_path": self.lock_file_path,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            return False
        except Exception as e:
            TreeLogger.error(f"Unexpected error creating lock file: {e}", None, {
                "lock_file_path": self.lock_file_path,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            return False
    ...
    def _remove_lock_file(self) -> bool:
        ...
        try:
            ...
            TreeLogger.error(f"Error removing lock file: {e}", None, {
                "lock_file_path": self.lock_file_path,
                "current_pid": self.current_pid
            }, service="ProcessLockManager")
            return False
...
_process_lock_manager: Optional[ProcessLockManager] = None
