...
from .logger import log_event, TreeLogger
...
class DIContainer:
    ...
    def __init__(self):
        ...
        TreeLogger.info("Dependency Injection Container initialized", {"status": "initialized"}, service="DIContainer")
    
    def register_singleton(self, service_type: Type[T], instance_or_factory: T | Callable[[], T]) -> None:
        """
        ...
        """
        if callable(instance_or_factory) and not isinstance(instance_or_factory, service_type):
            # Factory function
        ...
        TreeLogger.success(f"Singleton service registered: {service_type.__name__}", {"type": "singleton", "factory": callable(instance_or_factory)}, service="DIContainer")
    ...
    def register_transient(self, service_type: Type[T], factory: Callable[[], T]) -> None:
        ...
        TreeLogger.success(f"Transient service registered: {service_type.__name__}", {"type": "transient"}
        , service="DIContainer")
    
    def register_factory(self, service_type: Type[T], factory: Callable[["DIContainer"], T], singleton: bool = True) -> None:
        """
        ...
        TreeLogger.success(f"Factory service registered: {service_type.__name__}", {"type": service_scope, "factory": "container_aware"}
        , service="DIContainer")
    ...
    def get_service(self, service_type: Type[T]) -> T:
        ...
        try:
            ...
            TreeLogger.error(f"Failed to resolve service: {service_type.__name__}", None, e,
                {"registration_type": "singleton" if registration.singleton else "transient"}
            , service="DIContainer")
            raise
        finally:
    ...
    async def initialize_async_services(self) -> None:
        ...
        """
        TreeLogger.info("Initializing async services", {"status": "starting"}, service="DIContainer")
        ...
        for service_type, registration in self._registrations.items():
            try:
                ...
                if hasattr(instance, 'initialize') and inspect.iscoroutinefunction(instance.initialize):
                    ...
                    TreeLogger.success(f"Async service initialized: {service_type.__name__}", {"method": "initialize()"}, service="DIContainer")
                    
            except Exception as e:
                TreeLogger.error(f"Failed to initialize async service: {service_type.__name__}", e, None, service="DIContainer")
                raise
        
        TreeLogger.info("Async Services Initialization Complete", {
    ...
    async def shutdown_async_services(self) -> None:
        ...
        """
        TreeLogger.warning("Shutting down async services", {"status": "starting_shutdown"}, service="DIContainer")
        ...
        for service_type, registration in self._registrations.items():
            ...
            try:
                ...
                if hasattr(instance, 'shutdown') and inspect.iscoroutinefunction(instance.shutdown):
                    ...
                    TreeLogger.success(f"Service shutdown: {service_type.__name__}", {"method": "shutdown()"}, service="DIContainer")
                    
            except Exception as e:
                TreeLogger.error(f"Error shutting down service: {service_type.__name__}", e, None, service="DIContainer")
                # Continue shutdown process even if one service fails
        
        TreeLogger.info("Service Shutdown Complete", {
                "total_services": len(self._registrations),
    ...
    def clear(self) -> None:
        ...
        TreeLogger.warning("DI Container cleared", {"status": "cleared"}, service="DIContainer")