...
from ..config.timezone import APP_TIMEZONE, TIMEZONE
...
class LogRetentionManager:
    ...
    def __init__(self, base_log_folder: Path, retention_days: int = DEFAULT_RETENTION_DAYS,
        ...
        with self._lock:
            ...
            try:
                ...
                for date_folder in self.base_log_folder.iterdir():
                    if not date_folder.is_dir() or not self._is_date_folder(date_folder.name):
                        continue
                    ...
                    try:
                        ...
                        # Compress folders older than compression days but newer than deletion
                            ...
                            if not zip_file.exists():
                                ...
                                shutil.make_archive(str(date_folder), 'zip', date_folder)
                                shutil.rmtree(date_folder)
                                compressed_size = zip_file.stat().st_size
                                cleanup_stats["compressed_folders"] += 1
                                cleanup_stats["freed_bytes"] += (original_size - compressed_size)
                                
                    except Exception as e:
                        cleanup_stats["errors"].append(f"Error processing {date_folder.name}: {e}")
...
class PerformanceTracker:
    ...
    def track_operation(self, operation: str, duration_ms: float, success: bool = True,
                       service: str = "system", context: Optional[Dict[str, Any]] = None) -> None:
    ...
    def track_error(self, service: str = "system") -> None:
        ...
        with self._lock:
            ...
            self.metrics["errors_per_hour"] = {
                k: v for k, v in self.metrics["errors_per_hour"].items()
                if datetime.strptime(k, "%Y-%m-%d_%H").replace(tzinfo=APP_TIMEZONE) > cutoff
            }
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "performance_warnings": self._check_performance_warnings(avg_duration, errors_this_hour)
            }
    
    def _check_performance_warnings(self, avg_duration: float, errors_this_hour: int) -> List[str]:
...
class LogFileManager:
    ...
    def __init__(self, base_log_folder: Path = Path("logs"), 
                 retention_days: int = DEFAULT_RETENTION_DAYS,
                 compression_days: int = DEFAULT_COMPRESSION_DAYS):
        self.base_log_folder = base_log_folder
        self.retention_manager = LogRetentionManager(base_log_folder, retention_days, compression_days)
    ...
    def _cleanup_existing_logs(self) -> None:
        ...
        try:
            ...
            if cleanup_stats["deleted_folders"] > 0:
                print(f"🧹 Cleaned up {cleanup_stats['deleted_folders']} log folders "
    ...
    def _setup_handlers(self) -> None:
        ...
        self.handlers = {
            ...
            'json': logging.FileHandler(folder / "log.json", mode='a', encoding='utf-8'),
            'error': logging.FileHandler(folder / "error.log", mode='a', encoding='utf-8'),
            'performance': logging.FileHandler(folder / "performance.json", mode='a', encoding='utf-8'),
            'debug': logging.FileHandler(folder / "debug.log", mode='a', encoding='utf-8')
        }
    ...
    def _rotate_if_needed(self) -> None:
        ...
        now = datetime.now(APP_TIMEZONE)
        if now.replace(tzinfo=None) - self._last_cleanup.replace(tzinfo=None) > timedelta(hours=24):
            try:
                cleanup_stats = self.retention_manager.cleanup_old_logs()
                if cleanup_stats["compressed_folders"] > 0 or cleanup_stats["deleted_files"] > 0:
                    self._write_to_handler('log', f"📦 Log cleanup completed: {cleanup_stats}")
    ...
    def write_performance(self, operation: str, duration_ms: float, success: bool = True,
                         service: str = "system", context: Optional[Dict[str, Any]] = None) -> None:
        """Write performance metrics."""
        self.performance_tracker.track_operation(operation, duration_ms, success, service, context)
        ...
        self._write_to_handler('performance', json.dumps(performance_data, cls=JSONEncoder))
...
class TreeLogger:
    ...
    def _format_value(cls, key: str, value: Any) -> str:
        ...
        if isinstance(value, str) and ("time" in key.lower() or "timestamp" in key.lower()):
            try:
                ...
                if "T" in value and ("+" in value or "Z" in value or value.endswith("-04:00")):
        ...
        for i, (key, value) in enumerate(items):
            ...
            if isinstance(value, dict):
                ...
                for j, (nested_key, nested_value) in enumerate(nested_items):
                    ...
                    lines.append(f"{timestamp} {nested_prefix}{nested_branch} {nested_key}: {formatted_value}")
            else:
    ...
    @classmethod
    def section(cls, title: str, context: Optional[Dict[str, Any]] = None, 
               service: str = "system", correlation_id: Optional[str] = None) -> str:
        ...
        if correlation_id:
            corr_line = f"{timestamp} {cls.TREE_BRANCH} correlation_id: {correlation_id}"
            print(corr_line)
    ...
    def info(cls, message: str, context: Optional[Dict[str, Any]] = None,
            service: str = "system", correlation_id: Optional[str] = None) -> None:
        """Log info message."""
        cls._log_message(message, "INFO", context, service, correlation_id)
    
    @classmethod
    def success(cls, message: str, context: Optional[Dict[str, Any]] = None,
               service: str = "system", correlation_id: Optional[str] = None) -> None:
        """Log success message."""
    ...
    def error(cls, message: str, error: Optional[Exception] = None,
             context: Optional[Dict[str, Any]] = None, service: str = "system",
        ...
        if error:
            ...
            error_context.update({
                ...
                "error_args": getattr(error, 'args', ()) if hasattr(error, 'args') else ()
            })
    ...
    @classmethod
    def critical(cls, message: str, error: Optional[Exception] = None,
                context: Optional[Dict[str, Any]] = None, service: str = "system",
                correlation_id: Optional[str] = None) -> None:
    ...
    @classmethod
    def performance(cls, operation: str, duration_ms: float, success: bool = True,
                   context: Optional[Dict[str, Any]] = None, service: str = "system",
                   correlation_id: Optional[str] = None) -> None:
        ...
        perf_context.update({
            "duration_ms": duration_ms,
            "success": success,
            "slow_operation": duration_ms > PERFORMANCE_THRESHOLDS["slow_operation_ms"]
        })
        
        level = "WARNING" if duration_ms > PERFORMANCE_THRESHOLDS["slow_operation_ms"] else "PERFORMANCE"
        ...
        _file_manager.write_performance(operation, duration_ms, success, service, context)
    
    @classmethod
    def health(cls, service_name: str, health_data: Dict[str, Any],
              correlation_id: Optional[str] = None) -> None:
        ...
        _file_manager.performance_tracker.update_service_health(service_name, health_data)
    
    @classmethod
    def _log_message(cls, message: str, level: str, context: Optional[Dict[str, Any]] = None,
                    service: str = "system", correlation_id: Optional[str] = None) -> None:
        ...
        print(main_line)
        lines = [f"{timestamp} {level_icon} {service_icon} {message}"]  # Clean version for file
...
class PerformanceTimer:
    ...
    def __init__(self, operation: str, service: str = "system", 
                 correlation_id: Optional[str] = None, log_result: bool = True):
...
def setup_logging(log_level: str = DEFAULT_LOG_LEVEL, base_log_folder: Optional[Path] = None,
                 retention_days: int = DEFAULT_RETENTION_DAYS,
                 compression_days: int = DEFAULT_COMPRESSION_DAYS) -> None:
    ...
    if base_log_folder and str(base_log_folder) != str(_file_manager.base_log_folder):
        _file_manager = LogFileManager(base_log_folder, retention_days, compression_days)
        _file_manager._cleanup_existing_logs()
    ...
    TreeLogger.info("logging system initialized with fresh logs", {
