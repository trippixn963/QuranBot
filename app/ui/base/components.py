...
class ActivityTrackingMixin:
    ...
    def track_user_activity(self, user: discord.User, action: str, context: Optional[Dict[str, Any]] = None):
        ...
        TreeLogger.debug(f"User activity tracked: {action}", {
            "user_id": user_id,
            "username": user.display_name,
            "action": action,
    ...
    def get_activity_summary(self) -> Dict[str, Any]:
        ...
        total_interactions = sum(activity["total_interactions"] for activity in self.user_activity.values())
        
        # Users active in last 5 minutes
        active_users = sum(
            1 for activity in self.user_activity.values()
            if activity.get("last_interaction") and 
            (now - activity["last_interaction"]).total_seconds() < 300
        )
...
class LoggingMixin:
    ...
    def log_interaction(self, interaction: discord.Interaction, action: str, 
                       context: Optional[Dict[str, Any]] = None):
        """Log user interaction with consistent format."""
        TreeLogger.info(f"UI Interaction: {action}", {
    ...
    def log_error(self, error: Exception, context: Optional[Dict[str, Any]] = None):
        ...
        TreeLogger.error(f"UI Error: {str(error)}", error=error, context=context, service="UI")
    
    def log_state_change(self, component: str, old_state: Any, new_state: Any):
        """Log state changes for debugging."""
        TreeLogger.debug(f"State change in {component}", {
            "component": component,
...
class BaseView(discord.ui.View, ActivityTrackingMixin, UpdateableMixin, LoggingMixin):
    ...
    def __init__(self, *, timeout: Optional[float] = 300, **kwargs):
    ...
    async def on_timeout(self):
        ...
        TreeLogger.info("UI View timed out", {
            "view_type": self.__class__.__name__,
            "timeout_seconds": self.timeout,
            "interaction_count": self.interaction_count,
            "lifetime_seconds": (datetime.now(APP_TIMEZONE) - self.creation_time).total_seconds()
        }, service="UI")
    ...
    def track_interaction(self, interaction: discord.Interaction, action: str, 
                         context: Optional[Dict[str, Any]] = None):
