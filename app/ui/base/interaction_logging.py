...
class InteractionLoggingMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._interaction_start_time: Optional[float] = None
        self._current_interaction_id: Optional[str] = None
        self._processing_start_time: Optional[float] = None
    
    async def start_interaction_logging(self, interaction, component_type: str, 
                                      component_id: str, component_label: str,
        ...
        try:
            ...
            bot = getattr(self.view, 'bot', None) if hasattr(self, 'view') else getattr(self, 'bot', None)
            if not bot:
                TreeLogger.warning("Cannot log interaction - no bot reference", {
            ...
            if not user_logger:
                TreeLogger.warning("User interaction logger not available", {
                    "component_type": component_type,
                    "component_id": component_id
                }, service="InteractionLogging")
                return ""
            ...
            # Start logging
            self._current_interaction_id = await user_logger.log_control_panel_interaction(
            ...
            TreeLogger.error(f"Error starting interaction logging: {e}", None, {
                "component_type": component_type,
                "component_id": component_id
            }, service="InteractionLogging")
            return ""
    
    async def complete_interaction_logging(self, status: InteractionStatus,
        ...
        try:
            ...
            if processing_start_time:
                ...
                processing_time_ms = response_time_ms  # If no separate processing time tracked
            ...
            TreeLogger.error(f"Error completing interaction logging: {e}", None, {
                "interaction_id": self._current_interaction_id
            }, service="InteractionLogging")
    ...
    async def _get_current_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "current_surah": getattr(audio_manager, 'get_current_surah', lambda: {})(),
                "current_reciter": getattr(audio_manager, 'get_current_reciter', lambda: {})(),
            ...
            TreeLogger.error(f"Error getting current state: {e}", None, {
                "component": self.__class__.__name__
            }, service="InteractionLogging")
            return {}
    
    async def log_interaction_error(self, error: Exception, interaction_context: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            TreeLogger.error(f"Error logging interaction error: {e}", None, {
    ...
    async def is_user_in_quran_vc(self, interaction) -> bool:
        ...
        try:
            ...
            TreeLogger.info("Voice channel check", {
                "user_id": interaction.user.id,
                "username": interaction.user.name,
                "configured_vc_id": quran_vc_id,
                "user_vc_id": user_vc_id,
                "user_in_voice": member.voice is not None,
                "user_in_correct_vc": user_vc_id == quran_vc_id if user_vc_id else False
            })
            ...
            TreeLogger.error(f"Error checking user voice channel: {e}", None, {
