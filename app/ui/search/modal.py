...
class SurahSearchModal(Modal, LoggingMixin, InteractionLoggingMixin):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if self.control_panel_view and hasattr(self.control_panel_view, '_update_last_activity'):
                self.control_panel_view._update_last_activity(interaction.user, f"searched for '{query}'")
            
            # Execute intelligent search using the proper search service
            results = search_surahs(query)
            
            TreeLogger.info("Search query processed", {
            ...
            if len(results) == 1:
                ...
                TreeLogger.info("Single search result found", {
                    "user_id": interaction.user.id,
                    "query": query,
            ...
            TreeLogger.error(f"Search error: {e}", e, {
                "user_id": interaction.user.id,
    ...
    async def _show_confirmation_embed(self, interaction: discord.Interaction, 
                                      surah, query: str):
        ...
        if 'meaning' in surah and surah['meaning']:
        ...
        TreeLogger.info("Search confirmation sent", {
            "user_id": interaction.user.id,
    ...
    async def _handle_multiple_results(self, interaction: discord.Interaction,
        ...
        for i, result in enumerate(results[:5], 1):
            result_text.append(f"{i}. **{result['name_english']}** ({result['name_arabic']})")
