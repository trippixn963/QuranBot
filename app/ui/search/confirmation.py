...
class SurahConfirmationView(View, LoggingMixin):
    ...
    def __init__(self, surah: Dict[str, Any], query: str,
    ...
    async def play_surah(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            # Track activity
            if self.control_panel_view and hasattr(self.control_panel_view, '_update_last_activity'):
                surah_name = self.surah.get('name_english', f"Surah {self.surah.get('number', 'Unknown')}")
                self.control_panel_view._update_last_activity(
                    interaction.user, 
                    f"searched for '{self.query}' → `{surah_name}`"
                )
            ...
            surah_number = self.surah.get('number') if hasattr(self.surah, 'get') else getattr(self.surah, 'number', None)
            if not surah_number:
                embed = discord.Embed(
                    title="❌ Invalid Surah",
            ...
            if success:
                ...
                surah_name = (self.surah.get('name_english') if hasattr(self.surah, 'get') 
                ...
                TreeLogger.info("Surah playback started via search", {
                ...
                if self.control_panel_view and hasattr(self.control_panel_view, 'update_display'):
    ...
    async def search_again(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            # Track activity
            if self.control_panel_view and hasattr(self.control_panel_view, '_update_last_activity'):
                self.control_panel_view._update_last_activity(interaction.user, "opened search modal")
            ...
            TreeLogger.info("New search initiated", {
                "user_id": interaction.user.id,
    ...
    async def cancel(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            TreeLogger.info("Surah selection cancelled", {
                "user_id": interaction.user.id,
