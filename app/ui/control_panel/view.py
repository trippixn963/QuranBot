...
from .buttons import (
    ...
    SearchButton, PrevPageButton, NextPageButton, RefreshButton
)
from .dropdowns import SurahSelect, ReciterSelect, QuickJumpSelect
...
class ControlPanelView(BaseView):
    ...
    def __init__(self, audio_manager=None, bot=None, message=None, **kwargs):
        super().__init__(timeout=None, **kwargs)  # No timeout for persistent control panel
        ...
        TreeLogger.info("Control panel view initialized", {
    ...
    def _setup_components(self):
        ...
        try:
            ...
            self.reciter_select = ReciterSelect(
                ...
                audio_folder=getattr(self.audio_manager, 'audio_folder', None) if self.audio_manager else None
            )
            ...
            TreeLogger.info("Control panel components setup complete", {
                "total_components": len(self.children),
                "layout": "balanced_button_distribution"
            }, service="ControlPanelView")
            
        except Exception as e:
            TreeLogger.error(f"Error setting up control panel components: {e}", service="ControlPanelView")
            raise
    ...
    async def update_display(self, force_update: bool = False):
        ...
        try:
            if not self.message:
                TreeLogger.warning("No message to update in control panel", service="ControlPanelView")
                return
            
            # Get current audio state
            audio_state = await self._get_audio_state()
            
            TreeLogger.info(f"Control panel update requested", {
            ...
            if not force_update and not self._needs_update(audio_state):
                TreeLogger.info("Control panel update skipped - no changes detected", service="ControlPanelView")
                return
            ...
            if force_update and self.surah_select:
                TreeLogger.debug("Force update detected - refreshing dropdown options", {
            ...
            activity_info = {
                "recent_activity": self.get_recent_activity(limit=3) if hasattr(self, 'get_recent_activity') else [],
                "activity_summary": self.get_activity_summary() if hasattr(self, 'get_activity_summary') else {},
                "last_activity": self._get_last_activity_info() if self.last_activity_user else None
            }
            ...
            TreeLogger.debug("Control panel display updated", {
                "audio_state": {
                    "is_playing": audio_state.get("is_playing", False),
                    "current_surah": audio_state.get("current_surah", {}).get("number"),
            ...
            await self.error_handler.handle_error(
                e,
                {
                    ...
                    "last_update_time": self.last_embed_update.isoformat() if self.last_embed_update else None
                }
            )
            
            if self.update_failures >= self.max_update_failures:
                TreeLogger.warning("Control panel update failures exceeded limit", {
                    "failures": self.update_failures,
                    "stopping_updates": True
                }, service="ControlPanelView")
    ...
    async def _get_audio_state(self) -> Dict[str, Any]:
        ...
        if not self.audio_manager:
            TreeLogger.debug("No audio manager available, returning default state", service="ControlPanelView")
            return {
                "is_playing": False,
                "is_paused": False,
                "current_surah": {},
        ...
        try:
            ...
            current_surah = getattr(self.audio_manager, 'get_current_surah', lambda: {})()
            current_reciter = getattr(self.audio_manager, 'get_current_reciter', lambda: {})()
            ...
            TreeLogger.debug("Retrieved audio state", {
                "is_playing": is_playing,
                "is_paused": is_paused,
                "surah_number": current_surah.get("number") if current_surah else None,
            ...
            return {
                ...
                "shuffle_mode": getattr(self.audio_manager, 'get_shuffle_mode', lambda: False)()
            }
        except Exception as e:
            TreeLogger.error(f"Error getting audio state: {e}", service="ControlPanelView")
            return {
                "is_playing": False,
                "is_paused": False,
                "current_surah": {},
    ...
    def _needs_update(self, current_state: Dict[str, Any]) -> bool:
        ...
        if not self.last_audio_state:
            TreeLogger.debug("No previous state - update needed", service="ControlPanelView")
            return True
        
        # Check for significant state changes
        significant_changes = [
            "is_playing", "is_paused", "current_surah", "current_reciter",
            "loop_mode", "shuffle_mode"
        ]
        ...
        for key in significant_changes:
            ...
            if current_val != previous_val:
                TreeLogger.debug(f"State change detected: {key} changed from {previous_val} to {current_val}", 
                               service="ControlPanelView")
                return True
        ...
        if current_state.get("is_playing", False):
            ...
            if time_since_update.total_seconds() >= 30:
                TreeLogger.debug(f"Periodic update triggered - {time_since_update.total_seconds():.1f}s since last update", 
                               service="ControlPanelView")
                return True
        
        # Update if position changed significantly (more than 5 seconds)
        position_diff = abs(current_state.get("position", 0) - self.last_audio_state.get("position", 0))
        if position_diff >= 5:
            TreeLogger.debug(f"Position change detected - diff: {position_diff:.1f}s", service="ControlPanelView")
            return True
    ...
    def _sync_button_states(self, audio_state: Dict[str, Any]):
        ...
        try:
            ...
            TreeLogger.error(f"Error syncing button states: {e}", service="ControlPanelView")
    ...
    async def update_panel(self):
        ...
        try:
            ...
            TreeLogger.info("update_panel called - current audio state", {
                "current_surah": audio_state.get("current_surah", {}),
                "current_reciter": audio_state.get("current_reciter", {}),
                "is_playing": audio_state.get("is_playing", False)
            }, service="ControlPanelView")
            ...
            # Message was deleted
            TreeLogger.warning("Control panel message was deleted", service="ControlPanelView")
            if hasattr(self, 'update_task') and self.update_task and not self.update_task.done():
    ...
    def _update_last_activity(self, user: discord.User, action: str):
        ...
        """
        TreeLogger.debug("Updating last activity", {
            "user_id": user.id,
            "username": user.display_name,
            "action": action,
            "previous_action": self.last_activity_action,
            "time_since_last": self._format_time_elapsed(self.last_activity_time) if self.last_activity_time else "N/A"
        }, service="ControlPanelView")
    ...
    def _get_last_activity_info(self) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        if not (self.last_activity_user and self.last_activity_time and self.last_activity_action):
            return None
        ...
        try:
            ...
            TreeLogger.error("Error formatting last activity info", {
    ...
    async def start_monitoring(self):
        ...
        try:
            ...
            TreeLogger.info("Control panel monitoring started", {
                "update_interval": self.update_interval,
                "smart_updates": True
            }, service="ControlPanelView")
            
        except Exception as e:
            TreeLogger.error(f"Error starting control panel monitoring: {e}", service="ControlPanelView")
    ...
    async def stop_monitoring(self):
        ...
        try:
            ...
            TreeLogger.info("Control panel monitoring stopped", service="ControlPanelView")
            
        except Exception as e:
            TreeLogger.error(f"Error stopping control panel monitoring: {e}", service="ControlPanelView")
    
    async def on_timeout(self):
        """Handle view timeout (should not occur as timeout is None)."""
        TreeLogger.warning("Control panel view timed out unexpectedly", service="ControlPanelView")
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, item: discord.ui.Item):
        ...
        TreeLogger.error(f"Control panel interaction error: {error}", {
