...
class ControlPanelManager:
    ...
    def __init__(self, bot, audio_manager=None):
        ...
        TreeLogger.info("Control panel manager initialized", {
            "max_panels_per_guild": self.max_panels_per_guild,
            "panel_timeout_hours": self.panel_timeout_hours
        }, service="ControlPanelManager")
    
    async def create_control_panel(self, channel: discord.TextChannel, 
                                 replace_existing: bool = True) -> Optional[discord.Message]:
        """
        ...
        try:
            ...
            TreeLogger.debug("Starting control panel creation", {
            ...
            if not self._check_permissions(channel):
                TreeLogger.warning("Insufficient permissions for control panel", {
                    "channel_id": channel_id,
                    "guild_id": guild_id
                }, service="ControlPanelManager")
                return None
            ...
            guild_panels = [p for p in self.active_panels.values() if p["guild_id"] == guild_id]
            if len(guild_panels) >= self.max_panels_per_guild and not replace_existing:
                TreeLogger.warning("Guild panel limit reached", {
                    "guild_id": guild_id,
            ...
            TreeLogger.debug("Sending control panel message", {
                "channel_id": channel_id,
            ...
            TreeLogger.debug("Control panel message sent", {
            ...
            TreeLogger.info("Control panel created successfully", {
                "channel_id": channel_id,
                "guild_id": guild_id,
                "message_id": message.id,
            ...
            TreeLogger.error(f"Error creating control panel: {e}", {
                "channel_id": getattr(channel, 'id', None),
                "guild_id": getattr(channel.guild, 'id', None) if hasattr(channel, 'guild') else None
            }, service="ControlPanelManager")
            return None
    ...
    async def remove_control_panel(self, channel_id: int) -> bool:
        ...
        try:
            ...
            try:
                ...
                TreeLogger.warning("Cannot delete control panel message (no permissions)", {
                    "channel_id": channel_id,
            ...
            TreeLogger.info("Control panel removed", {
                "channel_id": channel_id,
            ...
            TreeLogger.error(f"Error removing control panel: {e}", {
                "channel_id": channel_id
            }, service="ControlPanelManager")
            return False
    ...
    async def refresh_all_panels(self):
        ...
        try:
            ...
            if refresh_tasks:
                ...
                TreeLogger.info("All control panels refreshed", {
                    "panel_count": len(refresh_tasks)
                }, service="ControlPanelManager")
            
        except Exception as e:
            TreeLogger.error(f"Error refreshing control panels: {e}", service="ControlPanelManager")
    ...
    async def cleanup_inactive_panels(self):
        ...
        try:
            ...
            if panels_to_remove:
                TreeLogger.info("Inactive control panels cleaned up", {
                    "removed_count": len(panels_to_remove),
                    "remaining_panels": len(self.active_panels)
                }, service="ControlPanelManager")
            
        except Exception as e:
            TreeLogger.error(f"Error cleaning up control panels: {e}", service="ControlPanelManager")
    ...
    async def setup_control_panel(self, channel_id: int) -> bool:
        ...
        try:
            ...
            if not channel:
                TreeLogger.error("Channel not found for control panel setup", {
                    "channel_id": channel_id
                }, service="ControlPanelManager")
                return False
            ...
            TreeLogger.error(f"Error setting up control panel: {e}", {
                "channel_id": channel_id
            }, service="ControlPanelManager")
            return False
    ...
    async def _cleanup_channel_messages(self, channel: discord.TextChannel) -> None:
        ...
        try:
            ...
            if not permissions.manage_messages:
                TreeLogger.warning("No permission to delete messages in control panel channel", {
                    "channel_id": channel.id,
                    "guild_id": channel.guild.id
                }, service="ControlPanelManager")
                return
            ...
            TreeLogger.info("Starting control panel channel cleanup", {
                "channel_id": channel.id,
                "channel_name": channel.name,
                "guild_id": channel.guild.id
            }, service="ControlPanelManager")
            ...
            try:
                ...
                async for message in channel.history(limit=50):
                    ...
                    if message.author == self.bot.user:
                        ...
                        is_control_panel = (
                            message.embeds or  # Has embeds (status displays)
                            message.components or  # Has buttons/dropdowns (interactive elements)
                            "Surah" in (message.content or "") or  # Contains surah information
                            "Reciter" in (message.content or "") or  # Contains reciter information
                            "🎵" in (message.content or "") or  # Has music emoji (playback indicator)
                            "▰" in (message.content or "") or "▱" in (message.content or "")  # Has progress bar (playback progress)
                        )
                        ...
                        if is_control_panel:
                            ...
                            TreeLogger.info("Skipping non-control-panel bot message", {
                                "channel_id": channel.id,
                                "message_id": message.id,
                ...
                TreeLogger.info(f"Found {len(messages_to_delete)} bot messages to delete", {
                    "channel_id": channel.id,
                ...
                for message in messages_to_delete:
                    try:
                        ...
                        TreeLogger.info(f"Deleted old control panel message", {
                            "channel_id": channel.id,
                            "message_id": message.id,
                            "message_content_preview": message.content[:50] if message.content else "[embed]"
                        }, service="ControlPanelManager")
                        ...
                        TreeLogger.info("Message already deleted", {
                            "channel_id": channel.id,
                            "message_id": message.id
                        }, service="ControlPanelManager")
                        ...
                        TreeLogger.warning("Cannot delete message (no permissions)", {
                            "channel_id": channel.id,
                            "message_id": message.id
                        }, service="ControlPanelManager")
                        ...
                        TreeLogger.error(f"Error deleting message: {e}", {
                            "channel_id": channel.id,
                            "message_id": message.id,
                ...
                TreeLogger.warning("Cannot read channel history (no permissions)", {
                    "channel_id": channel.id
                }, service="ControlPanelManager")
            
            if deleted_count > 0:
                TreeLogger.info(f"Successfully cleaned up {deleted_count} old control panel messages", {
                    "channel_id": channel.id,
                    "channel_name": channel.name,
                    "deleted_count": deleted_count
                }, service="ControlPanelManager")
            else:
                TreeLogger.info("No old control panel messages found to clean up", {
                    "channel_id": channel.id,
                    "channel_name": channel.name
                }, service="ControlPanelManager")
            
        except Exception as e:
            TreeLogger.error(f"Error cleaning up channel messages: {e}", {
                "channel_id": getattr(channel, 'id', None)
            }, service="ControlPanelManager")
    ...
    async def _get_initial_audio_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            TreeLogger.error(f"Error getting initial audio state: {e}", service="ControlPanelManager")
            return {
                "is_playing": False,
                "is_paused": False,
                "current_surah": {},
    ...
    def get_panel_info(self, channel_id: int) -> Optional[Dict[str, Any]]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        guilds_with_panels = len(set(p["guild_id"] for p in self.active_panels.values()))
    ...
    async def shutdown(self):
        ...
        try:
            TreeLogger.info("Shutting down control panel manager", {
            ...
            TreeLogger.info("Control panel manager shutdown complete", service="ControlPanelManager")
            
        except Exception as e:
            TreeLogger.error(f"Error during control panel manager shutdown: {e}", service="ControlPanelManager")