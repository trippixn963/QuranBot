...
def validate_panel_permissions(channel: discord.TextChannel) -> tuple[bool, str]:
    ...
    missing_permissions = [
        perm_name
        for perm_name, has_perm in required_permissions.items()
        if not has_perm
    ]
...
def sync_toggle_states(view, audio_manager) -> dict[str, Any]:
    ...
    if not audio_manager:
        return {"shuffle_mode": False, "loop_mode": "off"}
    ...
    try:
        ...
        return {"shuffle_mode": shuffle_mode, "loop_mode": loop_mode}
    ...
    async def setup_for_bot(bot, audio_manager, config):
        ...
        try:
            ...
            if hasattr(config, "panel_channel_id") and config.panel_channel_id:
                ...
                if success:
                    TreeLogger.info(
                        "Control panel auto-setup successful",
                        {"channel_id": config.panel_channel_id},
                        service="ControlPanelIntegration",
                    )
                else:
                    TreeLogger.warning(
                        "Control panel auto-setup failed",
                        {"channel_id": config.panel_channel_id},
                        service="ControlPanelIntegration",
                    )
            ...
            TreeLogger.error(
                f"Error setting up control panel integration: {e}",
                service="ControlPanelIntegration",
            )
            return None
...
async def safe_defer(interaction: discord.Interaction, ephemeral: bool = False) -> bool:
    ...
    try:
        ...
        TreeLogger.debug(
            f"safe_defer called - response already done: {is_done}",
            {
                "user": (
                ...
                "ephemeral": ephemeral,
                "custom_id": (
                    getattr(interaction.data, "custom_id", None)
        ...
        if not is_done:
            ...
            TreeLogger.debug(
                "Interaction deferred successfully", service="ControlPanelUtils"
            )
            return True
        return False
        ...
        # Interaction expired
        TreeLogger.debug(
            "Interaction not found - likely already responded",
            service="ControlPanelUtils",
        )
        return False
    except Exception as e:
        TreeLogger.warning(
            f"Failed to defer interaction: {e}", service="ControlPanelUtils"
        )
        return False
...
async def safe_interaction_response(
    ...
    try:
        ...
        TreeLogger.warning(
            "Interaction expired before response", service="ControlPanelUtils"
        )
        return False
    except Exception as e:
        TreeLogger.error(
            f"Failed to send interaction response: {e}", service="ControlPanelUtils"
        )
        return False
