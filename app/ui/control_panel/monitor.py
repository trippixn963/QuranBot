...
class ControlPanelMonitor:
    ...
    def __init__(self, manager):
        ...
        TreeLogger.info("Control panel monitor initialized", {
    ...
    async def start(self):
        ...
        TreeLogger.info("Control panel monitoring started", service="ControlPanelMonitor")
    ...
    async def stop(self):
        ...
        TreeLogger.info("Control panel monitoring stopped", service="ControlPanelMonitor")
    ...
    async def _monitor_loop(self):
        ...
        while self.is_running:
            try:
                ...
                TreeLogger.error(f"Error in control panel monitor loop: {e}", {
                    "failure_count": self.failure_count
                }, service="ControlPanelMonitor")
                
                # STEP 7: Exponential Backoff Calculation
                # Calculate backoff time to prevent overwhelming the system
                backoff_time = min(self.check_interval * (2 ** min(self.failure_count, 5)), 1800)
        ...
        try:
            ...
            for channel_id, panel_info in active_panels.items():
                try:
                    ...
                    if panel_info["view"] and hasattr(panel_info["view"], 'get_status_summary'):
                        status = panel_info["view"].get_status_summary()
                        
                        # Check for signs of health issues
                        if status.get("update_failures", 0) > 3:
                            unhealthy_panels += 1
                            TreeLogger.warning("Control panel health issue detected", {
                    ...
                    if panel_info["message"]:
                        try:
                            ...
                            unhealthy_panels += 1
                            TreeLogger.warning("Control panel message inaccessible", {
                                "channel_id": channel_id,
                    ...
                    unhealthy_panels += 1
                    TreeLogger.error(f"Error checking panel health: {e}", {
                        "channel_id": channel_id
                    }, service="ControlPanelMonitor")
            
            # Log health summary
            TreeLogger.debug("Control panel health check complete", {
            ...
            if self.failure_count > 0:
                ...
                self.failure_count = 0
                TreeLogger.info("Control panel monitor recovered", {
                    "recovery_count": self.recovery_count
                }, service="ControlPanelMonitor")
                
        except Exception as e:
            TreeLogger.error(f"Error performing control panel health check: {e}", service="ControlPanelMonitor")
            raise
    ...
    async def _perform_cleanup(self):
        ...
        try:
            TreeLogger.info("Starting control panel cleanup", service="ControlPanelMonitor")
            ...
            TreeLogger.info("Control panel cleanup complete", service="ControlPanelMonitor")
            
        except Exception as e:
            TreeLogger.error(f"Error performing control panel cleanup: {e}", service="ControlPanelMonitor")
