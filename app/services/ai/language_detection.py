...
class LanguageDetection:
    ...
    def __init__(self):
        ...
        try:
            TreeLogger.info("Initializing language detection module", service=self.service_name)
            ...
            # Language indicators
            self.arabic_indicators = ["؟", "،", "أ", "إ", "ال", "في", "من", "على", "ما", "هل"]
            self.english_indicators = ["the", "is", "are", "what", "how", "when", "where", "why"]
            
            TreeLogger.info("Language detection module initialized", {
            ...
            TreeLogger.error("Failed to initialize language detection module", e, {
    ...
    def detect_language(self, message: str) -> Tuple[str, float]:
        ...
        """
        try:
            TreeLogger.debug("Detecting language", {
                "message_length": len(message) if message else 0
            }, service=self.service_name)
            
            if not message:
                TreeLogger.warning("Empty message for language detection", service=self.service_name)
                return "en", 1.0
            ...
            arabic_phrase_count = sum(1 for phrase in self.arabic_phrases if phrase in message)
            
            # Check for Arabic indicators
            arabic_indicator_count = sum(1 for indicator in self.arabic_indicators if indicator in message)
            
            # Check for English indicators
            english_indicator_count = sum(1 for indicator in self.english_indicators if indicator in message_lower)
            
            # Calculate scores
            arabic_score = (arabic_ratio * 10) + (arabic_phrase_count * 2) + arabic_indicator_count
            english_score = english_indicator_count + (1 - arabic_ratio) * 5
            
            TreeLogger.debug("Language scores calculated", {
            ...
            TreeLogger.info("Language detected", {
                "language": language,
            ...
            TreeLogger.error("Error detecting language", e, {
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
    ...
    def detect_mixed_language(self, message: str) -> dict:
        ...
        """
        try:
            TreeLogger.debug("Detecting mixed language", {
                "message_length": len(message) if message else 0
            }, service=self.service_name)
            
            if not message:
                TreeLogger.warning("Empty message for mixed language detection", service=self.service_name)
                return {
                    "is_mixed": False,
                    "primary_language": "en",
            ...
            arabic_chars = len(self.arabic_pattern.findall(message))
            english_words = len([word for word in message.split() if word.isalpha() and word.isascii()])
            
            total_chars = len(message.replace(" ", ""))
            total_words = len(message.split())
            
            if total_chars == 0 or total_words == 0:
                TreeLogger.debug("No content to analyze for mixed language", service=self.service_name)
                return {
                    "is_mixed": False,
            ...
            TreeLogger.info("Mixed language analysis completed", {
                "is_mixed": is_mixed,
                "primary_language": primary_lang,
            ...
            TreeLogger.error("Error detecting mixed language", e, {
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
    ...
    def should_respond_in_arabic(self, message: str, user_preference: Optional[str] = None) -> bool:
        """
        ...
        """
        try:
            TreeLogger.debug("Determining response language", {
                "message_length": len(message) if message else 0,
            ...
            if user_preference:
                TreeLogger.debug("Using user preference for language", {
                    "preference": user_preference
                }, service=self.service_name)
                return user_preference.lower() == "ar"
            
            if not message:
                TreeLogger.debug("No message, defaulting to English", service=self.service_name)
                return False
            ...
            arabic_phrase_found = any(phrase in message for phrase in self.arabic_phrases)
            transliterated_found = any(phrase in message.lower() for phrase in self.transliterated_phrases)
            
            should_respond_arabic = (
                language == "ar" or 
                (confidence > 0.6 and language == "ar") or
                arabic_phrase_found or
                transliterated_found
            )
            
            TreeLogger.info("Response language determined", {
            ...
            TreeLogger.error("Error determining response language", e, {
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
    ...
    def get_language_appropriate_response(self, response: str, target_language: str) -> str:
        """
        ...
        """
        try:
            TreeLogger.debug("Getting language-appropriate response", {
            ...
            TreeLogger.debug("Returning original response", {
                "target_language": target_language
            }, service=self.service_name)
            ...
            TreeLogger.error("Error getting language-appropriate response", e, {
                "target_language": target_language,
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
    ...
    def extract_islamic_phrases(self, message: str) -> dict:
        ...
        """
        try:
            TreeLogger.debug("Extracting Islamic phrases", {
                "message_length": len(message) if message else 0
            }, service=self.service_name)
            
            if not message:
                TreeLogger.warning("Empty message for phrase extraction", service=self.service_name)
                return {
                    "arabic_phrases": [],
            ...
            arabic_phrases = [phrase for phrase in self.arabic_phrases if phrase in message]
            
            # Extract transliterated phrases
            message_lower = message.lower()
            transliterated_phrases = [phrase for phrase in self.transliterated_phrases if phrase in message_lower]
            
            total_phrases = len(arabic_phrases) + len(transliterated_phrases)
            
            TreeLogger.info("Islamic phrases extracted", {
            ...
            TreeLogger.error("Error extracting Islamic phrases", e, {
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
