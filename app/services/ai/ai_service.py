...
class AIService(BaseService):
    ...
    def __init__(self, bot):
        ...
        if self.config.openai_api_key:
            ...
            TreeLogger.warning("No OpenAI API key configured", service=self.service_name)
        ...
        try:
            ...
            TreeLogger.error("Failed to initialize tokenizer", e, service=self.service_name)
            self.tokenizer = None
        ...
        TreeLogger.info("AI enhancement modules initialized", {
            "modules": ["user_memory", "islamic_knowledge", "emotional_intelligence", "language_detection"]
...
Remember: You're not just an information source - you're a companion who loves Islam and wants to share its beauty with others."""
    ...
    def _load_knowledge_base(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if ISLAMIC_KNOWLEDGE_BASE:
                TreeLogger.info("Knowledge base loaded from module", {
            ...
            if os.path.exists(knowledge_path):
                with open(knowledge_path, 'r', encoding='utf-8') as f:
                    ...
                    TreeLogger.info("Knowledge base loaded from file", {
                        "path": knowledge_path,
            ...
            TreeLogger.error("Failed to load knowledge base", e, service=self.service_name)
            return {}
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            if not self.config.openai_api_key:
                TreeLogger.warning("AI service disabled - no API key", service=self.service_name)
            ...
            if self.client:
                try:
                    ...
                    TreeLogger.info("OpenAI API connection verified", service=self.service_name)
                except Exception as e:
                    TreeLogger.error("OpenAI API test failed", e, service=self.service_name)
                    # Don't fail initialization, just log the error
            
            TreeLogger.info("AI service initialized successfully", service=self.service_name)
            
        except Exception as e:
            TreeLogger.error("Failed to initialize AI service", e, service=self.service_name)
            await self.error_handler.handle_error(
                e,
                {"operation": "ai_service_init", "service": self.service_name}
            )
    ...
    def _format_knowledge_context(self) -> str:
        ...
        if 'islamic_phrases' in self.knowledge_base:
            phrases = list(self.knowledge_base['islamic_phrases'].get('common_expressions', {}).keys())
    ...
    def search_knowledge_base(self, query: str) -> Dict[str, Any]:
        ...
        if any(pillar in query_lower for pillar in ['shahada', 'salah', 'prayer', 'zakat', 'sawm', 'fasting', 'hajj']):
        ...
        if 'fatihah' in query_lower or 'fatiha' in query_lower:
            special_surahs = self.knowledge_base.get('quran_info', {}).get('special_surahs', {})
            relevant_info['surah_info'] = special_surahs.get('Al-Fatihah', {})
        
        # Search for prophets
        if any(term in query_lower for term in ['prophet', 'muhammad', 'pbuh', 'messenger']):
            relevant_info['prophets'] = self.knowledge_base.get('prophets', {})
        
        # Search for Islamic concepts
        if any(term in query_lower for term in ['tawheed', 'taqwa', 'ihsan', 'tawakkul']):
            relevant_info['concepts'] = self.knowledge_base.get('key_concepts', {})
        
        # Search for adhkar/dhikr
        if any(term in query_lower for term in ['dhikr', 'adhkar', 'remembrance', 'morning', 'evening']):
            relevant_info['adhkar'] = self.knowledge_base.get('daily_adhkar', {})
        
        # Search for Islamic calendar
        if any(term in query_lower for term in ['ramadan', 'eid', 'muharram', 'calendar', 'month']):
            relevant_info['calendar'] = self.knowledge_base.get('islamic_calendar', {})
        
        TreeLogger.debug("Knowledge base search completed", {
    ...
    def count_tokens(self, text: str) -> int:
        ...
        try:
            ...
            TreeLogger.debug("Tokens counted", {
    ...
    def calculate_cost(self, input_tokens: int, output_tokens: int, model: str = "gpt-3.5-turbo") -> float:
        ...
        TreeLogger.debug("Cost calculated", {
    ...
    async def generate_response(
        ...
        try:
            ...
            should_respond_arabic = self.language_detection.should_respond_in_arabic(question)
            
            # Detect emotional state
            emotional_context = self.emotional_intelligence.analyze_emotional_context(question)
            ...
            if user_context and user_context["is_returning_user"]:
                ...
                if greeting:
                    enriched_context.append(f"User context: Returning after a break. Consider warm greeting.")
            ...
            if should_respond_arabic:
                enriched_context.append("Respond primarily in Arabic with English translations where helpful.")
            elif language_info["has_arabic"]:
                enriched_context.append("User uses some Arabic phrases. Feel free to include Arabic terms with translations.")
            ...
            if relevant_knowledge:
                ...
                for topic, info in relevant_knowledge.items():
                    if isinstance(info, dict) and info:
                        knowledge_text += f"- {topic}: Available information on this topic\n"
            ...
            TreeLogger.debug("Generating AI response", {
            ...
            # Call OpenAI API
            TreeLogger.debug("Calling OpenAI API", {
            ...
            if should_respond_arabic:
                ai_response = self.language_detection.get_language_appropriate_response(ai_response, "ar")
            
            # Add related topics suggestion if appropriate
            if question_type in ["definition", "guidance"] and not emotional_context["needs_comfort"]:
                # Extract main topic from question
                main_topic = None
                for topic in ["prayer", "fasting", "hajj", "zakat", "marriage", "quran"]:
                    if topic in question.lower():
                        main_topic = topic
                        break
                
                if main_topic:
                    related_topics = self.islamic_knowledge.get_related_topics(main_topic)
                    if related_topics:
            ...
            cost = self.calculate_cost(input_tokens, output_tokens, self.config.openai_model)
            ...
            metadata = {
                ...
                "detected_emotion": emotional_context.get("emotion"),
                "question_type": question_type,
                "language": "ar" if should_respond_arabic else "en"
            }
            
            TreeLogger.info("AI response generated", {
                "input_tokens": input_tokens,
                "output_tokens": output_tokens,
                "total_tokens": total_tokens,
                "cost": f"${cost:.6f}",
                ...
                "question_type": question_type,
                "has_emotion": emotional_context["has_emotion"],
            ...
            TreeLogger.error("Failed to generate AI response", e, {
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            TreeLogger.debug("Cleaning up AI service resources", service=self.service_name)
            ...
            if hasattr(self, 'knowledge_base'):
                ...
                TreeLogger.debug("Knowledge base cleared from memory", service=self.service_name)
            
            TreeLogger.info("AI service cleanup completed", service=self.service_name)
            
        except Exception as e:
            TreeLogger.error("Error during AI service cleanup", e, {
                "error_type": type(e).__name__
            }, service=self.service_name)