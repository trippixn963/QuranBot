...
class UserMemory:
    ...
    def __init__(self, max_history_per_user: int = 10, history_timeout_hours: int = 24):
        ...
        try:
            ...
            self.user_histories: Dict[int, deque] = defaultdict(lambda: deque(maxlen=self.max_history))
            
            # user_id -> {topic: count}
            self.user_interests: Dict[int, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
            ...
            self.topic_keywords = {
            "prayer": ["salah", "pray", "prayer", "salat", "namaz", "wudu", "ablution", "قيام", "صلاة"],
            "fasting": ["fast", "fasting", "sawm", "siyam", "ramadan", "iftar", "suhoor", "صيام", "رمضان"],
            "zakat": ["zakat", "charity", "sadaqah", "giving", "zakah", "زكاة", "صدقة"],
            "hajj": ["hajj", "umrah", "pilgrimage", "mecca", "kaaba", "حج", "عمرة"],
            "quran": ["quran", "verse", "ayah", "surah", "recite", "recitation", "قرآن", "آية", "سورة"],
            "prophet": ["prophet", "muhammad", "pbuh", "messenger", "sunnah", "نبي", "محمد", "رسول"],
            "faith": ["iman", "faith", "belief", "believe", "tawheed", "إيمان", "عقيدة"],
            "dua": ["dua", "supplication", "prayer", "pray for", "دعاء"],
            "halal": ["halal", "haram", "permissible", "allowed", "forbidden", "حلال", "حرام"],
            "marriage": ["marriage", "nikah", "spouse", "husband", "wife", "marry", "نكاح", "زواج"],
            "death": ["death", "janazah", "grave", "afterlife", "akhirah", "موت", "آخرة"],
            "repentance": ["repent", "tawbah", "forgive", "sin", "mistake", "توبة", "استغفار"]
        }
        
            TreeLogger.info("User memory system initialized", {
            ...
            TreeLogger.error("Failed to initialize user memory system", e, {
    ...
    async def add_interaction(self, user_id: int, question: str, response: str) -> None:
        ...
        async with self._lock:
            try:
                ...
                TreeLogger.debug("Adding interaction to memory", {
                    "user_id": user_id,
                ...
                if not user_id or not isinstance(user_id, int):
                    TreeLogger.warning("Invalid user_id provided", {
                        "user_id": user_id,
                ...
                if not question or not response:
                    TreeLogger.warning("Empty question or response", {
                        "user_id": user_id,
                ...
                if len(self.user_histories) % 50 == 0:
                    TreeLogger.debug("Triggering periodic cleanup", {
                        "user_count": len(self.user_histories)
                    }, service=self.service_name)
                    await self._cleanup_old_data()
                
                TreeLogger.info("User interaction added to memory", {
                    "user_id": user_id,
                    "history_length": len(self.user_histories[user_id]),
                    "interests": dict(self.user_interests[user_id]),
                    "top_interest": max(self.user_interests[user_id].items(), key=lambda x: x[1])[0] if self.user_interests[user_id] else None
                }, service=self.service_name)
                
            except Exception as e:
                TreeLogger.error("Failed to add interaction to memory", e, {
                    "user_id": user_id,
                    "error_type": type(e).__name__,
                    "traceback": traceback.format_exc()
                }, service=self.service_name)
    ...
    def get_user_context(self, user_id: int) -> Dict[str, any]:
        ...
        """
        try:
            TreeLogger.debug("Retrieving user context", {
                "user_id": user_id
            }, service=self.service_name)
            
            # Validate user_id
            if not user_id or not isinstance(user_id, int):
                TreeLogger.warning("Invalid user_id for context retrieval", {
                    "user_id": user_id,
                    "type": type(user_id).__name__
                }, service=self.service_name)
            ...
            if user_id in self.user_histories:
                try:
                    ...
                    TreeLogger.warning("Error processing user history", e, {
                        "user_id": user_id,
                        "error_type": type(e).__name__
                    }, service=self.service_name)
            ...
            if user_id in self.user_interests:
                try:
                    ...
                    if interests:
                        ...
                        context["top_interests"] = [topic for topic, _ in sorted_interests[:3]]
                except Exception as e:
                    TreeLogger.warning("Error processing user interests", e, {
                        "user_id": user_id,
                        "error_type": type(e).__name__
                    }, service=self.service_name)
            
            TreeLogger.debug("User context retrieved", {
                "user_id": user_id,
            ...
            TreeLogger.error("Failed to get user context", e, {
                "user_id": user_id,
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
    ...
    def _track_interests(self, user_id: int, question: str) -> None:
        ...
        try:
            ...
            if topics_found:
                TreeLogger.debug("Topics detected in question", {
                    "user_id": user_id,
                    "topics": topics_found,
                    "question_length": len(question)
                }, service=self.service_name)
            ...
            TreeLogger.warning("Error tracking user interests", e, {
                "user_id": user_id,
                "error_type": type(e).__name__
            }, service=self.service_name)
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        async with self._lock:
            try:
                TreeLogger.debug("Starting cleanup of old data", {
                    "timeout_hours": self.timeout_hours,
                ...
                for user_id in list(self.user_histories.keys()):
                    try:
                        ...
                        while history and (current_time - history[0][2]) > timeout_delta:
                            history.popleft()
                            removed_entries += 1
                        ...
                        if not history:
                            ...
                            TreeLogger.debug("Removed user from memory", {
                                "user_id": user_id,
                                "entries_removed": initial_length
                            }, service=self.service_name)
                            
                    except Exception as e:
                        TreeLogger.warning("Error cleaning user data", e, {
                            "user_id": user_id,
                            "error_type": type(e).__name__
                        }, service=self.service_name)
                
                TreeLogger.info("Cleanup completed", {
                    "cleaned_users": cleaned_users,
                    "removed_entries": removed_entries,
                    "remaining_users": len(self.user_histories),
                    "total_entries": sum(len(h) for h in self.user_histories.values())
                }, service=self.service_name)
                
            except Exception as e:
                TreeLogger.error("Failed to clean up old data", e, {
                    "error_type": type(e).__name__,
                    "traceback": traceback.format_exc()
                }, service=self.service_name)
                ...
                await self.error_handler.handle_error(
                    e,
                    {
                        "operation": "_cleanup_old_data",
                        "service": self.service_name
                    }
                )
    
    def get_personalized_greeting(self, user_id: int) -> Optional[str]:
        """
        ...
        """
        try:
            TreeLogger.debug("Generating personalized greeting", {
                "user_id": user_id
            }, service=self.service_name)
            ...
            if context["is_returning_user"]:
                ...
                if context["top_interests"]:
                    ...
                    TreeLogger.debug("Customizing greeting based on interest", {
                        "user_id": user_id,
                        "top_interest": top_interest
                    }, service=self.service_name)
                    
                    if top_interest == "prayer":
                        greetings.append("Welcome back! Still seeking knowledge about Salah?")
                    elif top_interest == "quran":
                        greetings.append("Good to see you again! Ready to explore more of the Quran?")
                    elif top_interest == "fasting":
                        greetings.append("Assalamu alaikum! Back to learn more about fasting?")
                    elif top_interest == "hajj":
                        greetings.append("Welcome back! Continuing your journey learning about Hajj?")
                
                import random
                greeting = random.choice(greetings)
                
                TreeLogger.info("Personalized greeting generated", {
                    "user_id": user_id,
                    "is_returning": True,
            ...
            TreeLogger.error("Failed to generate personalized greeting", e, {
                "user_id": user_id,
                "error_type": type(e).__name__,
                "traceback": traceback.format_exc()
            }, service=self.service_name)
