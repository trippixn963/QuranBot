...
class OpenAIUsageTracker(BaseService):
    ...
    def __init__(self, bot):
        ...
        TreeLogger.debug("OpenAI Usage Tracker instance created", service=self.service_name)
    
    async def _initialize(self) -> bool:
        """Initialize the usage tracker."""
        try:
            TreeLogger.info("Initializing OpenAI Usage Tracker", service=self.service_name)
            
            if not self.config.openai_api_key:
                TreeLogger.warning("OpenAI API key not configured", service=self.service_name)
                return False
            
            # Test API connection
            usage = await self.fetch_current_month_usage()
            if usage:
                TreeLogger.info("OpenAI Usage Tracker initialized successfully", {
            ...
            await self.error_handler.handle_error(
                e,
                {"operation": "openai_usage_tracker_initialization"}
            )
            return False
    ...
    async def fetch_current_month_usage(self) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not token_tracker:
                TreeLogger.warning("Token tracker not available", service=self.service_name)
                # Return default data
                usage_data = {
                    'total_cost': 0.0,
                    'total_requests': 0,
                ...
                usage_data = {
                    ...
                    'remaining_budget': tracker_data.get('remaining_budget', self.config.openai_monthly_budget),
            ...
            TreeLogger.info("Usage data fetched", {
            ...
            TreeLogger.error("Failed to fetch usage data", e, {
    ...
    def _process_usage_data(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'daily_costs' in raw_data:
            for day_data in raw_data['daily_costs']:
                daily_costs.append({
                    ...
                    'cost': day_data.get('line_items', [{}])[0].get('cost', 0) / 100
                })
        ...
        budget_used_percent = (total_cost / self.config.openai_monthly_budget * 100) if self.config.openai_monthly_budget > 0 else 0
    ...
    async def get_usage_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'will_exceed_budget': projected_monthly_cost > self.config.openai_monthly_budget
        }
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            TreeLogger.debug("Cleaning up OpenAI usage tracker resources", service=self.service_name)
            ...
            TreeLogger.info("OpenAI usage tracker cleanup completed", service=self.service_name)
            
        except Exception as e:
            TreeLogger.error("Error during OpenAI usage tracker cleanup", e, {
                "error_type": type(e).__name__
            }, service=self.service_name)
