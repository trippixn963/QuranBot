...
class IslamicAIService(BaseService):
    ...
    def __init__(self, bot):
        ...
        TreeLogger.debug(
            "Islamic AI Service instance created", service=self.service_name
        )
    ...
    async def _initialize(self) -> bool:
        ...
        try:
            TreeLogger.info(
                "Initializing Islamic AI Service", service=self.service_name
            )

            TreeLogger.debug(
                "Islamic patterns loaded",
                {
            ...
            TreeLogger.info(
                "Islamic AI Service initialized successfully", service=self.service_name
            )
            return True

        except Exception as e:
            await self.error_handler.handle_error(
                e, {"operation": "islamic_ai_initialization"}
            )
            return False
    ...
    def detect_greeting(self, message: str) -> str | None:
        """
    ...
    def get_current_playing_context(self) -> dict[str, Any]:
        ...
        try:
            TreeLogger.debug(
                "Getting current playing context", service=self.service_name
            )

            audio_service = self.bot.services.get("audio")
            if not audio_service:
                TreeLogger.debug(
                    "Audio service not available", service=self.service_name
                )
                return {}

            # Check if playing
            if (
                not hasattr(audio_service, "voice_client")
                or not audio_service.voice_client
            ):
                return {}
            ...
            context = {
                ...
                "reciter": (
                    current_reciter_info.get("name", "Unknown")
                    if current_reciter_info
                    else "Unknown"
                ),
            }

            TreeLogger.debug(
                "Playing context retrieved",
                {
    ...
    def enhance_response(self, response: str) -> str:
        ...
        response = re.sub(
            r"Surah\s+([A-Za-z\-\s]+)\s*(\d+):(\d+)", r"📖 Surah \1 (\2:\3)", response
        )

        # Format standalone verse references (e.g., "verse 255" → "verse 255")
        response = re.sub(
            r"\bverse\s+(\d+)\b", r"verse \1", response, flags=re.IGNORECASE
        )

        # Add Hadith book emojis
        response = re.sub(r"\b(Sahih\s+)?(Bukhari|Muslim)\b", r"📚 \1\2", response)

        # Ensure proper formatting for Prophet's name
        response = re.sub(
            r"Prophet\s+Muhammad(?:\s+\(PBUH\))?", "Prophet Muhammad ﷺ", response
        )
    ...
    def get_related_suggestions(self, topic: str) -> list[str]:
        ...
        """
        TreeLogger.debug(
            "Generating related suggestions",
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            TreeLogger.debug(
                "Cleaning up Islamic AI service resources", service=self.service_name
            )
            ...
            TreeLogger.info(
                "Islamic AI service cleanup completed", service=self.service_name
            )

        except Exception as e:
            TreeLogger.error(
                "Error during Islamic AI service cleanup",
                e,
                {"error_type": type(e).__name__},
                service=self.service_name,
            )
