...
class EmotionalIntelligence:
    ...
    def __init__(self) -> None:
        ...
        try:
            TreeLogger.info(
                "Initializing emotional intelligence module", service=self.service_name
            )
            ...
            TreeLogger.error(
                "Failed to initialize emotional intelligence module",
                e,
    ...
    def detect_emotion(self, message: str) -> tuple[str | None, float]:
        """
        ...
        try:
            ...
            if not message:
                TreeLogger.warning(
                    "Empty message provided for emotion detection",
                    service=self.service_name,
                )
                return None, 0.0
            ...
            if not emotion_scores:
                TreeLogger.debug(
                    "No emotions detected in message", service=self.service_name
                )
                return None, 0.0

            # Find the emotion with highest score
            best_emotion = max(
                emotion_scores.keys(), key=lambda e: emotion_scores[e]["score"]
            )
            ...
            TreeLogger.debug(
                "Emotion detected",
                {
                    ...
                    "score": best_score,
                    "matched_keywords": emotion_scores[best_emotion][
                        "matched_keywords"
                    ],
    ...
    def get_emotional_response(self, emotion: str, confidence: float) -> dict[str, Any]:
        ...
        try:
            ...
            if not emotion or confidence < 0.3:
                TreeLogger.debug(
                    "Low confidence emotion, using general response",
                    service=self.service_name,
                )
                return {
            ...
            if not comfort_responses:
                TreeLogger.debug(
                    "No specific comfort response for emotion",
                    {"emotion": emotion},
                    service=self.service_name,
                )
                return {
            ...
            TreeLogger.error(
                "Error getting emotional response",
                e,
                {
                    "emotion": emotion,
                    "confidence": confidence,
    ...
    def analyze_emotional_context(self, message: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not message:
                TreeLogger.warning(
                    "Empty message provided for emotional analysis",
                    service=self.service_name,
                )
                return {
            ...
            needs_support = (
                emotion in ["sadness", "anxiety", "guilt", "loneliness"]
                and confidence >= 0.4
            )
            ...
            TreeLogger.error(
                "Error analyzing emotional context",
                e,
                {"error_type": type(e).__name__, "traceback": traceback.format_exc()},
                service=self.service_name,
            )
    ...
    def get_emotion_aware_greeting(self, emotion: str | None) -> str | None:
        """
        ...
        try:
            ...
            if greeting:
                TreeLogger.debug(
                    "Emotion-aware greeting provided",
                    {"emotion": emotion, "greeting_length": len(greeting)},
                    service=self.service_name,
                )
            ...
            TreeLogger.error(
                "Error getting emotion-aware greeting",
                e,
                {
                    "emotion": emotion,
                    "error_type": type(e).__name__,
