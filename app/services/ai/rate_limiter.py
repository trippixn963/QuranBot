...
class RateLimiter(BaseService):
    ...
    def __init__(self, bot):
        ...
        TreeLogger.debug("Rate Limiter instance created", {
    ...
    async def _initialize(self) -> bool:
        ...
        try:
            ...
            TreeLogger.info("Rate Limiter initialized", {
    ...
    async def _create_tables(self) -> None:
        ...
        TreeLogger.debug("Rate limit tables created successfully", service=self.service_name)
    
    async def check_mention_rate_limit(self, user_id: int, username: str) -> Tuple[bool, Optional[timedelta], str]:
        """
    ...
    async def check_user(self, user_id: int) -> Tuple[bool, Optional[timedelta]]:
        """
        ...
        if user_id == self.config.developer_id:
            TreeLogger.debug("Developer bypass for rate limit", {
                "user_id": user_id
            }, service=self.service_name)
            return True, None
        ...
        if user_id in self.cache:
            ...
            if time_since < timedelta(hours=1):
                ...
                TreeLogger.debug("Rate limit hit (from cache)", {
                    "user_id": user_id,
            ...
            TreeLogger.debug("Cache entry expired, checking database", {
                "user_id": user_id,
                "time_since": str(time_since)
            }, service=self.service_name)
        ...
        if not user_data:
            # First time user
            TreeLogger.debug("First time user, allowing mention", {
                "user_id": user_id
            }, service=self.service_name)
            return True, None
        ...
        if time_since >= timedelta(hours=1):
            TreeLogger.debug("Rate limit expired, allowing mention", {
                "user_id": user_id,
                "time_since": str(time_since)
            }, service=self.service_name)
            return True, None
        ...
        TreeLogger.debug("Rate limit active", {
            "user_id": user_id,
            "time_since": str(time_since),
            "time_until_next": str(time_until_next)
        }, service=self.service_name)
    ...
    async def record_mention(self, user_id: int, username: str) -> None:
        ...
        TreeLogger.debug("Recording AI mention", {
            "user_id": user_id,
            "username": username
        }, service=self.service_name)
        ...
        """, (str(user_id), now.isoformat(), now.isoformat(), now.isoformat(), now.isoformat()))
        
        TreeLogger.info("AI mention recorded", {
            "user": username,
            "user_id": user_id,
    ...
    async def get_user_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        TreeLogger.debug("Fetching user rate limit stats", {
            "user_id": user_id
        }, service=self.service_name)
        ...
        can_mention_next = last_mention + timedelta(hours=1) if time_since < timedelta(hours=1) else datetime.now()
        
        return {
            "has_used": True,
            "total_mentions": stats["total_mentions"],
            "last_mention": last_mention,
            "first_mention": datetime.fromisoformat(stats["first_mention"]) if stats["first_mention"] else None,
    ...
    def format_cooldown_message(self, time_remaining: timedelta) -> str:
        ...
        TreeLogger.debug("Formatting cooldown message", {
            "time_remaining": str(time_remaining),
            "total_seconds": total_seconds
        }, service=self.service_name)
        
        if total_seconds < 60:
            return f"Please wait {total_seconds} seconds before sending another message."
    ...
    async def _cache_cleanup_loop(self) -> None:
        ...
        while self.state == "running":
            try:
                ...
                if expired_users:
                    TreeLogger.debug(f"Cleaned {len(expired_users)} expired cache entries", 
                                   service=self.service_name)
                ...
                TreeLogger.error("Error in cache cleanup", e, {
                    "error_type": type(e).__name__,
                    "cache_size": len(self.cache)
                }, service=self.service_name)
                await self.error_handler.handle_error(
                    e,
                    {"operation": "cache_cleanup_loop"}
                )
                await asyncio.sleep(300)
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            TreeLogger.debug("Cleaning up rate limiter resources", service=self.service_name)
            
            # Clear cache
            if hasattr(self, 'cache'):
                cache_size = len(self.cache)
                self.cache.clear()
                TreeLogger.debug(f"Cleared {cache_size} cache entries", service=self.service_name)
            ...
            TreeLogger.error("Error during rate limiter cleanup", e, {
                "error_type": type(e).__name__
            }, service=self.service_name)
