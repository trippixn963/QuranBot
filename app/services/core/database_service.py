...
from ...core.errors import DatabaseError, ResourceError, ValidationError, ErrorSeverity, ErrorCategory
...
class QueryStats:
    ...
    def update(self, query_time: float, success: bool, query_type: str = "unknown") -> None:
...
class DatabaseService(BaseService):
    ...
    async def _initialize(self) -> None:
        ...
        TreeLogger.section("Initializing database service with error handling",
        ...
        try:
            ...
            TreeLogger.success("Database service initialization complete with error handling",
                               service="DatabaseService")
    ...
    async def _start(self) -> None:
        ...
        TreeLogger.section("Starting database service with monitoring",
                            service="DatabaseService")
        ...
        try:
            ...
            TreeLogger.success("Database service started successfully with monitoring",
                               service="DatabaseService")
    ...
    async def _stop(self) -> None:
        ...
        TreeLogger.section("Stopping database service with cleanup",
                            service="DatabaseService")
        ...
        try:
            ...
            TreeLogger.success("Database service stopped successfully with cleanup",
                               service="DatabaseService")
    ...
    async def _cleanup(self) -> None:
        ...
        TreeLogger.section("Cleaning up database service resources",
                            service="DatabaseService")
        ...
        try:
            ...
            TreeLogger.success("Database service cleanup complete with resource management",
                               service="DatabaseService")
    ...
    async def _ensure_database_directory(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Database directories ensured", {
                "database_path": str(self.db_path),
                "backup_folder": str(self.backup_dir),
    ...
    async def _establish_connection(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Database connection established successfully", {
    ...
    async def _initialize_schema(self) -> None:
        ...
        try:
            ...
            self.connection.execute("CREATE INDEX IF NOT EXISTS idx_user_stats_user_id ON user_stats(user_id)")
            indexes_created += 1
            
            self.connection.execute("CREATE INDEX IF NOT EXISTS idx_bot_state_guild_id ON bot_state(guild_id)")
            indexes_created += 1
            
            self.connection.execute("CREATE INDEX IF NOT EXISTS idx_playback_history_user_id ON playback_history(user_id)")
            indexes_created += 1
            
            self.connection.execute("CREATE INDEX IF NOT EXISTS idx_playback_history_timestamp ON playback_history(timestamp)")
            indexes_created += 1
            
            self.connection.execute("CREATE INDEX IF NOT EXISTS idx_system_logs_timestamp ON system_logs(timestamp)")
            indexes_created += 1
            ...
            TreeLogger.info("Database schema initialized successfully", {
    ...
    async def _validate_data_integrity(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Data integrity validation complete", {
            ...
            self.integrity_stats["last_integrity_check"] = datetime.now(APP_TIMEZONE).isoformat()
            
            if orphaned_playback_records == 0 and invalid_surah_records == 0 and invalid_bot_state_records == 0:
                self.integrity_stats["passed_checks"] += 1
            else:
                self.integrity_stats["failed_checks"] += 1
                self.integrity_stats["data_corruption_events"] += 1
                
                TreeLogger.warning("Data integrity issues detected", {
                        "orphaned_playback_records": orphaned_playback_records,
                        "invalid_surah_records": invalid_surah_records,
                        "invalid_bot_state_records": invalid_bot_state_records
                    }
    ...
    async def _setup_backup_system(self) -> None:
        ...
        try:
            ...
            if self.db_path.exists() and self.db_path.stat().st_size > 0:
                ...
                TreeLogger.info("Initial backup skipped (no data)", service="DatabaseService")
            
            TreeLogger.info("Backup system ready", {
                    "backup_directory": str(self.backup_dir),
                    "backup_interval_hours": 6,  # Default backup interval
                    "initial_backup": "completed" if self.backup_stats["total_backups"] > 0 else "skipped"
                }
            )
            ...
            raise DatabaseError(
                f"Failed to setup backup system: {e}",
                operation="setup_backup_system"
            )
    ...
    async def _start_background_tasks(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Background tasks started successfully", {
    ...
    async def _stop_background_tasks(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Background tasks stopped successfully", service="DatabaseService")
    ...
    async def _perform_backup(self, backup_type: str = "scheduled") -> None:
        ...
        try:
            ...
            self.backup_stats["last_backup_time"] = datetime.now(APP_TIMEZONE).isoformat()
            ...
            TreeLogger.info(f"Database backup completed successfully", {
    ...
    async def _cleanup_old_backups(self) -> None:
        ...
        """
        try:
            TreeLogger.debug("Starting backup cleanup process", {
            ...
            if not backup_files:
                TreeLogger.debug("No backup files found to clean up", service="DatabaseService")
                return
            ...
            cutoff_time = datetime.now(APP_TIMEZONE) - timedelta(days=self.config.backup_retention_days)
            ...
            for i, backup_file in enumerate(backup_files):
                ...
                # Check time-based retention
                    ...
                    should_remove = True
                    file_age_days = (datetime.now(APP_TIMEZONE).timestamp() - backup_file.stat().st_mtime) / 86400
                ...
                if should_remove:
                    ...
                    TreeLogger.debug(f"Removing old backup: {backup_file.name}", {
                        "reason": reason,
                        "size_mb": round(file_size_mb, 2),
                        "age_days": round((datetime.now(APP_TIMEZONE).timestamp() - backup_file.stat().st_mtime) / 86400, 1)
                    }, service="DatabaseService")
            ...
            if files_removed > 0:
                TreeLogger.info("Backup cleanup completed", {
                ...
                TreeLogger.debug("No backups needed cleanup", {
                    "total_backups": len(backup_files),
                    "newest_age_days": round((datetime.now(APP_TIMEZONE).timestamp() - backup_files[0].stat().st_mtime) / 86400, 1) if backup_files else 0
                }, service="DatabaseService")
            
        except Exception as e:
            # Non-critical error - log but don't fail
            TreeLogger.error(f"Error during backup cleanup: {e}", {
    ...
    async def _verify_connection(self) -> None:
        ...
        try:
            if not self.connection:
                raise DatabaseError("Database connection not established", operation="verify_connection")
    ...
    async def _log_startup_statistics(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Database startup statistics", {
                    "database_size_mb": round(self._get_database_size_mb(), 2),
                    "table_counts": table_counts,
                    "wal_mode_enabled": self._is_wal_mode_enabled(),
                    "available_disk_space_mb": round(self._get_available_disk_space_mb(), 2)
                }
            )
    ...
    async def _log_final_statistics(self) -> None:
        ...
        try:
            TreeLogger.info("Database final statistics", {
                    "total_queries": self.query_stats.total_queries,
    ...
    def _get_success_rate(self) -> float:
        ...
        if self.query_stats.total_queries == 0:
            return 100.0
        return (self.query_stats.successful_queries / self.query_stats.total_queries) * 100
    ...
    def _update_query_stats(self, query_time: float, success: bool, query_type: str = "unknown") -> None:
        """Update query statistics."""
    ...
    async def create_user_stats(self, user_id: int, username: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to create user stats: {e}",
                operation="create_user_stats"
            )
    
    async def _execute_query(self, query: str, params: tuple, query_type: str) -> Optional[sqlite3.Row]:
        """Execute database query with error handling."""
        try:
            TreeLogger.debug(f"Executing {query_type} query", {
            ...
            if query_type.upper() in ["SELECT"]:
                ...
                TreeLogger.debug(f"Query completed in {(time.time() - start_time)*1000:.2f}ms", {
                ...
                self.connection.commit()
                TreeLogger.debug(f"Query committed in {(time.time() - start_time)*1000:.2f}ms", {
                    "query_type": query_type,
    ...
    async def cleanup_old_logs(self, days: int = 30) -> int:
        ...
        try:
            ...
            TreeLogger.info(f"Cleaned up {deleted_count} old log entries", {
    ...
    async def execute(self, query: str, params: Optional[Tuple] = None) -> sqlite3.Cursor:
        """
        ...
        try:
            TreeLogger.debug("Executing query", {
            ...
            TreeLogger.debug("Query executed successfully", {
            ...
            query_time = time.time() - start_time
            self._update_query_stats(query_time, False, query.strip().split()[0].upper())
            
            TreeLogger.error("Database query failed", e, {
                "query_preview": query[:100] + "..." if len(query) > 100 else query,
                "error_type": type(e).__name__
            }, service=self.service_name)
    ...
    async def fetch_one(self, query: str, params: Optional[Tuple] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            TreeLogger.error("Failed to fetch one", e, {
                "query_preview": query[:100] + "..." if len(query) > 100 else query
            }, service=self.service_name)
    ...
    async def fetch_all(self, query: str, params: Optional[Tuple] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            TreeLogger.error("Failed to fetch all", e, {
                "query_preview": query[:100] + "..." if len(query) > 100 else query
            }, service=self.service_name)
