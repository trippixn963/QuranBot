...
from ...core.errors import ErrorHandler, ServiceError, ErrorSeverity, ErrorCategory
...
class BaseService(ABC):
    ...
    async def initialize(self) -> None:
        ...
        TreeLogger.section(f"Initializing service '{self.service_name}'", service="system")
    ...
    async def cleanup(self) -> None:
        ...
        TreeLogger.section(f"Cleaning up service '{self.service_name}'", service="system")
    ...
    async def _retry_operation(self, operation: callable, operation_name: str,
                              context: Dict[str, Any], max_retries: Optional[int] = None) -> Any:
        """
    ...
    def _update_performance_metrics(self, operation_time: float, success: bool) -> None:
        ...
        self.performance_metrics["last_operation_time"] = datetime.now(APP_TIMEZONE).isoformat()
    ...
    def _calculate_health_score(self, health_data: Dict[str, Any]) -> None:
        ...
        if self.health.warning_count > 0:
            score -= min(self.health.warning_count * 2, 20)  # Max 20 points for warnings
        ...
        if avg_response_time > 5.0:  # More than 5 seconds average
            score -= min((avg_response_time - 5.0) * 2, 20)  # Max 20 points for slow performance
    ...
    def _get_initialization_duration_ms(self) -> float:
        ...
        if self.initialization_time and self.state in [ServiceState.INITIALIZED, ServiceState.STARTING, ServiceState.RUNNING, ServiceState.STOPPING, ServiceState.STOPPED]:
            return (datetime.now(APP_TIMEZONE) - self.initialization_time.replace(tzinfo=APP_TIMEZONE)).total_seconds() * 1000
        return 0.0
    
    def _get_startup_duration_ms(self) -> float:
        """Get startup duration in milliseconds."""
        if self.start_time and self.state in [ServiceState.RUNNING, ServiceState.STOPPING, ServiceState.STOPPED]:
            return (datetime.now(APP_TIMEZONE) - self.start_time.replace(tzinfo=APP_TIMEZONE)).total_seconds() * 1000
        return 0.0
    
    def _get_uptime_seconds(self) -> float:
        """Get service uptime in seconds."""
        if self.start_time:
            return (datetime.now(APP_TIMEZONE) - self.start_time.replace(tzinfo=APP_TIMEZONE)).total_seconds()
        return 0.0
    
    def _get_total_lifetime_seconds(self) -> float:
        """Get total service lifetime in seconds."""
        if self.initialization_time:
            return (datetime.now(APP_TIMEZONE) - self.initialization_time.replace(tzinfo=APP_TIMEZONE)).total_seconds()
        return 0.0
