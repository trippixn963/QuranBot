...
from ...core.errors import StateError, ResourceError, ValidationError, ErrorSeverity, ErrorCategory
...
class BotState:
    ...
    current_position: PlaybackPosition = field(default_factory=lambda: PlaybackPosition(1, 0.0))
    volume: float = 1.0
    ...
    def model_dump(self, mode: str = "json") -> Dict[str, Any]:
        ...
        return {
            ...
            "playback_mode": self.playback_mode.value if hasattr(self.playback_mode, 'value') else self.playback_mode,
            "playback_state": self.playback_state.value if hasattr(self.playback_state, 'value') else self.playback_state,
            "current_position": {
...
class StateService(BaseService):
    ...
    def __init__(self):
        ...
        self.auto_save_interval = getattr(self.config, 'auto_save_interval', 300)  # 5 minutes
        self.corruption_checks_enabled = getattr(self.config, 'corruption_checks_enabled', True)
        self.max_history = getattr(self.config, 'max_history', 100)
    
    async def _initialize(self) -> None:
        """Initialize state service with retry mechanisms."""
        TreeLogger.info("Initializing state service with error handling", service="StateService")
        ...
        try:
            ...
            TreeLogger.info("State service initialization complete with error handling", service="StateService")
    ...
    async def _start(self) -> None:
        ...
        TreeLogger.info("Starting state service with monitoring", service="StateService")
        ...
        try:
            ...
            TreeLogger.info("State service started successfully with monitoring", service="StateService")
            
        except Exception as e:
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "state_service_startup",
                    "service_name": "StateService"
                }
            )
            raise StateError(
    ...
    async def _stop(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("State service stopped successfully with cleanup", service="StateService")
            
        except Exception as e:
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "state_service_shutdown",
                    "service_name": "StateService"
                }
            )
            raise StateError(
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("State service cleanup complete with resource management", service="StateService")
            
        except Exception as e:
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "state_service_cleanup",
                    "service_name": "StateService"
                }
            )
            raise StateError(
    ...
    async def _health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_data = {
                ...
                "state_checksum": self.state_checksum[:8] + "..." if self.state_checksum else None,
                ...
                "auto_save_task_running": self.auto_save_task is not None and not self.auto_save_task.done(),
                "state_monitor_task_running": self.state_monitor_task is not None and not self.state_monitor_task.done(),
    ...
    async def _ensure_state_directory(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("State directory ensured successfully", {
                    "state_directory": str(self.state_dir),
                    "state_file": str(self.state_file),
                    "backup_file": str(self.backup_file),
    ...
    async def _load_or_create_state(self) -> None:
        ...
        try:
            ...
            if self.state_file.exists():
                TreeLogger.info("Existing state file found, loading from persistent storage", {
                    "state_file": str(self.state_file),
                ...
                TreeLogger.info("No existing state file found, creating default state", {
                    "state_file": str(self.state_file),
            ...
            TreeLogger.info("State loaded successfully", {
            ...
            raise StateError(
                f"Failed to load or create state: {e}",
                operation="load_or_create_state"
            )
    ...
    async def _load_state_from_file(self) -> None:
        ...
        try:
            ...
            if not isinstance(state_data, dict):
                raise ValidationError("Invalid state file format - expected dictionary structure")
            ...
            # This provides debugging information and performance tracking
            TreeLogger.info("State loaded from file successfully", {
    ...
    async def _create_default_state(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Default state created successfully", {
                    "guild_id": self.bot_state.guild_id,
            ...
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "create_default_state",
                    "guild_id": self.config.guild_id
                }
            )
            raise StateError(
                f"Failed to create default state: {e}",
                operation="create_default_state"
            )
    ...
    def _create_bot_state_from_dict(self, state_data: Dict[str, Any]) -> BotState:
        ...
        try:
            ...
            current_position = PlaybackPosition(
                ...
                total_duration_seconds=current_position_data.get("total_duration_seconds"),
                timestamp=datetime.fromisoformat(current_position_data.get("timestamp", datetime.now(APP_TIMEZONE).isoformat()))
            )
            ...
            bot_state = BotState(
                ...
                current_reciter=state_data.get("current_reciter", self.config.default_reciter),
                playback_mode=PlaybackMode(state_data.get("playback_mode", "sequential")),
                playback_state=PlaybackState(state_data.get("playback_state", "stopped")),
                ...
                last_activity=datetime.fromisoformat(state_data.get("last_activity", datetime.now(APP_TIMEZONE).isoformat())),
                created_at=datetime.fromisoformat(state_data.get("created_at", datetime.now(APP_TIMEZONE).isoformat())),
                updated_at=datetime.fromisoformat(state_data.get("updated_at", datetime.now(APP_TIMEZONE).isoformat()))
            )
    ...
    async def _validate_state_integrity(self) -> None:
        ...
        try:
            if not self.bot_state:
                raise StateError("No state loaded for validation", operation="validate_state_integrity")
            ...
            if issues:
                TreeLogger.warning("State integrity issues detected", {
                        "issues": issues,
                        "guild_id": self.bot_state.guild_id,
                ...
                TreeLogger.info("State integrity validation passed", service="StateService")
    ...
    async def _fix_state_issues(self, issues: List[str]) -> None:
        ...
        try:
            ...
            if "Invalid current surah" in issues:
                self.bot_state.current_position.surah = max(1, min(114, self.bot_state.current_position.surah))
            
            if "Invalid position seconds" in issues:
                self.bot_state.current_position.position_seconds = max(0.0, self.bot_state.current_position.position_seconds)
            ...
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "fix_state_issues",
                    "issues": issues
                }
            )
            raise StateError(
                f"Failed to fix state issues: {e}",
                operation="fix_state_issues"
            )
    ...
    async def _setup_state_monitoring(self) -> None:
        ...
        try:
            TreeLogger.info("State monitoring setup complete", {
                    "auto_save_interval": self.auto_save_interval,
                    "corruption_checks_enabled": self.corruption_checks_enabled,
                    "max_history": self.max_history
                }
    ...
    async def _start_auto_save_task(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Auto-save task started successfully", {
                    "auto_save_interval": self.auto_save_interval,
                    "task_running": True
                }
            , service="StateService")
            ...
            raise StateError(
                f"Failed to start auto-save task: {e}",
                operation="start_auto_save_task"
            )
    ...
    async def _start_state_monitoring_task(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("State monitoring task started successfully", {
                    "corruption_checks_enabled": self.corruption_checks_enabled,
                    "task_running": True
                }
            , service="StateService")
    ...
    async def _stop_background_tasks(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Background tasks stopped successfully", service="StateService")
    ...
    async def _save_state(self, save_type: str = "auto_save") -> None:
        ...
        try:
            ...
            TreeLogger.debug(f"Starting state save operation", {
                "save_type": save_type,
            ...
            if self.state_file.exists():
                ...
                TreeLogger.debug("Created backup of existing state file", service="StateService")
            ...
            TreeLogger.info(f"State saved successfully", {
                    "save_type": save_type,
    ...
    async def _check_state_corruption(self) -> None:
        ...
        try:
            ...
            if self.state_checksum and current_checksum != self.state_checksum:
                TreeLogger.warning("State corruption detected", {
            ...
            await self.error_handler.handle_error(
                e,
                {
                    ...
                    "current_checksum": current_checksum[:8] + "..." if 'current_checksum' in locals() else None,
                    "stored_checksum": self.state_checksum[:8] + "..." if self.state_checksum else None
                }
            )
    ...
    async def _attempt_backup_recovery(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("State recovered from backup successfully", service="StateService")
    ...
    async def _log_startup_state(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("State service startup information", {
                    "guild_id": self.bot_state.guild_id,
                    "voice_channel_id": self.bot_state.voice_channel_id,
    ...
    async def _log_state_statistics(self) -> None:
        ...
        try:
            TreeLogger.info("State service statistics", self.state_stats
            , service="StateService")
            ...
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "log_state_statistics",
                    "service_name": "StateService"
                }
            )
    ...
    async def _log_final_statistics(self) -> None:
        ...
        try:
            TreeLogger.info("State service final statistics", {
                    **self.state_stats,
                    "total_listeners": len(self.state_change_listeners),
                    "state_changes_count": self.state_changes_count,
                    "last_change": self.last_state_change.isoformat() if self.last_state_change else None
                }
            )
    ...
    def _calculate_state_checksum(self) -> str:
        ...
        try:
            ...
            state_json = json.dumps(
                self.bot_state.model_dump(mode='json'),
                sort_keys=True,
                default=str
            )
    ...
    async def get_state_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "checksum": self.state_checksum[:8] + "..." if self.state_checksum else None
            }
    ...
    async def update_state(self, **kwargs) -> None:
        ...
        try:
            ...
            TreeLogger.debug(f"Updating state with {len(kwargs)} fields", {
            ...
            TreeLogger.info("State updated successfully", {
                    "update_fields": list(kwargs.keys()),
                    "state_changes_count": self.state_changes_count
                }
            )
        ...
        try:
            ...
            for field, value in updates.items():
                if hasattr(self.bot_state, field):
                    ...
                    TreeLogger.warning(f"Unknown state field: {field}", service="StateService")
            ...
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "perform_state_update",
                    "updates": updates
                }
            )
            raise StateError(
                f"Failed to perform state update: {e}",
                operation="perform_state_update"
            )
