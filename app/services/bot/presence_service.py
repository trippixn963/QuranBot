...
class PresenceService(BaseService):
    ...
    def __init__(self, bot: discord.Client):
        ...
        try:
            ...
            TreeLogger.debug(
                f"State directory ensured: {self.state_file.parent}",
                service="PresenceService",
            )
    ...
    async def _initialize(self) -> bool:
        ...
        try:
            TreeLogger.debug(
                "Starting presence service initialization", service="PresenceService"
            )

            # STEP 1: Load saved state
            state_loaded = self.load_state()
            TreeLogger.debug(
                f"State loading {'successful' if state_loaded else 'failed'}",
                {
            ...
            if not self.bot or not hasattr(self.bot, "change_presence"):
                raise ValueError(
                    "Invalid bot instance or missing change_presence method"
                )

            # STEP 3: Set initial presence
            TreeLogger.debug(
                "Setting initial connecting presence", service="PresenceService"
            )
            initial_update = await self.update_presence_with_template(
                "connecting", silent=True
            )

            if not initial_update:
                TreeLogger.warning(
                    "Failed to set initial presence", service="PresenceService"
                )

            TreeLogger.info(
                "Presence service initialized successfully",
                {
                    "is_enabled": self.is_enabled,
    ...
    async def update_presence(
        ...
        try:
            ...
            if not self.is_enabled:
                TreeLogger.debug(
                    "Presence update skipped - service disabled",
                    service="PresenceService",
                )
                return True
            ...
            if start_time:
                ...
                if start_time > now:
                    TreeLogger.warning(
                        "Start time is in the future, using current time",
                        {
            ...
            if not self.bot.ws:
                TreeLogger.warning(
                    "Bot websocket not ready, skipping presence update",
                    {
            ...
            update_duration = (
                datetime.now(APP_TIMEZONE) - update_start
            ).total_seconds()
            ...
            if not silent:
                ...
                TreeLogger.debug(
                    "Silent presence update completed",
                    {"update_duration_ms": round(update_duration * 1000, 2)},
                    service="PresenceService",
                )
            ...
            TreeLogger.debug(
                "Presence update skipped - connection closing during shutdown",
            ...
            self.failed_updates += 1
            TreeLogger.error(
                f"Discord API error updating presence: {e}",
                {
            ...
            if (
                "cannot write to closing transport" in error_message
                or "websocket is closed" in error_message
            ):
                TreeLogger.debug(
                    "Presence update skipped - connection closing (likely during shutdown)",
                    {
                        "error_type": type(e).__name__,
                        "is_shutting_down": self.bot.is_closed() if self.bot else True,
                    },
            ...
            self.failed_updates += 1
            TreeLogger.error(
                f"Unexpected error updating presence: {e}",
                {
                    "error_type": type(e).__name__,
                    "failed_updates": self.failed_updates,
                    "has_bot_connection": bool(
                        self.bot and hasattr(self.bot, "change_presence")
                    ),
                },
                service="PresenceService",
            )
    ...
    async def update_presence_with_template(
        ...
        try:
            ...
            if template_name not in self.PRESENCE_TEMPLATES:
                TreeLogger.error(
                    f"Invalid template requested: {template_name}",
                    {
            ...
            TreeLogger.debug(
                f"Using template '{template_name}' for presence update",
                {
            ...
            if not status:
                TreeLogger.warning(
                    "Empty status after formatting, using default",
                    {"template": status_template, "data": data},
                    service="PresenceService",
                )
                status = "QuranBot"
            ...
            TreeLogger.error(
                f"Unexpected error using template '{template_name}': {e}",
                {
                    "error_type": type(e).__name__,
                    "template_name": template_name,
                    "has_data": bool(data),
    ...
    def _format_template(self, template: str, data: dict[str, Any]) -> str:
        ...
        try:
            ...
            if not data:
                TreeLogger.debug(
                    "No data provided for template formatting",
                    {"template": template},
                    service="PresenceService",
                )
                return template
            ...
            if "{" in formatted and "}" in formatted:
                TreeLogger.warning(
                    "Template may have unfilled placeholders",
                    {
                        "template": template,
                        "result": formatted,
                        "data_keys": list(data.keys()),
            ...
            TreeLogger.warning(
                f"Missing template placeholder: {missing_key}",
                {
                    "template": template,
            ...
            TreeLogger.error(
                f"Invalid template format: {e}",
                {"template": template, "error_detail": str(e)},
                service="PresenceService",
            )
            return template
            ...
            TreeLogger.error(
                f"Unexpected error formatting template: {e}",
                {
                    "template": template,
                    "error_type": type(e).__name__,
    ...
    async def update_for_playback(
        ...
        try:
            ...
            if not surah_name or not reciter_name:
                TreeLogger.error(
                    "Invalid playback data - missing names",
                    {
            ...
            if current_verse < 1:
                TreeLogger.warning(
                    f"Invalid verse number: {current_verse}",
                    {"provided_verse": current_verse, "using_default": 1},
                    service="PresenceService",
                )
                current_verse = 1
            ...
            TreeLogger.debug(
                "Updating presence for playback",
                {
                    "surah": f"{surah_number}. {surah_name}",
                    "reciter": reciter_name,
                    "progress": (
                        f"{current_verse}/{total_verses}"
                        if total_verses > 0
                        else f"Verse {current_verse}"
                    ),
                    "template": "playing",
                },
                service="PresenceService",
            ...
            if success:
                TreeLogger.info(
                    "Playback presence updated",
                    {"surah": f"{surah_number}. {surah_name}", "reciter": reciter_name},
                    service="PresenceService",
                )
            else:
                TreeLogger.warning(
                    "Failed to update playback presence",
                    {"surah": surah_name, "is_enabled": self.is_enabled},
                    service="PresenceService",
                )
            ...
            TreeLogger.error(
                f"Error updating playback presence: {e}",
                {
                    "error_type": type(e).__name__,
    ...
    async def update_to_idle(self) -> bool:
        ...
        try:
            TreeLogger.debug(
                "Updating presence to idle state",
                {
            ...
            if success:
                ...
                TreeLogger.warning(
                    "Failed to update to idle presence",
                    {"is_enabled": self.is_enabled},
                    service="PresenceService",
                )
            ...
            TreeLogger.error(
                f"Error updating to idle presence: {e}",
                {"error_type": type(e).__name__, "traceback": True},
                service="PresenceService",
            )

            await self.error_handler.handle_error(e, {"operation": "update_to_idle"})
            return False
    ...
    async def clear_presence(self) -> bool:
        ...
        try:
            ...
            if not self.bot.ws:
                TreeLogger.warning(
                    "Bot websocket not ready, skipping presence clear",
                    {
                        "is_ready": self.bot.is_ready(),
                        "is_closed": self.bot.is_closed(),
                    },
            ...
            # This is expected during shutdown - just log as debug
            TreeLogger.debug(
                "Presence clear skipped - connection closing during shutdown",
                {"is_shutting_down": self.bot.is_closed() if self.bot else True},
                service="PresenceService",
            )
            return False
            ...
            TreeLogger.error(
                f"Discord API error clearing presence: {e}",
                {
                    "status_code": getattr(e, "status", None),
                    "error_text": getattr(e, "text", str(e)),
                },
            ...
            if (
                ...
                TreeLogger.debug(
                    "Presence clear skipped - connection closing (likely during shutdown)",
                    {
                        "error_type": type(e).__name__,
                        "is_shutting_down": self.bot.is_closed() if self.bot else True,
                    },
            ...
            TreeLogger.error(
                f"Error clearing presence: {e}",
                {"error_type": type(e).__name__, "has_bot_connection": bool(self.bot)},
                service="PresenceService",
            )

            await self.error_handler.handle_error(e, {"operation": "clear_presence"})
            return False
    ...
    def toggle_presence(self) -> bool:
        ...
        try:
            ...
            if not self.is_enabled:
                TreeLogger.info(
                    "Presence disabled - clearing activity", service="PresenceService"
                )
                ...
                TreeLogger.info(
                    "Presence enabled - ready for updates", service="PresenceService"
                )
            ...
            TreeLogger.info(
                f"Rich presence {'enabled' if self.is_enabled else 'disabled'}",
                {
    ...
    def save_state(self, silent: bool = False) -> bool:
        ...
        try:
            ...
            if not self.state_file.parent.exists():
                TreeLogger.debug(
                    f"Creating state directory: {self.state_file.parent}",
                    service="PresenceService",
                )
                self.state_file.parent.mkdir(parents=True, exist_ok=True)
            ...
            if not silent:
                TreeLogger.debug(
                    "Presence state saved successfully",
                    {
                        "is_enabled": self.is_enabled,
            ...
            TreeLogger.error(
                f"Permission denied saving presence state: {e}",
                {
            ...
            TreeLogger.error(
                f"JSON encoding error saving state: {e}",
                {
                    "error_detail": str(e),
            ...
            TreeLogger.error(
                f"Unexpected error saving presence state: {e}",
                {"error_type": type(e).__name__, "file_path": str(self.state_file)},
                service="PresenceService",
            )
            return False
    ...
    def load_state(self) -> bool:
        ...
        try:
            ...
            if not self.state_file.exists():
                TreeLogger.debug(
                    "No saved presence state found, using defaults",
                    {"state_file": str(self.state_file)},
                    service="PresenceService",
                )
            ...
            TreeLogger.debug(
                f"Loading presence state from {self.state_file}",
                service="PresenceService",
            )
            ...
            if not isinstance(state, dict):
                TreeLogger.warning(
                    "Invalid state file format, using defaults",
            ...
            if self.current_activity_type not in self.ACTIVITY_TYPES:
                TreeLogger.warning(
                    f"Invalid saved activity type: {self.current_activity_type}",
            ...
            if start_time:
                try:
                    ...
                    if datetime.now(APP_TIMEZONE) - restored_time < timedelta(hours=24):
                        ...
                        TreeLogger.debug(
                            "Ignoring old start time (>24 hours)",
                            service="PresenceService",
                        )
                        self.start_time = None
                except (ValueError, TypeError) as e:
                    TreeLogger.warning(
                        f"Invalid start time in saved state: {e}",
                        service="PresenceService",
                    )
                    self.start_time = None
            else:
            ...
            TreeLogger.info(
                "Presence state loaded successfully",
                {
                    "is_enabled": self.is_enabled,
            ...
            TreeLogger.error(
                f"Corrupted presence state file: {e}",
                {
                    "file_path": str(self.state_file),
                    "error_position": (
            ...
            TreeLogger.error(
                f"Permission denied reading presence state: {e}",
                {
                    "file_path": str(self.state_file),
            ...
            TreeLogger.error(
                f"Unexpected error loading presence state: {e}",
                {
                    "error_type": type(e).__name__,
                    "file_exists": self.state_file.exists(),
                    "file_size": (
                        self.state_file.stat().st_size
    ...
    async def _start(self) -> bool:
        ...
        try:
            ...
            if not self.bot:
                TreeLogger.error(
                    "Cannot start presence service without bot instance",
                    service="PresenceService",
                )
                return False
            ...
            # Service is ready
            TreeLogger.info(
                "Presence service started successfully",
                {
                    "is_enabled": self.is_enabled,
            ...
            TreeLogger.error(
                f"Error starting presence service: {e}",
                {"error_type": type(e).__name__},
                service="PresenceService",
            )
            return False
    ...
    async def _stop(self) -> bool:
        ...
        try:
            ...
            if self.is_enabled and self.current_status:
                TreeLogger.info(
                    "Clearing presence before shutdown", service="PresenceService"
                )
                try:
                    ...
                    if self.bot and self.bot.ws and not self.bot.is_closed():
                        ...
                        TreeLogger.debug(
                            "Skipping presence clear - bot connection already closed",
                            service="PresenceService",
                        )
                except Exception as e:
                    # Log but don't fail shutdown for presence clearing errors
                    TreeLogger.debug(
                        f"Could not clear presence during shutdown: {e}",
                        {
                            "error_type": type(e).__name__,
            ...
            TreeLogger.info(
                "Presence service stopped",
                {
                    ...
                    "failed_updates": self.failed_updates,
                    "success_rate": (
                        f"{((self.update_count - self.failed_updates) / self.update_count * 100):.1f}%"
            ...
            TreeLogger.error(
                f"Error stopping presence service: {e}",
                {"error_type": type(e).__name__},
                service="PresenceService",
            )
            return False
    ...
    async def _cleanup(self) -> bool:
        ...
        try:
            ...
            TreeLogger.info(
                "Presence service cleanup completed", service="PresenceService"
            )
            return True

        except Exception as e:
            TreeLogger.error(
                f"Error during presence service cleanup: {e}",
                {"error_type": type(e).__name__},
                service="PresenceService",
            )
            return False
    ...
    async def _health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not has_bot:
                TreeLogger.warning(
                    "Presence service unhealthy - invalid bot instance",
                    service="PresenceService",
                )
                is_healthy = False
            ...
            if self.update_count > 10:
                ...
                if failure_rate > 0.5:
                    TreeLogger.warning(
                        "Presence service unhealthy - high failure rate",
                        {
                            "total_updates": self.update_count,
                            "failed_updates": self.failed_updates,
            ...
            if self.state_file.parent.exists() and not os.access(
                str(self.state_file.parent), os.W_OK
            ):
                TreeLogger.warning(
                    "Presence service unhealthy - cannot write state",
            ...
            if self.is_enabled and self.last_update_time:
                ...
                if time_since_update > timedelta(hours=1):
                    TreeLogger.debug(
                        "Presence not updated recently",
                        {
                            "last_update": self.last_update_time.isoformat(),
                            "hours_ago": round(
                                time_since_update.total_seconds() / 3600, 1
                            ),
                        },
                        service="PresenceService",
                    )
            ...
            return {
                ...
                "time_since_update": (
                    str(time_since_update) if time_since_update else None
                ),
                "is_enabled": self.is_enabled,
                "issues": issues,
            }
            ...
            TreeLogger.error(
                f"Error during presence health check: {e}",
                {"error_type": type(e).__name__},
                service="PresenceService",
            )
            return {
