...
class UserInfo:
    ...
    joined_guild: Optional[datetime] = None
...
class VoiceInteraction:
    ...
    guild_id: int
    guild_name: str
    session_duration: Optional[float] = None  # For leaves, duration in channel
    previous_channel_id: Optional[int] = None  # For moves
...
class ControlPanelInteraction:
    ...
    # Interaction details
    selected_values: Optional[List[str]] = None  # For dropdowns
    input_values: Optional[Dict[str, str]] = None  # For modals
...
class UserInteractionLogger(BaseService):
    ...
    def __init__(self):
        ...
        self.recent_interactions: List[Union[VoiceInteraction, ControlPanelInteraction]] = []
        ...
        TreeLogger.info("User interaction logger initialized", {
    ...
    async def _start(self):
        ...
        try:
            TreeLogger.info("Starting user interaction logger service", service="UserInteractionLogger")
            
            # Start periodic cleanup task
            self.cleanup_task = asyncio.create_task(self._periodic_cleanup())
            
            TreeLogger.info("User interaction logger service started successfully", service="UserInteractionLogger")
            
        except Exception as e:
            TreeLogger.error(f"Failed to start user interaction logger: {e}", None, {
                "service": "UserInteractionLogger"
            }, service="UserInteractionLogger")
            raise
    ...
    async def _stop(self):
        ...
        try:
            TreeLogger.info("Stopping user interaction logger service", service="UserInteractionLogger")
            ...
            TreeLogger.info("User interaction logger service stopped successfully", service="UserInteractionLogger")
            
        except Exception as e:
            TreeLogger.error(f"Error stopping user interaction logger: {e}", None, {
                "service": "UserInteractionLogger"
            }, service="UserInteractionLogger")
    ...
    async def _cleanup(self):
        ...
        try:
            ...
            TreeLogger.info("User interaction logger cleanup completed", service="UserInteractionLogger")
            
        except Exception as e:
            TreeLogger.error(f"Error during user interaction logger cleanup: {e}", None, {
                "service": "UserInteractionLogger"
            }, service="UserInteractionLogger")
    ...
    async def _health_check(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "error_rate": (self.metrics["error_count"] / max(1, self.metrics["total_interactions"])) * 100
            }
        except Exception as e:
            return {
                "healthy": False,
    ...
    def _extract_user_info(self, user: discord.User, member: Optional[discord.Member] = None) -> UserInfo:
    ...
    async def log_voice_join(self, user: discord.User, channel: discord.VoiceChannel, 
                           member: Optional[discord.Member] = None) -> str:
        """
        ...
        try:
            ...
            # Log the interaction
            TreeLogger.info("🎤 User joined voice channel", {
                "user_id": user.id,
            ...
            TreeLogger.error(f"Error logging voice join: {e}", None, {
                "user_id": getattr(user, 'id', None),
                "channel_id": getattr(channel, 'id', None)
            }, service="UserInteractionLogger")
            return ""
    
    async def log_voice_leave(self, user: discord.User, channel: discord.VoiceChannel, 
                            member: Optional[discord.Member] = None) -> str:
        """
        ...
        try:
            ...
            if user.id in self.voice_sessions:
                ...
                session_duration = (timestamp - session_data["join_time"]).total_seconds()
            ...
            interaction = VoiceInteraction(
                ...
                additional_data={
                    "channel_user_count": len(channel.members),
                    "remaining_users": [m.display_name for m in channel.members if not m.bot]
                }
            )
            ...
            # Log the interaction
            TreeLogger.info("🚪 User left voice channel", {
                "user_id": user.id,
                "username": user_info.formatted_name,
                "discord_username": user_info.username,
                "display_name": user_info.display_name,
                ...
                "session_duration_formatted": self._format_duration(session_duration) if session_duration else None,
                "channel_user_count": len(channel.members),
                "remaining_users": [self._format_member_name(m) for m in channel.members if not m.bot]
            }, service="UserInteractionLogger")
            
            return interaction_id
            
        except Exception as e:
            TreeLogger.error(f"Error logging voice leave: {e}", None, {
                "user_id": getattr(user, 'id', None),
                "channel_id": getattr(channel, 'id', None)
            }, service="UserInteractionLogger")
            return ""
    
    async def log_control_panel_interaction(self, interaction: discord.Interaction, 
                                          component_type: str, component_id: str, 
                                          component_label: str, state_before: Dict[str, Any],
                                          selected_values: Optional[List[str]] = None,
                                          input_values: Optional[Dict[str, str]] = None) -> str:
        """
        ...
        """
        try:
            interaction_id = f"{component_type}_{interaction.user.id}_{int(time.time() * 1000)}"
            timestamp = datetime.now(APP_TIMEZONE)
            
            user_info = self._extract_user_info(interaction.user, getattr(interaction, 'member', None))
            
            control_interaction = ControlPanelInteraction(
                interaction_id=interaction_id,
                interaction_type=InteractionType.BUTTON_CLICK if component_type == "button" else InteractionType.DROPDOWN_SELECT,
                user_info=user_info,
            ...
            # Log the start
            TreeLogger.info(f"🎛️ Control panel {component_type} interaction started", {
            ...
            TreeLogger.error(f"Error logging control panel interaction start: {e}", None, {
    ...
    async def complete_control_panel_interaction(self, interaction_id: str, 
        ...
        try:
            ...
            for i, cached_interaction in enumerate(self.recent_interactions):
                if (isinstance(cached_interaction, ControlPanelInteraction) and 
                    cached_interaction.interaction_id == interaction_id):
                    ...
                    # Log completion
                    TreeLogger.info(f"✅ Control panel interaction completed", {
                        "interaction_id": interaction_id,
                        ...
                        "state_after": {
                            "current_surah": state_after.get("current_surah", {}) if state_after else {},
                            "current_reciter": state_after.get("current_reciter", {}) if state_after else {},
                            "is_playing": state_after.get("is_playing", False) if state_after else False,
                            "loop_mode": state_after.get("loop_mode", "off") if state_after else "off",
                            "shuffle_mode": state_after.get("shuffle_mode", False) if state_after else False
            ...
            TreeLogger.error(f"Error completing control panel interaction log: {e}", None, {
                "interaction_id": interaction_id
            }, service="UserInteractionLogger")
    
    def _cache_interaction(self, interaction: Union[VoiceInteraction, ControlPanelInteraction]) -> None:
    ...
    def _calculate_state_changes(self, before: Dict[str, Any], after: Dict[str, Any]) -> Dict[str, Any]:
        ...
        fields_to_check = [
            "current_surah", "current_reciter", "is_playing", "is_paused",
            "loop_mode", "shuffle_mode", "position", "duration"
        ]
    ...
    def _update_average_response_time(self, response_time_ms: float) -> None:
        ...
        if total_interactions > 1:
            self.metrics["average_response_time_ms"] = (
                (current_avg * (total_interactions - 1) + response_time_ms) / total_interactions
            )
        else:
        ...
        while True:
            try:
                ...
                for user_id, session_data in self.voice_sessions.items():
                    if (current_time - session_data["join_time"]).total_seconds() > 86400:
                ...
                if expired_sessions:
                    TreeLogger.info("Cleaned up expired voice sessions", {
                ...
                TreeLogger.error(f"Error in periodic cleanup: {e}", None, {
                    "service": "UserInteractionLogger"
                }, service="UserInteractionLogger")
    ...
    async def _log_final_metrics(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("📊 User Interaction Logger Final Metrics", {
                ...
                "error_rate_percent": (self.metrics["error_count"] / max(1, self.metrics["total_interactions"])) * 100,
                "active_voice_sessions": len(self.voice_sessions),
                "cached_interactions": len(self.recent_interactions)
            }, service="UserInteractionLogger")
            
        except Exception as e:
            TreeLogger.error(f"Error logging final metrics: {e}", None, {
                "service": "UserInteractionLogger"
            }, service="UserInteractionLogger")
