...
from ...config import get_config, ReciterName
from ...core.errors import ServiceError, BotError
from ...core.logger import log_event, TreeLogger
from ...data.models import AudioFileInfo, ReciterInfo, SurahInfo, AudioQuality
...
class MetadataCache(BaseService):
    ...
    def __init__(self):
        ...
        TreeLogger.info("🗂️ Metadata Cache Configuration", {
                ...
                "file_watching": "✅ Enabled" if self.enable_file_watching else "❌ Disabled",
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            TreeLogger.success("Metadata cache initialized", {
                    ...
                    "preloading": "✅ Complete" if self.enable_preloading else "⏭️ Skipped",
                    "file_watching": "✅ Active" if self.enable_file_watching else "❌ Disabled"
                }
            )
    ...
    async def _start(self) -> None:
        ...
        try:
            ...
            TreeLogger.success("Metadata cache started", {
                    "cleanup_task": "✅ Running",
                    "file_watcher": "✅ Running" if self.enable_file_watching else "❌ Disabled",
            ...
            raise ServiceError(
                f"Failed to start metadata cache: {e}",
                service_name="MetadataCache"
            )
    ...
    async def _stop(self) -> None:
        ...
        try:
            ...
            TreeLogger.success("Metadata cache stopped", {
            ...
            TreeLogger.error("Error during metadata cache stop", None, {"error": str(e)})
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            ...
            TreeLogger.success("Metadata cache cleanup complete", {
                    "caches": "✅ Cleared",
                    "statistics": "✅ Reset",
                    "memory": "✅ Released"
                }
    ...
    async def _health_check(self) -> Dict[str, Any]:
        ...
        try:
            # Cache status
            total_entries = (len(self.audio_metadata_cache) + 
            ...
            health_data["background_tasks"] = {
                "cleanup": "running" if self.cleanup_task and not self.cleanup_task.done() else "stopped",
                "file_watcher": "running" if self.file_watcher_task and not self.file_watcher_task.done() else "stopped"
            }
    ...
    async def _ensure_cache_directory(self) -> None:
        ...
        try:
            if not self.cache_dir.exists():
                TreeLogger.info(f"Creating cache directory: {self.cache_dir}", service="system")
                self.cache_dir.mkdir(parents=True, exist_ok=True)
                
                TreeLogger.success("Cache directory created", {"path": str(self.cache_dir)}
                )
    ...
    async def _load_cache_from_disk(self) -> None:
        ...
        try:
            ...
            TreeLogger.success("Cache loaded from disk", {
            ...
            TreeLogger.error(f"Failed to load cache from disk: {e}", None, service="system")
    ...
    async def _save_cache_to_disk(self) -> None:
        ...
        try:
            ...
            cache_data = {
                'audio_metadata': {
                    key: info.model_dump() for key, info in self.audio_metadata_cache.items()
                },
                'reciters': {
            ...
            TreeLogger.info("Cache saved to disk", {
    ...
    async def _initialize_surah_metadata(self) -> None:
        ...
        try:
            ...
            for i in range(1, 115):  # Surahs 1-114
                surah_name = surah_names[i-1] if i-1 < len(surah_names) else f"Surah {i}"
            ...
            TreeLogger.success("Surah metadata initialized", {
                    "total_surahs": len(self.surah_cache),
                    "meccan_surahs": sum(1 for s in self.surah_cache.values() if s.is_meccan),
                    "medinan_surahs": sum(1 for s in self.surah_cache.values() if s.is_medinan)
                }
            )
            
        except Exception as e:
            TreeLogger.error("Failed to initialize Surah metadata", None, {"error": str(e)})
    ...
    async def _preload_audio_metadata(self) -> None:
        ...
        try:
            ...
            if not audio_folder.exists():
                TreeLogger.warning(f"Audio folder not found: {audio_folder}", service="system")
                return
            ...
            for reciter_folder in audio_folder.iterdir():
                ...
                TreeLogger.debug(f"Preloading metadata for {reciter_name}", service="system")
                ...
                for audio_file in reciter_folder.glob("*.mp3"):
                    try:
                        ...
                        duration = 0.0
                        if cache_key in self.audio_metadata_cache:
                            duration = self.audio_metadata_cache[cache_key].duration_seconds
                        ...
                        file_info = AudioFileInfo(
                            ...
                            created_at=datetime.fromtimestamp(audio_file.stat().st_ctime, APP_TIMEZONE),
                        ...
                        TreeLogger.warning(f"Skipping invalid audio file {audio_file}: {e}", service="system")
                        continue
            ...
            TreeLogger.success("Audio metadata preloading complete", {
            ...
            TreeLogger.error("Audio metadata preloading failed", None, {"error": str(e)})
    
    # =========================================================================
    # Cache Operations
    # =========================================================================
    
    async def get_audio_metadata(self, reciter: str, surah_number: int) -> Optional[AudioFileInfo]:
        ...
        try:
            ...
            TreeLogger.debug(f"Cache lookup for audio metadata", {
                "cache_key": cache_key,
                "reciter": reciter,
                "surah": surah_number,
            ...
            if audio_file.exists():
                metadata = await self._create_audio_metadata(audio_file, reciter, surah_number)
            ...
            TreeLogger.error(f"Failed to get audio metadata for {reciter}:{surah_number}", None, {"error": str(e)})
            return None
    ...
    async def _extract_audio_duration(self, file_path: str) -> Optional[float]:
        ...
        try:
            ...
            if audio.info and hasattr(audio.info, 'length'):
                ...
                TreeLogger.debug(f"Extracted duration: {duration}s for {file_path}", service="MetadataCache")
                return duration
                
        except Exception as e:
            TreeLogger.error(f"Error extracting duration from {file_path}: {e}", service="MetadataCache")
            
        return None
    
    async def _create_audio_metadata(self, audio_file: Path, reciter: str, surah_number: int) -> AudioFileInfo:
        ...
        try:
            ...
            TreeLogger.error(f"Failed to create metadata for {audio_file}", None, {"error": str(e)})
            raise
    ...
    def _track_lookup_time(self, lookup_time: float) -> None:
        ...
        if self.lookup_times:
            self.cache_stats["average_lookup_time"] = sum(self.lookup_times) / len(self.lookup_times)
    ...
    async def _calculate_memory_usage(self) -> None:
        ...
        try:
            ...
            TreeLogger.error("Failed to calculate memory usage", None, {"error": str(e)})
        ...
        async def cleanup_loop():
            while True:
                try:
                    ...
                    TreeLogger.error("Cache cleanup task error", None, {"error": str(e)})
        
        self.cleanup_task = asyncio.create_task(cleanup_loop())
        TreeLogger.info(f"Cache cleanup task started (interval: {self.auto_cleanup_interval}s)", service="system")
    ...
    async def _stop_background_tasks(self) -> None:
        ...
        for task_name, task in tasks_to_stop:
            try:
                ...
                TreeLogger.error(f"Error stopping {task_name} task", None, {"error": str(e)})
    ...
    async def _perform_cache_cleanup(self) -> None:
        ...
        try:
            ...
            if self.cache_stats["cache_size_mb"] > self.memory_limit_mb:
                TreeLogger.info("Cache memory limit exceeded, performing cleanup", service="system")
            ...
            TreeLogger.debug("Cache cleanup completed", {
    ...
    async def _check_file_changes(self) -> None:
        ...
        try:
                ...
                for audio_file in directory.glob("*.mp3"):
                    ...
                    if file_path in self.file_modification_times:
                        if current_mtime > self.file_modification_times[file_path]:
                            ...
                            TreeLogger.info(f"File change detected: {audio_file.name}", service="system")
            ...
            if changes_detected > 0:
                TreeLogger.info(f"Detected {changes_detected} file changes", service="system")
    ...
    async def _setup_file_watching(self) -> None:
        ...
        try:
            audio_folder = self.config.audio_folder
            if not audio_folder.exists():
                TreeLogger.warning("Audio folder not found for file watching", service="system")
                return
            ...
            TreeLogger.info(f"File watching setup complete", {"watched_directories": len(self.watched_directories)}
            )
    ...
    async def _log_startup_statistics(self) -> None:
        ...
        TreeLogger.info("Metadata cache startup statistics", context={
    ...
    async def _log_final_statistics(self) -> None:
        ...
        TreeLogger.info("Metadata cache final statistics", context={
                "total_requests": self.cache_stats["total_requests"],
    ...
    async def invalidate_cache(self, pattern: Optional[str] = None) -> int:
        ...
        try:
            if pattern is None:
                ...
                count = 0
                keys_to_remove = [key for key in self.audio_metadata_cache.keys() if pattern in key]
            ...
            TreeLogger.info(f"Cache invalidated: {count} entries removed", service="system")
            return count
