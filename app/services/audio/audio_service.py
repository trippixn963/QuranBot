...
from ...core.errors import AudioError, NetworkError, DiscordAPIError, ResourceError, ValidationError, ErrorSeverity, ErrorCategory
from ...core.logger import TreeLogger, log_event
from ...core.errors import ErrorHandler
from ...core.validation import (
    FileSystemValidator, DependencyValidator, CompositeValidator
)
...
class AudioService(BaseService):
    ...
    def __init__(self, bot: discord.Client):
        ...
        self.current_reciter_name = str(self.config.default_reciter.value) if hasattr(self.config.default_reciter, 'value') else str(self.config.default_reciter)
        self.manual_control_active = False  # Flag to pause auto-monitoring during manual interactions
    ...
    async def _initialize(self) -> None:
        ...
        TreeLogger.section("Initializing audio service with error handling", service="AudioService")
        ...
        try:
            ...
            TreeLogger.success("Audio service initialization complete with error handling", service="AudioService")
    ...
    async def _start(self) -> None:
        ...
        TreeLogger.section("Starting audio service with robust connection handling", service="AudioService")
        
        try:
            TreeLogger.info("Audio service started - voice connection will be established after bot is ready", service="AudioService")
            TreeLogger.success("Audio service started successfully with error handling", service="AudioService")
            
        except Exception as e:
            TreeLogger.error(f"Failed to start audio service: {e}", service="AudioService")
            raise
    
    async def _stop(self) -> None:
        """Stop audio service with cleanup."""
        TreeLogger.section("Stopping audio service with comprehensive cleanup", service="AudioService")
        ...
        try:
            ...
            if self.continuous_playback_task and not self.continuous_playback_task.done():
            ...
            TreeLogger.success("Audio service stopped successfully with cleanup", service="AudioService")
    ...
    async def connect_and_start_playback(self) -> None:
        ...
        try:
            TreeLogger.section("Connecting to voice channel and starting playback", service="AudioService")
            
            # Connect to voice channel if configured with retry
            if self.config.voice_channel_id:
                TreeLogger.info(f"Attempting to connect to voice channel {self.config.voice_channel_id}", service="AudioService")
                
                await self._retry_operation(
                    operation=lambda: self._connect_to_voice_channel(self.config.voice_channel_id),
                ...
                TreeLogger.success("Voice connection and playback started successfully", service="AudioService")
            else:
                TreeLogger.warning("No voice channel configured", service="AudioService")
    ...
    async def _start_continuous_playback(self) -> None:
        ...
        try:
            TreeLogger.section("Starting continuous 24/7 Quran audio playback", service="AudioService")
            
            # Start the continuous playback task
            self.continuous_playback_task = asyncio.create_task(self._continuous_playback_loop())
            
            TreeLogger.success("Continuous playback task started successfully", service="AudioService")
    ...
    async def _continuous_playback_loop(self) -> None:
        ...
        """
        try:
            TreeLogger.section("Starting continuous playback loop", service="AudioService")
            ...
            while True:
                try:
                    ...
                    if self.manual_control_active:
                        TreeLogger.debug("Skipping main loop iteration - manual control active", service="AudioService")
                    ...
                    if not self.is_connected or not self.voice_client:
                        TreeLogger.error("Voice connection lost, attempting to reconnect", service="AudioService")
                        ...
                        if not reconnection_success:
                            TreeLogger.error("Failed to reconnect to voice channel, waiting before retry", service="AudioService")
                        ...
                        TreeLogger.success("Successfully reconnected to voice channel", service="AudioService")
                    ...
                    audio_file = await self._get_audio_file(self.current_surah_index, self.current_reciter_name)
                    
                    if not audio_file:
                        TreeLogger.error(f"No audio file found for surah {self.current_surah_index}", service="AudioService")
                        # Move to next surah if current one is missing
                        self.current_surah_index = (self.current_surah_index % 114) + 1
                        self.current_surah = self.current_surah_index  # Keep both variables in sync
                        continue
                    
                    # STEP 5: Start Surah Playback
                    # Play the current surah with comprehensive logging
                    TreeLogger.success(f"Playing surah {self.current_surah_index} by {self.current_reciter_name}", service="AudioService")
                    
                    # Add formatted info log for user visibility
                    TreeLogger.info(f"🎵 Now playing: Surah {self.current_surah_index} by {self.current_reciter_name}", 
                                  service="AudioService")
                    
                    success = await self.play_surah(self.current_surah_index, self.current_reciter_name)
                    ...
                    if success:
                        ...
                        timeout_seconds = 600  # 10 minutes max per surah (prevents infinite hanging)
                        
                        TreeLogger.section(f"Waiting for surah {self.current_surah_index} to complete...", service="AudioService")
                        ...
                        while self.playback_state == PlaybackState.PLAYING:
                            ...
                            if self.manual_control_active:
                                TreeLogger.debug("Skipping monitoring - manual control active", service="AudioService")
                            ...
                            if self.voice_client and not self.voice_client.is_playing() and not self.manual_control_active:
                                TreeLogger.success(f"Voice client stopped playing surah {self.current_surah_index}", service="AudioService")
                                self.playback_state = PlaybackState.STOPPED
                                break
                            ...
                            if time.time() - start_time > timeout_seconds:
                                TreeLogger.error(f"Playback timeout for surah {self.current_surah_index}", service="AudioService")
                        ...
                        TreeLogger.success(f"Completed surah {self.current_surah_index}, moving to next surah", service="AudioService")
                        
                        # Add formatted info log for user visibility
                        TreeLogger.info(f"✅ Completed Surah {self.current_surah_index}, moving to next surah...", 
                                      service="AudioService")
                        ...
                        if self.loop_mode == "single":
                            ...
                            TreeLogger.info(f"Loop mode is 'single', replaying surah {self.current_surah_index}", service="AudioService")
                            ...
                            TreeLogger.info(f"Shuffle mode: randomly selected surah {self.current_surah_index}", service="AudioService")
                        else:
                        ...
                        TreeLogger.error(f"Failed to play surah {self.current_surah_index}", service="AudioService")
                        await asyncio.sleep(5)  # Wait before retrying
                
                # STEP 15: Graceful Shutdown Handling
                # Handle cancellation when service is being stopped
                except asyncio.CancelledError:
                    TreeLogger.section("Continuous playback loop cancelled", service="AudioService")
                    break
    ...
    async def _cleanup(self) -> None:
        ...
        TreeLogger.section("Cleaning up audio service resources", service="AudioService")
        ...
        try:
            ...
            TreeLogger.success("Audio service cleanup complete with resource management", service="AudioService")
            
        except Exception as e:
            await self.error_handler.handle_error(
                e,
                {
                    "operation": "audio_service_cleanup",
                    "service_name": "AudioService"
                }
            )
            raise AudioError(
    ...
    async def _health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_data = {
                ...
                "cached_audio_files": sum(len(files) for files in self.audio_files_cache.values()),
                ...
                "voice_client_status": self.voice_client.is_connected() if self.voice_client else False
            }
    ...
    async def _restore_playback_state(self) -> None:
        ...
        try:
            TreeLogger.section("Restoring playback state from saved data", service="AudioService")
            
            state_service = await self._get_state_service()
            if not state_service:
                default_reciter_str = str(self.config.default_reciter.value) if hasattr(self.config.default_reciter, 'value') else str(self.config.default_reciter)
                TreeLogger.warning("State service not available, using default settings", {
            ...
            if not current_state:
                default_reciter_str = str(self.config.default_reciter.value) if hasattr(self.config.default_reciter, 'value') else str(self.config.default_reciter)
                TreeLogger.info("No saved state found, using default settings", {
                    "default_reciter": default_reciter_str,
                    "default_surah": 1
                }, service="AudioService")
                return
            ...
            if hasattr(current_state, 'current_reciter') and current_state.current_reciter:
                ...
                if saved_reciter != default_reciter_str:
                    ...
                    if saved_reciter in self.available_reciters:
                        ...
                        TreeLogger.success(f"Restored saved reciter: {self.current_reciter_name}", {
                            "saved_reciter": saved_reciter,
                            "default_reciter": default_reciter_str
                        }, service="AudioService")
                    else:
                        TreeLogger.warning(f"Saved reciter '{saved_reciter}' not available, using default", {
                            "saved_reciter": saved_reciter,
                            "default_reciter": default_reciter_str,
                            "available_reciters": list(self.available_reciters.keys())
                        }, service="AudioService")
                else:
                    TreeLogger.info(f"Using default reciter: {default_reciter_str}", service="AudioService")
            
            # Restore current position
            if hasattr(current_state, 'current_position') and current_state.current_position:
                ...
                if hasattr(position, 'surah') and position.surah:
                    ...
                    if 1 <= saved_surah <= 114:
                        ...
                        TreeLogger.success(f"Restored playback position: Surah {self.current_surah_index}", {
                            "restored_surah": self.current_surah_index,
                            "reciter": self.current_reciter_name,
                            "timestamp": position.timestamp.isoformat() if hasattr(position, 'timestamp') and position.timestamp else "unknown"
                        }, service="AudioService")
                    else:
                        TreeLogger.warning(f"Invalid saved surah number: {saved_surah}, starting from Surah 1", service="AudioService")
                        
            # Log final restoration summary
            TreeLogger.success("Playback state restoration completed", {
            ...
            default_reciter_str = str(self.config.default_reciter.value) if hasattr(self.config.default_reciter, 'value') else str(self.config.default_reciter)
            TreeLogger.error(f"Failed to restore playback state: {e}", None, {
    ...
    async def _save_current_position(self) -> None:
        ...
        try:
            state_service = await self._get_state_service()
            if not state_service:
                TreeLogger.warning("State service not available, cannot save position", service="AudioService")
                return
            ...
            TreeLogger.success(f"Saved current position: Surah {self.current_surah_index}", {
                "surah": self.current_surah_index,
                "reciter": self.current_reciter_name,
                "playback_state": self.playback_state,
            ...
            TreeLogger.error(f"Failed to save current position: {e}", None, {
                "error_type": type(e).__name__,
    ...
    async def _connect_to_voice_channel(self, channel_id: int) -> bool:
        ...
        """
        try:
            TreeLogger.info(f"Connecting to voice channel {channel_id} with error handling", service="AudioService")
            
            TreeLogger.debug(f"Starting voice connection process", {
                "channel_id": channel_id,
                "guild_id": self.config.guild_id,
                "has_existing_connection": bool(self.voice_client),
                "existing_connected": self.voice_client.is_connected() if self.voice_client else False
            }, service="AudioService")
            ...
            self.connection_health["last_connection_time"] = datetime.now(APP_TIMEZONE).isoformat()
            ...
            TreeLogger.info(f"Voice connection established successfully", {
                    "guild": guild.name,
    ...
    async def _disconnect_voice(self) -> None:
        ...
        try:
            TreeLogger.debug("Attempting to disconnect voice client", {
            ...
            TreeLogger.info("Voice connection disconnected successfully", service="AudioService")
    ...
    async def reconnect_voice(self) -> bool:
        ...
        if not self.current_voice_channel_id:
            TreeLogger.warning("No voice channel to reconnect to", service="AudioService")
            return False
        
        try:
            TreeLogger.debug("Starting voice reconnection process", {
                "target_channel_id": self.current_voice_channel_id,
                "current_voice_client": bool(self.voice_client),
                "is_connected": self.is_connected,
                "connection_retry_count": self.connection_retry_count
            }, service="AudioService")
            TreeLogger.info(f"Reconnecting to voice channel {self.current_voice_channel_id}", service="AudioService")
            
            # Check if the voice channel still exists and is accessible
            guild = self.bot.get_guild(self.config.guild_id)
            if not guild:
                TreeLogger.error(f"Guild {self.config.guild_id} not found during reconnection", service="AudioService")
                return False
            
            TreeLogger.debug("Guild found during reconnection", {
                "guild_id": guild.id,
            ...
            if not channel:
                TreeLogger.error(f"Voice channel {self.current_voice_channel_id} not found during reconnection", service="AudioService")
                # Reset the current voice channel ID since it doesn't exist
                self.current_voice_channel_id = None
                return False
            
            if not isinstance(channel, discord.VoiceChannel):
                TreeLogger.error(f"Channel {self.current_voice_channel_id} is not a voice channel", service="AudioService")
                self.current_voice_channel_id = None
                return False
            
            # Check bot permissions
            permissions = channel.permissions_for(guild.me)
            TreeLogger.debug("Checking bot permissions for voice channel", {
            ...
            if not permissions.connect:
                TreeLogger.error(f"Bot lacks permission to connect to channel {self.current_voice_channel_id}", service="AudioService")
                return False
            
            # Disconnect first if connected
            if self.voice_client:
                TreeLogger.debug("Disconnecting existing voice client before reconnection", service="AudioService")
                await self._disconnect_voice()
            
            # Attempt reconnection with retry logic
            success = await self._retry_operation(
                operation=lambda: self._connect_to_voice_channel(self.current_voice_channel_id),
            ...
            if success:
                ...
                TreeLogger.error("Voice reconnection failed after all retries", service="AudioService")
                return False
    ...
    async def _validate_audio_folder(self) -> None:
        ...
        try:
            ...
            TreeLogger.info("Audio folder validation successful", {
    ...
    async def _extract_audio_duration(self, file_path: str) -> Optional[float]:
        ...
        try:
            ...
            if audio.info and hasattr(audio.info, 'length'):
                ...
                TreeLogger.debug(f"Extracted duration: {duration}s for {file_path}", service="AudioService")
                return duration
                
        except Exception as e:
            TreeLogger.error(f"Error extracting duration from {file_path}: {e}", service="AudioService")
    ...
    async def _discover_reciters(self) -> None:
        ...
        try:
            ...
            available_reciter_folders = [folder for folder in audio_folder.iterdir() 
                                       if folder.is_dir() and not folder.name.startswith('.')]
            
            TreeLogger.debug(f"Found {len(available_reciter_folders)} reciter folders", {
            ...
            for reciter_folder in available_reciter_folders:
                ...
                if not reciter_folder.exists():
                    TreeLogger.warning(f"Reciter folder not found: {reciter_name}", {"reciter_folder": str(reciter_folder)}
                    )
                    continue
                
                TreeLogger.debug(f"Scanning reciter: {reciter_name}", service="AudioService")
                ...
                for audio_file in reciter_folder.glob("*.mp3"):
                    try:
                        ...
                        if not (1 <= surah_number <= 114):
                            TreeLogger.warning(f"Invalid surah number in file: {audio_file.name}", {"surah_number": surah_number}
                            , service="AudioService")
                            continue
                        ...
                        audio_info = AudioFileInfo(
                            ...
                            file_size=int(audio_file.stat().st_size),  # Use bytes for Pydantic model
                        ...
                        self.available_reciters[reciter_name]["available_surahs"].append(surah_number)
                        self.available_reciters[reciter_name]["total_files"] += 1
                        self.available_reciters[reciter_name]["total_size_mb"] += file_size_mb
                        
                        total_files += 1
                        total_size_mb += file_size_mb
                        
                    except ValueError:
                        TreeLogger.warning(f"Invalid filename format: {audio_file.name}", {"file_path": str(audio_file)}
                        )
                        continue
                ...
                TreeLogger.debug(f"Reciter scan complete: {reciter_name}", {
                        "available_surahs": len(self.available_reciters[reciter_name]["available_surahs"]),
                        "total_files": self.available_reciters[reciter_name]["total_files"],
                        "total_size_mb": round(self.available_reciters[reciter_name]["total_size_mb"], 1)
                    }
                )
            
            TreeLogger.info("Audio library scan complete with validation", {
                    "total_reciters": len(self.available_reciters),
                    "total_files": total_files,
                    "total_size_mb": round(total_size_mb, 1),
                    "default_reciter": self.config.default_reciter,
                    "default_reciter_found": self.config.default_reciter in self.available_reciters,
                    "available_reciters": list(self.available_reciters.keys())
                }
            )
            ...
            if self.config.default_reciter not in self.available_reciters:
                TreeLogger.warning(f"Default reciter '{self.config.default_reciter}' not found in audio library", {
                    "default_reciter": self.config.default_reciter,
                    "available_reciters": list(self.available_reciters.keys())
                }, service="AudioService")
                ...
                if self.available_reciters:
                    ...
                    TreeLogger.info(f"Using '{first_reciter}' as fallback reciter", service="AudioService")
    ...
    async def _validate_ffmpeg(self) -> None:
        ...
        try:
            TreeLogger.info(f"Validating FFmpeg installation: {self.config.ffmpeg_path}", service="AudioService")
            ...
            TreeLogger.info("FFmpeg validation successful", {
                    "ffmpeg_path": self.config.ffmpeg_path
                }
            )
    ...
    async def play_surah(self, surah_number: int, reciter: Optional[str] = None) -> bool:
        """
        ...
        """
        try:
            TreeLogger.info(f"Starting playback of surah {surah_number} with error handling", service="AudioService")
            ...
            self.playback_stats["last_playback_time"] = datetime.now(APP_TIMEZONE).isoformat()
            
            TreeLogger.info(f"Playback started successfully", {
                    "surah_number": surah_number,
                    "reciter": selected_reciter,
                    "audio_file": audio_file.file_path,
                    "file_size_mb": audio_file.file_size / (1024 * 1024) if hasattr(audio_file, 'file_size') and audio_file.file_size else 0
                }
            , service="AudioService")
    ...
    async def _get_audio_file(self, surah_number: int, reciter: str) -> Optional[AudioFileInfo]:
        """
        ...
        try:
            ...
            if reciter not in self.audio_files_cache:
                TreeLogger.warning(f"Reciter '{reciter}' not found in cache", {
            ...
            if surah_number not in self.audio_files_cache[reciter]:
                TreeLogger.warning(f"Surah {surah_number} not found for reciter '{reciter}'", {
                    "surah_number": surah_number,
                    "reciter": reciter,
                    "available_surahs": list(self.audio_files_cache[reciter].keys())
                }, service="AudioService")
                return None
            ...
            if not os.path.exists(audio_file.file_path):
                TreeLogger.warning(f"Audio file no longer exists: {audio_file.file_path}", {
                        "surah_number": surah_number,
                        "reciter": reciter,
                        "file_exists": False
                    }
            ...
            TreeLogger.debug(f"Audio file retrieved successfully", {
                "surah_number": surah_number,
                "reciter": reciter,
            ...
            TreeLogger.info(f"Audio file retrieved for surah {surah_number}", {
                "reciter": reciter,
                "file_path": audio_file.file_path
            }, service="AudioService")
    ...
    async def _start_playback(self, audio_file: AudioFileInfo) -> None:
        ...
        try:
            ...
            if self.voice_client.is_playing():
                TreeLogger.info("Stopping current playback for new audio", service="AudioService")
            ...
            TreeLogger.debug("FFmpeg options configured", {
            ...
            if hasattr(audio_file, 'duration_seconds') and audio_file.duration_seconds:
                ...
                TreeLogger.debug(f"Duration not cached for {audio_file.file_path}, extracting...", service="AudioService")
                duration = await self._extract_audio_duration(audio_file.file_path)
                if duration:
                    TreeLogger.debug(f"Extracted duration: {duration}s for {audio_file.file_path}", service="AudioService")
            ...
            try:
                ...
                if presence_service and hasattr(presence_service, "update_for_playback"):
                    surah_info = self.get_current_surah()
                    await presence_service.update_for_playback(
                        surah_name=surah_info.get("name_english", f"Surah {self.current_surah_index}"),
                ...
                TreeLogger.debug(f"Could not update presence: {e}", service="AudioService")
    ...
    async def _stop_playback(self) -> None:
        ...
        try:
            if self.voice_client and self.voice_client.is_playing():
                self.voice_client.stop()
                TreeLogger.info("Audio playback stopped successfully", service="AudioService")
            ...
            try:
                ...
                TreeLogger.debug(f"Could not update presence to idle: {e}", service="AudioService")
    ...
    def _playback_finished(self, error: Optional[Exception]) -> None:
        ...
        try:
            ...
            if hasattr(self, '_jump_occurred') and self._jump_occurred:
                TreeLogger.debug("Ignoring playback finished callback - jump occurred", service="AudioService")
            ...
            if error:
                ...
                TreeLogger.error(f"Playback finished with error: {error}", None, {
                ...
                TreeLogger.info("Playback finished successfully", {
                    "operation": "audio_playback_callback",
                    "current_surah": self.current_surah,
                    "current_reciter": self.current_reciter
                }, service="AudioService")
            ...
            TreeLogger.info("Playback completed - ready for next surah", {
            ...
            TreeLogger.error(f"Playback finished callback error: {e}", None, {
    ...
    async def pause_playback(self) -> bool:
        ...
        try:
            TreeLogger.debug("Attempting to pause playback", {
            ...
            TreeLogger.debug("Cannot pause - not currently playing", service="AudioService")
            return False
    ...
    async def resume_playback(self) -> bool:
        ...
        try:
            TreeLogger.debug("Attempting to resume playback", {
                "has_voice_client": bool(self.voice_client),
                "is_playing": self.voice_client.is_playing() if self.voice_client else False,
                "is_paused": self.voice_client.is_paused() if self.voice_client else False,
                "current_state": self.playback_state,
            ...
            if self.voice_client and self.voice_client.is_paused():
                ...
                if self.pause_time:
                    ...
                    TreeLogger.debug(f"Tracked pause duration: {pause_duration:.2f}s", {
            ...
            TreeLogger.debug("Cannot resume - not currently paused", service="AudioService")
            return False
    ...
    async def next_surah(self) -> bool:
        ...
        try:
            ...
            TreeLogger.debug("Manual control: Next surah activated", service="AudioService")
            ...
            TreeLogger.info(f"Skipping to next surah: {next_surah_number}", {
                "current_surah": current_surah,
                "next_surah": next_surah_number
            }, service="AudioService")
            ...
            if success:
                TreeLogger.info(f"Successfully skipped to surah {next_surah_number}", service="AudioService")
                ...
                TreeLogger.error(f"Failed to skip to surah {next_surah_number}", None, {
                    "current_surah": current_surah,
                    "target_surah": next_surah_number
                }, service="AudioService")
    ...
    async def previous_surah(self) -> bool:
        ...
        try:
            # Set manual control flag to prevent monitoring interference
            self.manual_control_active = True
            TreeLogger.debug("Manual control: Previous surah activated", service="AudioService")
            ...
            TreeLogger.info(f"Skipping to previous surah: {prev_surah_number}", {
                "current_surah": current_surah,
                "previous_surah": prev_surah_number
            }, service="AudioService")
            ...
            if success:
                TreeLogger.info(f"Successfully skipped to surah {prev_surah_number}", service="AudioService")
                # Clear manual control flag after a delay to allow playback to stabilize
                asyncio.create_task(self._clear_manual_control_after_delay())
            else:
                TreeLogger.error(f"Failed to skip to surah {prev_surah_number}", None, {
                    "current_surah": current_surah,
                    "target_surah": prev_surah_number
                }, service="AudioService")
                self.manual_control_active = False
    ...
    async def cycle_loop_mode(self) -> str:
        ...
        try:
            # Set manual control flag to prevent monitoring interference
            self.manual_control_active = True
            TreeLogger.debug("Manual control: Cycle loop mode activated", service="AudioService")
            ...
            TreeLogger.info(f"Loop mode cycled from '{current_mode}' to '{new_mode}'", {
    ...
    async def toggle_shuffle(self) -> bool:
        ...
        try:
            # Set manual control flag to prevent monitoring interference
            self.manual_control_active = True
            TreeLogger.debug("Manual control: Toggle shuffle activated", service="AudioService")
            ...
            TreeLogger.info(f"Shuffle mode {'enabled' if new_shuffle else 'disabled'}", {
    ...
    async def change_surah(self, surah_number: int) -> bool:
        ...
        """
        try:
            TreeLogger.debug(f"change_surah called with surah_number: {surah_number}", {
                "current_surah": self.current_surah,
                "current_reciter": self.current_reciter,
                "is_playing": self.is_playing(),
                "voice_connected": bool(self.voice_client and self.voice_client.is_connected())
            }, service="AudioService")
            
            if not (1 <= surah_number <= 114):
                return False
            
            TreeLogger.info(f"Changing to surah {surah_number}", {
                "current_surah": self.current_surah_index,
            ...
            if success:
                TreeLogger.info(f"Successfully changed to surah {surah_number}", service="AudioService")
            else:
    ...
    async def change_reciter(self, reciter_name: str) -> bool:
        ...
        """
        try:
            TreeLogger.debug(f"change_reciter called with reciter_name: {reciter_name}", {
                "current_reciter": self.current_reciter,
                "current_surah": self.current_surah,
                "is_playing": self.is_playing(),
                "available_reciters": list(self.available_reciters.keys())
            ...
            if reciter_name not in self.available_reciters:
                TreeLogger.error(f"Reciter not available: {reciter_name}", None, {
                    "requested_reciter": reciter_name,
                    "available_reciters": list(self.available_reciters.keys())
                }, service="AudioService")
                return False
            
            TreeLogger.info(f"Changing reciter to {reciter_name}", {
                "current_reciter": self.current_reciter,
            ...
            if self.current_surah_index and self.playback_state == PlaybackState.PLAYING:
                # Stop current playback to prevent callback interference
                if self.voice_client and self.voice_client.is_playing():
                    self.voice_client.stop()
                ...
                if not success:
                    ...
                    TreeLogger.error(f"Failed to change to reciter {reciter_name}, reverted", None, {
                        "failed_reciter": reciter_name,
                        "reverted_to": old_reciter
                    }, service="AudioService")
                    return False
            ...
            TreeLogger.info(f"Successfully changed reciter to {reciter_name}", service="AudioService")
            return True
    ...
    def is_playing(self) -> bool:
        ...
        try:
            return (
                ...
                self.voice_client.is_playing() and 
                self.playback_state == PlaybackState.PLAYING
            )
        except Exception:
            return False
    ...
    def is_paused(self) -> bool:
        ...
        try:
            return (
                self.voice_client is not None and 
                self.voice_client.is_paused() and 
                self.playback_state == PlaybackState.PAUSED
            )
        except Exception:
            return False
    ...
    def get_current_surah(self) -> dict:
        ...
        try:
            ...
            TreeLogger.error(f"Error getting current surah info: {e}", service="AudioService")
            return {}
    ...
    def get_current_reciter(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                "name_arabic": reciter_arabic_names.get(self.current_reciter, self.current_reciter),
                "available_surahs": len(self.available_reciters.get(self.current_reciter, {}))
            }
        except Exception:
            return {}
    ...
    def get_position(self) -> float:
        ...
        """
        try:
            if self.playback_state == PlaybackState.STOPPED or not self.playback_start_time:
                return 0.0
            ...
            if self.playback_state == PlaybackState.PAUSED:
                ...
                if self.pause_time:
                    elapsed = (self.pause_time - self.playback_start_time) - self.paused_duration
    ...
    async def _clear_manual_control_after_delay(self, delay: int = 5):
        ...
        try:
            ...
            self.manual_control_active = False
            TreeLogger.debug(f"Manual control cleared after {delay}s delay", service="AudioService")
        except Exception as e:
            TreeLogger.error(f"Error clearing manual control flag: {e}", service="AudioService")
            self.manual_control_active = False