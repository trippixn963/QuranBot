...
class ReciterInfo(BaseModel):
    ...
    total_surahs: int = Field(default=114, ge=1, le=114, description="Total surahs available")
    available_surahs: List[int] = Field(default_factory=list, description="List of available surah numbers")
    audio_quality: AudioQuality = Field(default=AudioQuality.HIGH, description="Audio quality")
    
    def has_surah(self, surah_number: int) -> bool:
        """Check if this reciter has a specific surah."""
        return surah_number in self.available_surahs if self.available_surahs else 1 <= surah_number <= 114
...
class AudioFileInfo(BaseModel):
    ...
    created_at: datetime = Field(default_factory=lambda: datetime.now(EST), description="File creation time")
    last_accessed: datetime = Field(default_factory=lambda: datetime.now(EST), description="Last access time")
...
class PlaybackPosition(BaseModel):
    ...
    position_seconds: float = Field(default=0.0, ge=0.0, description="Position within current surah in seconds")
    total_duration_seconds: Optional[float] = Field(default=0.0, ge=0.0, description="Total duration of current surah")
    timestamp: datetime = Field(default_factory=lambda: datetime.now(EST), description="When this position was recorded")
...
class AudioServiceConfig(BaseModel):
    ...
    audio_base_folder: Path = Field(default=Path("audio"), description="Base audio directory")
    ffmpeg_path: Path = Field(default=Path("/opt/homebrew/bin/ffmpeg"), description="FFmpeg executable path")
    default_volume: float = Field(default=1.0, ge=0.0, le=1.0, description="Default audio volume")
    default_reciter: str = Field(default="Saad Al Ghamdi", description="Default reciter name")
    connection_timeout: float = Field(default=30.0, ge=5.0, description="Voice connection timeout")
    playback_timeout: float = Field(default=300.0, ge=60.0, description="Playback operation timeout")
    ...
    ffmpeg_options: Dict[str, Any] = Field(default_factory=dict, description="Additional FFmpeg options")
    reconnect_attempts: int = Field(default=5, ge=1, description="Voice reconnection attempts")
    reconnect_delay: float = Field(default=2.0, ge=0.5, description="Delay between reconnection attempts")
    auto_resume: bool = Field(default=True, description="Automatically resume after disconnection")
    continuous_play: bool = Field(default=True, description="Enable continuous audio playback")
    cache_enabled: bool = Field(default=True, description="Enable metadata caching")
    preload_metadata: bool = Field(default=True, description="Preload metadata on startup")
    playback_buffer_size: int = Field(default=1024, ge=512, le=8192, description="Audio buffer size")
    enable_reconnection: bool = Field(default=True, description="Enable automatic reconnection")
...
class BotState(BaseModel):
    ...
    panel_channel_id: Optional[int] = Field(None, description="Control panel channel ID")
    current_reciter: str = Field(..., description="Currently selected reciter")
    playback_mode: PlaybackMode = Field(default=PlaybackMode.SEQUENTIAL, description="Current playback mode")
    playback_state: PlaybackState = Field(default=PlaybackState.STOPPED, description="Current playback state")
    current_position: PlaybackPosition = Field(default_factory=lambda: PlaybackPosition(surah=1), description="Current playback position")
    volume: float = Field(default=1.0, ge=0.0, le=1.0, description="Current volume level")
    ...
    last_activity: datetime = Field(default_factory=lambda: datetime.now(EST), description="Last bot activity")
...
class UserStats(BaseModel):
    ...
    favorite_reciter: Optional[str] = Field(None, description="User's favorite reciter")
    total_listening_time: float = Field(default=0.0, ge=0.0, description="Total listening time in seconds")
    ...
    first_seen: datetime = Field(default_factory=lambda: datetime.now(EST), description="First interaction date")
    last_seen: datetime = Field(default_factory=lambda: datetime.now(EST), description="Last interaction date")
...
class QuizStats(BaseModel):
    ...
    total_correct_answers: int = Field(default=0, ge=0, description="Total correct answers")
    total_participants: int = Field(default=0, ge=0, description="Total unique participants")
    average_accuracy: float = Field(default=0.0, ge=0.0, le=100.0, description="Overall accuracy percentage")
    most_difficult_question: Optional[str] = Field(None, description="Question with lowest accuracy")
    easiest_question: Optional[str] = Field(None, description="Question with highest accuracy")
    last_reset: datetime = Field(default_factory=lambda: datetime.now(EST), description="Last statistics reset")
...
class VerseInfo(BaseModel):
    ...
    revelation_context: Optional[str] = Field(None, description="Context of revelation")
...
class DuaInfo(BaseModel):
    ...
    benefits: Optional[str] = Field(None, description="Benefits of reciting this dua")
...
def create_default_bot_state(guild_id: int, voice_channel_id: int) -> BotState:
    ...
    default_reciter_str = str(config.default_reciter.value) if hasattr(config.default_reciter, 'value') else str(config.default_reciter)
