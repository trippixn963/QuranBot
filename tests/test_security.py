...
from src.security.error_handler import ErrorSeverity, SecureErrorHandler
...
class TestSecureValidator:
    ...
    def test_discord_token_validation(self):
        ...
        with pytest.raises(SecurityError):
            SecureValidator.validate_discord_token(
                "A" * 58 + "<script>"
            )  # Dangerous chars
    ...
    def test_webhook_url_validation(self):
        ...
        with pytest.raises(SecurityError):
            SecureValidator.validate_webhook_url(
    ...
    def test_text_input_validation(self):
        ...
        with pytest.raises(SecurityError):
            SecureValidator.validate_text_input(
                "'; DROP TABLE users; --"
            )  # SQL injection
    ...
    def test_file_path_validation(self):
        ...
        assert (
            SecureValidator.validate_file_path("data/backup.json") == "data/backup.json"
        )
        ...
        with pytest.raises(SecurityError):
            SecureValidator.validate_file_path(
                "file.exe", allowed_extensions=[".mp3", ".json"]
            )
    ...
    def test_admin_user_ids_validation(self):
        ...
        # Duplicates
        with pytest.raises(SecurityError):
            SecureValidator.validate_admin_user_ids(
    ...
    def test_log_data_sanitization(self):
        ...
        sensitive_data = {
            ...
            "long_text": "A" * 300,
            "nested": {"api_key": "secret_key", "safe_data": "safe_value"},
        }
...
class TestSecureErrorHandler:
    ...
    def test_error_code_mapping(self):
        ...
        assert (
            self.error_handler._get_error_code(PermissionError("test"))
            == "AUTH_INSUFFICIENT_PERMISSIONS"
        )
        assert (
            self.error_handler._get_error_code(FileNotFoundError("test"))
            == "AUDIO_FILE_NOT_FOUND"
        )

        # Test message-based mapping
        assert (
            self.error_handler._get_error_code(Exception("token invalid"))
            == "AUTH_INVALID_TOKEN"
        )
        assert (
            self.error_handler._get_error_code(Exception("surah not found"))
            == "INVALID_SURAH_NUMBER"
        )
        assert (
            self.error_handler._get_error_code(Exception("audio playback failed"))
    ...
    def test_error_severity_classification(self):
        ...
        assert (
            self.error_handler._get_error_severity(security_error)
        ...
        assert (
            self.error_handler._get_error_severity(value_error) == ErrorSeverity.MEDIUM
        )

        # Low severity errors
        generic_error = Exception("generic error")
        assert (
            self.error_handler._get_error_severity(generic_error) == ErrorSeverity.LOW
        )
    ...
    async def test_validation_error_handling(self):
        ...
        response = await self.error_handler.handle_validation_error(
            field="surah_number", value=150, constraint="Must be between 1 and 114"
        )
    ...
    async def test_rate_limit_error_handling(self):
        ...
        response = await self.error_handler.handle_rate_limit_error(
            limit=10, window=60, retry_after=45
        )
