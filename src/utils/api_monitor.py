...
class APICallMetric:
    ...
    rate_limit_reset_after: float | None = None
...
class DiscordAPIMonitor:
        ...
        async def monitored_request(route, **kwargs):
            ...
            try:
                ...
                if hasattr(response, "headers"):
                    ...
                    rate_limit_reset_after = response.headers.get(
    ...
    def _record_gateway_event(
        ...
        self.last_heartbeat = (
            time.time()
            if event_type in ["connect", "heartbeat"]
    ...
    def _check_rate_limits(self, metric: APICallMetric):
        ...
        if (
            ...
            and metric.rate_limit_limit is not None
        ):
            ...
            if (
                usage >= RATE_LIMIT_CRITICAL_THRESHOLD
                and current_time - last_warning > 300
            ):
                ...
                usage >= RATE_LIMIT_WARNING_THRESHOLD
                and current_time - last_warning > 600
            ):
                log_perfect_tree_section(
    ...
    def _update_health_status(self):
        ...
        recent_gateway_events = [
            m for m in self.gateway_metrics if m.timestamp > recent_cutoff
        ]

        # Calculate average response time
        if recent_api_calls:
            avg_response_time = statistics.mean(
                [m.response_time for m in recent_api_calls]
            )
            error_rate = len(
        ...
        if (
            avg_response_time > RESPONSE_TIME_CRITICAL
            or error_rate > 0.1
            or current_rate_limit_usage > RATE_LIMIT_CRITICAL_THRESHOLD
            or not gateway_connected
        ):
            is_healthy = False
            status = "critical"
        elif (
            avg_response_time > RESPONSE_TIME_WARNING
            or error_rate > 0.05
            or current_rate_limit_usage > RATE_LIMIT_WARNING_THRESHOLD
            or (gateway_latency and gateway_latency > 0.5)
        ):
            status = "warning"
        ...
        while True:
            try:
                ...
                log_error_with_traceback(
                    "Error cleaning up Discord API monitor data", e
                )
    ...
    def _save_data(self):
        ...
        try:
            data = {
                "health_history": [
                    asdict(h) for h in list(self.health_history)[-100:]
                ],  # Keep last 100
                "api_metrics": [
                    asdict(m) for m in list(self.api_metrics)[-500:]
                ],  # Keep last 500
                "gateway_metrics": [
                    asdict(m) for m in list(self.gateway_metrics)[-100:]
                ],  # Keep last 100
        ...
        try:
            if MONITOR_DATA_FILE.exists():
                ...
                if "stats" in data:
                    ...
                    self.connection_start_time = stats.get(
...
_discord_monitor: DiscordAPIMonitor | None = None
...
def get_discord_monitor() -> DiscordAPIMonitor | None:
