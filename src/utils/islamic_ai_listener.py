...
from src.services.enhanced_islamic_ai_service import get_enhanced_islamic_ai_service
...
class TranslationView(discord.ui.View):
    ...
    def __init__(self, ai_response: str, original_embed: discord.Embed, original_user_id: int):
        ...
        language_buttons = [
            ...
            ('de', discord.ButtonStyle.secondary),  # Gray for German
    ...
    def create_translation_callback(self, language_code: str):
        ...
        async def translation_callback(interaction: discord.Interaction):
            ...
            try:
                success, translated_text = await self.translation_service.translate_ai_response(
                ...
                if success:
                    ...
                    lang_display = self.translation_service.get_language_display_name(language_code)
                    if language_code == 'en':
                    ...
                    await interaction.edit_original_response(
                        content=f"❌ Translation failed: {translated_text}",
                        view=None
                    )
...
class IslamicAIMentionListener(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, container: DIContainer):
        ...
        self.islamic_keywords = {
            # English keywords
            'english': [
                'islam', 'muslim', 'allah', 'prophet', 'muhammad', 'quran', 'koran',
                'prayer', 'salah', 'salat', 'wudu', 'ablution', 'mosque', 'masjid',
                'ramadan', 'fasting', 'sawm', 'zakat', 'charity', 'hajj', 'pilgrimage',
                'shahada', 'faith', 'belief', 'hadith', 'sunnah', 'imam', 'scholar',
                'halal', 'haram', 'makruh', 'mustahab', 'fiqh', 'sharia', 'tawhid',
                'iman', 'dua', 'dhikr', 'recitation', 'surah', 'ayah', 'verse',
                'mecca', 'medina', 'kaaba', 'qibla', 'eid', 'jummah', 'friday'
            ],
            # Arabic keywords (common Islamic terms)
            'arabic': [
                'الله', 'إسلام', 'مسلم', 'محمد', 'قرآن', 'صلاة', 'وضوء', 'مسجد',
                'رمضان', 'صوم', 'زكاة', 'حج', 'شهادة', 'حديث', 'سنة', 'إمام',
                'حلال', 'حرام', 'فقه', 'شريعة', 'توحيد', 'إيمان', 'دعاء', 'ذكر',
                'سورة', 'آية', 'مكة', 'المدينة', 'الكعبة', 'قبلة', 'عيد', 'جمعة'
            ]
        }
        
        # Keywords that indicate questions about other religions (should be declined)
        self.non_islamic_keywords = [
            'christianity', 'christian', 'jesus', 'christ', 'bible', 'church', 'pope',
            'judaism', 'jewish', 'jew', 'torah', 'synagogue', 'rabbi',
            'hinduism', 'hindu', 'buddha', 'buddhism', 'buddhist',
            'atheism', 'atheist', 'agnostic', 'secular',
            'compare', 'comparison', 'versus', 'vs', 'better than', 'different from',
            'why not', 'debunk', 'prove', 'disprove', 'wrong', 'false'
        ]
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            async with message.channel.typing():
                ...
                # Check rate limit
                rate_status = enhanced_ai_service.get_rate_limit_status(message.author.id)
                ...
                if isinstance(requests_remaining, str) or requests_remaining > 0:
                    ...
                    reset_time_str = reset_datetime.strftime("%I:%M %p")  # 12-hour format with AM/PM
                ...
                success, ai_response, error_message = await enhanced_ai_service.ask_question(
                ...
                question_display = content if len(content) <= 100 else content[:97] + "..."
                embed.add_field(
                    name="❓ Your Question",
                ...
                updated_rate_status = enhanced_ai_service.get_rate_limit_status(message.author.id)
                ...
                translation_view = TranslationView(ai_response, embed, message.author.id)
                
                # Send response with translation buttons
                await message.reply(embed=embed, view=translation_view, mention_author=False)
            ...
            try:
                ...
                log_error_with_traceback("Failed to send error message in AI mention listener", e)
    ...
    def _is_islamic_question(self, content: str) -> bool:
        ...
        is_direct_address = any(word in content_lower for word in [
            'hello', 'hi', 'hey', 'how are you', 'what are you', 'who are you',
            'tell me', 'can you', 'are you', 'do you', 'what do you'
        ])
