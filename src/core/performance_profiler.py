...
class PerformanceProfiler:
    ...
    def __init__(
        ...
        bottleneck_thresholds: dict[str, float] | None = None,
    ):
        ...
        self.memory_baseline: tracemalloc.Snapshot | None = None
        self.memory_leaks: list[dict[str, Any]] = []

        # CPU profiling
        self.cpu_profiler: cProfile.Profile | None = None
        self.line_profiler: Any | None = (
            None  # Will be line_profiler.LineProfiler if available
        )
    ...
    async def initialize(self) -> None:
        ...
        if self.enable_line_profiling and HAS_LINE_PROFILER:
            ...
            await self.logger.warning(
                "Line profiling requested but line_profiler not available"
            )
    ...
    async def _record_operation(
        ...
        await self._check_operation_bottlenecks(
            operation_name, duration, memory_delta, cpu_usage
        )
        ...
        if duration > self.bottleneck_thresholds["operation_time_threshold"]:
            bottlenecks.append(
                BottleneckReport(
                    ...
                    metrics={
                        "duration": duration,
                        "threshold": self.bottleneck_thresholds[
        ...
        if cpu_usage > self.bottleneck_thresholds["cpu_threshold"]:
            bottlenecks.append(
                BottleneckReport(
                    ...
                    metrics={
                        "cpu_usage": cpu_usage,
                        "threshold": self.bottleneck_thresholds["cpu_threshold"],
                    },
                    recommendations=[
    ...
    async def _update_system_metrics(self) -> None:
        ...
        try:
            ...
            self.system_metrics["memory_rss_mb"] = (
    ...
    async def _check_system_bottlenecks(self) -> None:
        ...
        if (
            ...
            > self.bottleneck_thresholds["cpu_threshold"]
        ):
            bottlenecks.append(
                BottleneckReport(
                    bottleneck_type=BottleneckType.CPU_INTENSIVE,
                    severity=(
                        "critical"
                        if self.system_metrics["cpu_percent"] > 95.0
                        else "high"
                    ),
        ...
        if (
            ...
            > self.bottleneck_thresholds["memory_threshold"]
        ):
            bottlenecks.append(
                BottleneckReport(
                    bottleneck_type=BottleneckType.MEMORY_LEAK,
                    severity=(
                        "critical"
                        if self.system_metrics["memory_percent"] > 95.0
                        else "high"
                    ),
                    ...
                    impact_score=min(
                        self.system_metrics["memory_percent"] / 100.0, 1.0
                    ),
                )
            )
        ...
        with self.lock:
            ...
            for op in self.operations.values():
                for bottleneck in op.bottlenecks[
                    -5:
                ]:  # Last 5 bottlenecks per operation
    ...
    async def get_operation_profile(self, operation_name: str) -> dict[str, Any] | None:
        ...
        with self.lock:
            ...
            recent_history = [
                h
                for h in self.operation_history[-100:]  # Last 100 entries
