...
class AudioCacheOptimizer:
    ...
    def __init__(self, logger: StructuredLogger, cache_dir: Path, max_memory_mb: int = 500):
    ...
    async def initialize(self) -> None:
        ...
        await self.logger.info("Initializing audio cache optimizer", {
    ...
    async def _analyze_existing_files(self) -> None:
        ...
        await self.logger.info("Audio file analysis complete", {
            "total_files": len(audio_files),
            "total_size_mb": round(total_size / (1024 * 1024), 1),
            "average_file_size_mb": round(total_size / len(audio_files) / (1024 * 1024), 1) if audio_files else 0,
        ...
        if large_files:
            await self.logger.info("Large audio files detected", {
    ...
    async def get_optimized_file_path(self, original_path: Path, surah_number: int) -> Optional[Path]:
        ...
        if file_key in self.cached_files:
            ...
            await self.logger.debug("Audio cache hit", {
        ...
        if original_path.exists():
            ...
            if (self.current_memory_usage + file_size <= self.max_memory_bytes and 
                file_size < 50 * 1024 * 1024):  # Max 50MB per file
    ...
    async def _cache_file(self, file_path: Path, file_size: int) -> None:
        ...
        await self.logger.debug("File cached", {
            "file": file_path.name,
    ...
    async def _ensure_cache_space(self, needed_bytes: int) -> None:
        ...
        while (self.current_memory_usage + needed_bytes > self.max_memory_bytes and 
               self.cached_files):
            ...
            await self.logger.debug("Evicted file from cache", {
        ...
        while True:
            try:
                ...
                for file_key in list(self.cached_files.keys()):
                    ...
                    if (current_time - cache_info["last_accessed"] > 3600 and 
                        cache_info["access_count"] < 3):
                ...
                if cleaned_patterns > 0 or cleaned_cache > 0:
                    await self.logger.info("Cache cleanup completed", {
                        ...
                        "current_cache_size_mb": round(self.current_memory_usage / (1024 * 1024), 1),
        ...
        while True:
            try:
                ...
                for file_key, access_times in self.access_patterns.items():
                    if (len(access_times) >= self.prefetch_threshold and 
                        file_key not in self.cached_files):
                ...
                for candidate in prefetch_candidates[:5]:  # Prefetch up to 5 files
                    if (self.current_memory_usage + candidate["size"] <= 
                        self.max_memory_bytes * 0.8):  # Don't fill more than 80%
                ...
                if prefetched > 0:
                    await self.logger.info("Prefetch completed", {
                        "files_prefetched": prefetched,
                        "candidates_considered": len(prefetch_candidates),
                        "cache_usage_percent": round(self.current_memory_usage / self.max_memory_bytes * 100, 1)
                    })
    ...
    def get_optimization_statistics(self) -> Dict:
        ...
        memory_efficiency = (self.current_memory_usage / self.max_memory_bytes * 100) if self.max_memory_bytes > 0 else 0
        ...
        return {
            ...
            "access_patterns": {
                ...
                "average_accesses_per_file": round(total_accesses / active_patterns, 1) if active_patterns > 0 else 0
            },
            "optimization_potential": {
                "files_frequently_accessed": len([p for p in self.access_patterns.values() if len(p) >= self.prefetch_threshold]),
                "memory_savings_potential_mb": round((684 * 4 - self.current_memory_usage / (1024 * 1024)), 1),  # Estimated savings
                "recommended_max_cache_mb": min(500, len(self.access_patterns) * 4)  # Dynamic recommendation
            }
        }
    ...
    async def shutdown(self) -> None:
        ...
        await self.logger.info("Audio cache optimizer shutdown", {
            "final_hit_rate": round(self.get_hit_rate(), 2),
            "total_prefetches": self.prefetch_successes,
            "memory_saved_mb": round((684 * 4 - self.current_memory_usage / (1024 * 1024)), 1)
        }) 