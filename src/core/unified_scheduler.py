...
class UnifiedTaskScheduler:
    ...
    async def initialize(self) -> None:
        ...
        await self.logger.info("Task scheduler initialized", {
    ...
    async def start(self) -> None:
        ...
        await self.logger.info("Unified scheduler started", {
    ...
    async def _execute_task(self, task: ScheduledTask, group: TaskGroup) -> None:
        ...
        try:
            ...
            await self.logger.warning(f"Task {task.name} failed", {
                "error": str(e),
    ...
    async def reschedule_task(self, task_name: str, new_interval: float) -> bool:
        ...
        if task_name in self.tasks:
            ...
            task.update_next_run()
            await self.logger.info(f"Task {task_name} rescheduled", {
...
class LegacyTaskMigrator:
    ...
    async def migrate_all_tasks(self, services: Dict[str, Any]) -> int:
        ...
        for service_name, service in services.items():
            try:
                ...
                await self.logger.warning(f"Failed to migrate {service_name} tasks", {
                    "error": str(e)
                })
        
        await self.logger.info("Task migration completed", {
    ...
    async def _create_wrapper_task(self, service: Any, method_name: str, task_key: str) -> None:
        """Create a wrapper task for a service method"""
        method = getattr(service, method_name)
        task_config = self.task_mappings.get(task_key, {
            "interval": 300,
            "priority": TaskPriority.MEDIUM,
            "group": "monitoring"
        })
