...
class VisualizationBuilder:
    ...
    def create_multi_progress_bars(
        items: list[dict[str, Any]], max_value: float = None, bar_length: int = 15
    ) -> list[str]:
        """
    ...
    def create_sparkline(
        values: list[float], width: int = 20, show_trend: bool = True
    ) -> str:
        """
    ...
    def create_circular_progress(
        value: float, max_value: float, size: str = "medium"
    ) -> str:
        """
    ...
    def create_comparison_bars(
        ...
        # Format the change
        change_str = (
            f"{arrow} {abs(change_percent):.1f}%"
            if show_percentage
            else f"{arrow} {current - previous:+.1f}"
        )
    ...
    def create_activity_heatmap(
        data: dict[int, int], hours: int = 24, show_labels: bool = True
    ) -> str:
        """
    ...
    def create_stats_card(
        title: str, stats: list[dict[str, Any]], use_columns: bool = True
    ) -> dict[str, Any]:
        """
        ...
        if use_columns and len(lines) > 1:
            ...
            content = ""
            for left, right in zip(left_column, right_column, strict=False):
                if right:
                    content += f"{left}\n{right}\n"
                else:
                    content += f"{left}\n"
...
def quick_gauge(
    label: str, value: float, min_val: float = 0, max_val: float = 100
) -> str:
