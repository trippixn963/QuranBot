...
class ImportTracker:
    ...
    def track_import(
        self, module_name: str, import_time: float, size_estimate: int = 0
    ):
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        unused_modules = len(
            [imp for imp in self.imports.values() if imp["access_count"] == 0]
        )
        ...
        return {
            ...
            "efficiency": (
                round((total_modules - unused_modules) / total_modules * 100, 2)
                if total_modules > 0
                else 0
            ),
            "avg_import_time": (
                round(total_time / total_modules, 4) if total_modules > 0 else 0
            ),
        }
    ...
    def _get_module(self):
        ...
        with self._lock:
            ...
            try:
                ...
                _import_tracker.track_import(
                    self._module_name, import_time, size_estimate
                )
                _import_tracker.track_access(self._module_name)
...
class ImportBatch:
    ...
    def add_module(
        self, module_name: str, fallback_factory: Callable = None
    ) -> LazyModule:
...
class SmartImportManager:
    ...
    async def _optimize_startup_imports(self):
        ...
        await self.logger.debug(
            ...
            {"modules": critical_modules, "startup_time_saved": "estimated 200-500ms"},
        )
    ...
    def preload_batch(self, batch_name: str) -> dict[str, Any] | None:
    ...
    async def warmup_frequently_used(self) -> dict[str, Any]:
        ...
        frequent_modules = ["asyncio", "json", "datetime", "pathlib", "logging"]
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                raise ImportError(
                    f"Function {func.__name__} requires {module_name}: {e}"
                )
                        ...
                        imports.append(
                            {
                                ...
                                "module": alias.name,
                                "line": node.lineno,
                                "can_be_lazy": self._can_be_lazy_import(
                                    alias.name, tree
                                ),
                            }
                        )
...
_smart_import_manager: SmartImportManager | None = None
