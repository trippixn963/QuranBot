...
class StateManager:
    ...
    def __init__(self, state_file: str = "bot_state.json"):
        ...
        try:
            ...
            logger.error(
                f"🔍 State manager init error traceback: {traceback.format_exc()}"
            )
            raise
    ...
    def remove_event_listener(self, event: str, callback: Callable) -> None:
        ...
        try:
            ...
            logger.error(f"❌ Error removing event listener: {e}")
    ...
    async def _emit_event(
        ...
        try:
            ...
            if listener_count == 0:
                log_operation(
                    "events", "DEBUG", {"action": "emit_no_listeners", "event": event}
                )
                return
                    ...
                    with open(self.state_file, "r", encoding="utf-8") as f:
                        ...
                        log_operation(
                            ...
                            "INFO",
                            {
                                ...
                                "current_song_index": default_state.get(
                                    "current_song_index"
                                ),
                                "current_song_name": default_state.get(
                                    "current_song_name"
                                ),
                                "total_songs_played": default_state.get(
                                    "total_songs_played"
                                ),
                            },
                        )
    ...
    def set_current_song_index(self, index: int):
        ...
        if index < 0:
            log_operation(
                "state",
                "WARNING",
                {"action": "set_song_index_negative", "provided_index": index},
            )
            index = 0
    ...
    def set_last_change(
        ...
        try:
            ...
            try:
                ...
                log_operation(
                    "last_change", "DEBUG", {"action": "no_event_loop_for_change_event"}
                )
