...
class SmartRateLimiter:
    ...
    async def _process_queue(self, endpoint: APIEndpoint):
        ...
        try:
            while self.request_queues[endpoint]:
                ...
                sorted_requests = sorted(queue, key=lambda r: (r.priority, r.created_at))
                ...
                try:
                    ...
                    if request.retries < request.max_retries:
                        ...
                        request.error = e
                        request.completed = True
                        await self.logger.error(f"Request failed after {request.max_retries} retries", {
    ...
    async def _wait_for_rate_limit(self, request: APIRequest):
        ...
        if bucket in self.rate_limits:
            ...
            if rate_limit.remaining <= 0 and current_time < rate_limit.reset_at:
                ...
                await self.logger.debug(f"Rate limit hit, waiting {wait_time:.2f}s", {
                    "bucket": bucket,
    ...
    async def _wait_for_request(self, request: APIRequest) -> Any:
        ...
        if self.stats["avg_wait_time"] == 0:
            ...
            self.stats["avg_wait_time"] = (self.stats["avg_wait_time"] * 0.9) + (wait_time * 0.1)
    ...
    def update_rate_limit(self, bucket: str, limit: int, remaining: int, reset_at: float):
...
class ConnectionPool:
    ...
    async def initialize(self):
        ...
        await self.logger.info("Initializing Discord connection pool", {
    ...
    async def get_session(self) -> aiohttp.ClientSession:
        ...
        if (self.session_usage[best_session] > 100 and 
            len(self.sessions) < self.max_connections):
            return await self._create_session()
    ...
    async def cleanup_old_sessions(self):
        ...
        for session in self.sessions:
            ...
            if session_age > 3600 and usage < 10:
                sessions_to_remove.append(session)
            # Remove sessions older than 6 hours regardless
            elif session_age > 21600:
                sessions_to_remove.append(session)
...
class DiscordOptimizer:
    ...
    async def initialize(self):
        ...
        await self.logger.info("Discord optimizer active", {
            "rate_limiting": True,
    ...
    def _can_batch_request(self, request: APIRequest) -> bool:
        ...
        return request.endpoint in batchable_endpoints and request.method in ["GET", "POST"]
    ...
    async def _batch_request(self, request: APIRequest) -> Any:
        ...
        if (len(self.batch_queues[batch_key]) >= self.batch_size or
    ...
    async def _update_performance_metrics(self):
        ...
        total_requests = rate_limiter_stats["requests_processed"] + rate_limiter_stats["cache_hits"]
        if total_requests > 0:
        ...
        self.performance_metrics["avg_response_time"] = rate_limiter_stats["avg_wait_time"]
