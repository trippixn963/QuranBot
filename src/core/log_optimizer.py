...
class BatchedLogWriter:
    ...
    def __init__(
        self, log_dir: Path, max_batch_size: int = 100, flush_interval: float = 5.0
    ):
        ...
        try:
            ...
            for date_key, logs_by_type in grouped_entries.items():
                ...
                if logs_by_type["json"]:
                    ...
                    await self._write_batch_to_file(date_dir / "logs.json", json_lines)
...
class LogCompressor:
    ...
    def __init__(self, log_dir: Path, compress_after_days: int = 1, keep_days: int = 7):
        ...
        self.stats = {"files_compressed": 0, "bytes_saved": 0, "files_deleted": 0}
    ...
    async def compress_old_logs(self) -> dict[str, int]:
        ...
        delete_date = datetime.now(UTC) - timedelta(days=self.keep_days)
    ...
    async def _compress_directory(self, date_dir: Path) -> None:
        ...
        for log_file in date_dir.iterdir():
            if log_file.suffix in [".log", ".json"] and not log_file.name.endswith(
                ".gz"
            ):
    ...
    async def _compression_loop(self) -> None:
        ...
        while not self._shutdown:
            try:
                ...
                # Update total stats
                self.total_stats["disk_space_saved_mb"] = compression_stats[
                    "bytes_saved"
                ] / (1024 * 1024)
