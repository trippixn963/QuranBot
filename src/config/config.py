...
class QuranBotConfig(BaseSettings):
    ...
    control_channel_id: Optional[int] = Field(
        None, 
    ...
    def validate_logging_config(self) -> "QuranBotConfig":
        ...
        if self.use_webhook_logging:
            ...
            if not any(url for url in webhook_urls):
                raise ValueError("At least one webhook URL is required when webhook logging is enabled")
    ...
    def get_reciter_by_name(self, reciter_name: str) -> Optional[ReciterName]:
    ...
    def get_all_webhook_urls(self) -> Dict[str, Optional[str]]:
    ...
    def get_configured_webhooks(self) -> List[str]:
        ...
        return [
            event_type for event_type in [
                "bot_status", "quran_audio", "commands_panel", "user_activity",
                "data_analytics", "errors_alerts", "daily_reports"
            ]
    ...
    def validate_audio_setup(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "audio_folder_is_directory": self.audio_folder.is_dir() if self.audio_folder.exists() else False,
            "ffmpeg_exists": self.ffmpeg_path.exists(),
            "ffmpeg_executable": os.access(self.ffmpeg_path, os.X_OK) if self.ffmpeg_path.exists() else False,
            "reciter_folder_exists": self.reciter_audio_folder.exists(),
...
_config: Optional[QuranBotConfig] = None
...
def get_webhook_url(event_type: str = None) -> Optional[str]:
...
def print_config_summary():
    ...
    print(f"   Multi-Channel Webhooks: {'✅' if summary['multi_channel_webhooks'] else '❌'}")
