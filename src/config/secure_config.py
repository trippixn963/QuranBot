...
class SecureQuranBotConfig(BaseSettings):
    ...
    discord_token: str = Field(..., description="Discord bot token", min_length=50)
    ...
    target_channel_id: int = Field(
        ..., description="Voice channel ID for audio playback", gt=4194304, lt=2**63
    )
    ...
    panel_channel_id: int | None = Field(
        None, description="Channel ID for control panel", gt=4194304, lt=2**63
    )

    logs_channel_id: int | None = Field(
        None, description="Channel ID for bot logs", gt=4194304, lt=2**63
    )

    daily_verse_channel_id: int | None = Field(
        None, description="Channel ID for daily verses", gt=4194304, lt=2**63
    )

    panel_access_role_id: int | None = Field(
        None, description="Role ID for panel access", gt=4194304, lt=2**63
    )
    ...
    developer_id: int | None = Field(
        None, description="Developer user ID", gt=4194304, lt=2**63
    )
    ...
    audio_folder: Path = Field(
        default=Path("audio"), description="Audio files directory"
    )

    default_reciter: ReciterName = Field(
        default=ReciterName.SAAD_AL_GHAMDI, description="Default Quran reciter"
    )

    ffmpeg_path: Path = Field(
        default=Path("/usr/bin/ffmpeg"), description="FFmpeg executable path"
    )
    ...
    default_shuffle: bool = Field(
        default=False, description="Enable shuffle by default"
    )

    default_loop: bool = Field(default=False, description="Enable loop by default")
    ...
    use_webhook_logging: bool = Field(
        default=True, description="Use Discord webhook for logging"
    )
    ...
    rate_limit_per_minute: int = Field(
        default=10, description="Commands per minute per user", ge=1, le=100
    )

    max_concurrent_audio: int = Field(
        default=1, description="Max concurrent audio streams", ge=1, le=5
    )

    backup_interval_hours: int = Field(
        default=24, description="Backup interval in hours", ge=1, le=168
    )
    ...
    openai_api_key: str | None = Field(
        None, description="OpenAI API key for Islamic AI", min_length=40, max_length=100
    )

    vps_host: str | None = Field(
        None, description="VPS host for deployment", max_length=100
    )
    ...
    @classmethod
    def validate_discord_ids(cls, v: int | None) -> int | None:
    ...
    @classmethod
    def validate_vps_host(cls, v: str | None) -> str | None:
    ...
    def get_security_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "production_ready": (
                self.is_production
                and self.use_webhook_logging
                and self.log_level != LogLevel.DEBUG
            ),
            "security_features": [
                "Input validation",
...
_secure_config_instance: SecureQuranBotConfig | None = None
