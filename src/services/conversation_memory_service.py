...
class ConversationMemoryService:
            ...
            if self.memory_file.exists():
                ...
                log_perfect_tree_section(
                    ...
                    [
                        ...
                        ("total_conversations", str(sum(len(user.get('conversation_history', [])) for user in self.memory['users'].values()))),
    ...
    def add_conversation(self, user_id: int, question: str, response: str, topics: List[str]) -> None:
        ...
        try:
            ...
            user_data['knowledge_level'] = self._determine_knowledge_level(user_data['question_complexity_trend'])
            ...
            for topic in topics:
                user_data['topic_frequency'][topic] = user_data['topic_frequency'].get(topic, 0) + 1
            
            # Update favorite topics (top 5)
            sorted_topics = sorted(user_data['topic_frequency'].items(), key=lambda x: x[1], reverse=True)
            ...
            user_data['conversation_style'] = self._determine_conversation_style(user_data)
            ...
            recent_responses = user_data['conversation_history'][-5:]  # Last 5 conversations
            avg_response_length = sum(conv['response_length'] for conv in recent_responses) / len(recent_responses)
    ...
    def _extract_cultural_indicators(self, question: str) -> List[str]:
        ...
        cultural_patterns = {
            'syrian': ['syria', 'syrian', 'damascus', 'aleppo', 'levantine', 'sham', 'bilad al-sham', 'levant', 'homs', 'latakia', 'daraa', 'tartus'],
            'south_asian': ['urdu', 'pakistan', 'india', 'bangladesh', 'desi', 'subcontinental', 'hanafi'],
            'arab': ['arabic', 'saudi', 'egypt', 'lebanon', 'jordan', 'gulf', 'makkah', 'madinah'],
            'southeast_asian': ['indonesia', 'malaysia', 'brunei', 'singapore', 'shafi'],
            'african': ['nigeria', 'senegal', 'morocco', 'tunisia', 'algeria', 'mali', 'maliki'],
            'western': ['america', 'europe', 'canada', 'australia', 'workplace', 'minority'],
            'convert': ['new muslim', 'convert', 'revert', 'recently accepted', 'just became muslim', 'first time']
        }
    ...
    def _analyze_question_complexity(self, question: str) -> str:
        ...
        # Basic indicators
        basic_indicators = ['what is', 'who is', 'when is', 'where is', 'how many', 'simple', 'basic']
        intermediate_indicators = ['how do', 'why do', 'what does', 'explain', 'difference between', 'compare']
        advanced_indicators = ['jurisprudence', 'fiqh', 'madhab', 'scholarly opinion', 'detailed', 'in depth', 'comprehensive']
    ...
    def _extract_emotional_indicators(self, question: str) -> List[str]:
        ...
        emotional_patterns = {
            'struggling': ['struggling', 'difficult', 'hard time', 'can\'t', 'impossible'],
            ...
            'seeking_motivation': ['motivation', 'inspiration', 'encourage', 'strength'],
    ...
    def _extract_practice_indicators(self, question: str) -> str:
        ...
        beginner_indicators = ['new to islam', 'just started', 'basic', 'how to start', 'first time']
        practicing_indicators = ['regular prayer', 'daily dhikr', 'follow sunnah', 'practice regularly']
        advanced_indicators = ['seeking perfection', 'detailed fiqh', 'scholarly level', 'deep understanding']
    ...
    def get_user_context(self, user_id: int) -> Dict:
        ...
        recent_emotions = user_data.get('emotional_patterns', [])[-5:]  # Last 5 emotional indicators
        ...
        needs_encouragement = any(emotion in recent_emotions for emotion in ['struggling', 'doubting', 'sad', 'guilty'])
        ...
        return {
            ...
            'religious_practice_level': user_data.get('religious_practice_level', 'unknown'),
            'preferred_response_length': user_data.get('preferred_response_length', 'medium'),
    ...
    def classify_question_topics(self, question: str) -> List[str]:
        ...
        # Topic mapping
        topic_keywords = {
            'prayer': ['prayer', 'salah', 'namaz', 'wudu', 'ablution', 'qibla', 'mosque'],
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_conversations = sum(len(user.get('conversation_history', [])) for user in self.memory['users'].values())
            ...
            popular_topics = sorted(all_topics.items(), key=lambda x: x[1], reverse=True)[:5]
