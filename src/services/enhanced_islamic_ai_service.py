...
class EnhancedIslamicAIService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            log_perfect_tree_section(
                ...
                [
                    ("hadiths_loaded", str(len(self.hadith_database.get('hadiths', [])))),
                    ("verse_topics", str(len(self.verse_database.get('verse_topics', {})))),
                    ("practical_tools", str(len(self.practical_tools.get('tools', {})))),
        ...
        try:
            ...
            log_perfect_tree_section(
                ...
                [
                    ...
                    ("contradiction_detected", str(contradiction_analysis.get('has_contradiction', False))),
                    ("emotional_support_needed", str(emotional_analysis.get('needs_support', False))),
                    ("cultural_adaptation", cultural_context.get('primary_culture', 'general')),
                    ("user_experience_level", user_context.get('conversation_style', 'unknown')),
    ...
    def _classify_query_type(self, query: str) -> str:
        ...
        elif any(word in query_lower for word in ['prayer time', 'qibla', 'zakat', 'calculate']):
            return 'practical_tool'
        elif any(word in query_lower for word in ['explain', 'tell me about', 'what is', 'deep dive']):
            return 'deep_dive'
        else:
            return 'general'
    ...
    def _is_deep_dive_request(self, query: str) -> bool:
        ...
        deep_dive_indicators = [
            'explain in detail', 'tell me more about', 'deep dive',
...
"""
        ...
        if context_info.get('hadiths'):
            ...
            for hadith in context_info['hadiths'][:2]:
                collection_name = self.hadith_database.get('hadith_collections', {}).get(hadith.get('collection', ''), hadith.get('collection', 'Unknown'))
        ...
        if islamic_context:
            ...
            if islamic_context.get('current_hijri_date'):
                base_prompt += f"• Today's Hijri Date: {islamic_context['current_hijri_date']}\n"
        ...
        if user_context:
            ...
            if user_context.get('conversation_style'):
                base_prompt += f"• Learning Journey: {user_context['conversation_style']}\n"
...
"""
                ...
                if pattern1_found or pattern2_found:
                    ...
                    for past_conv in conversation_history[-10:]:  # Check last 10 conversations
                        ...
                        if pattern1_found and any(word in past_query for word in patterns['pattern2']):
                            ...
                            break
                        elif pattern2_found and any(word in past_query for word in patterns['pattern1']):
                            contradictions.update({
                                'has_contradiction': True,
                                'contradiction_type': patterns['type'],
                                'previous_statement': past_conv.get('question', ''),
    ...
    def _detect_emotional_state(self, query: str) -> dict:
        ...
        try:
            ...
            emotional_patterns = {
                'distress': {
                    'keywords': ['struggling', 'difficult', 'hard time', 'can\'t', 'impossible', 'lost', 'confused', 'overwhelmed'],
                    'level': 'high',
                    'type': 'distress'
                },
                'doubt': {
                    'keywords': ['doubt', 'questioning', 'not sure', 'confused about faith', 'losing faith', 'crisis'],
                    'level': 'high',
                    'type': 'faith_crisis'
                },
                'sadness': {
                    'keywords': ['sad', 'depressed', 'hopeless', 'crying', 'grief', 'mourning', 'death', 'loss'],
                    'level': 'medium',
                    'type': 'grief_sadness'
                },
                'anxiety': {
                    'keywords': ['worried', 'anxious', 'scared', 'afraid', 'nervous', 'stress', 'panic'],
                    'level': 'medium',
                    'type': 'anxiety'
                },
                'guilt': {
                    'keywords': ['guilty', 'shame', 'regret', 'sinned', 'wrong', 'forgiveness', 'repent'],
                    'level': 'medium',
                    'type': 'guilt_shame'
                },
                'loneliness': {
                    'keywords': ['alone', 'lonely', 'isolated', 'no friends', 'no support', 'disconnected'],
                    'level': 'medium',
                    'type': 'loneliness'
                },
                'encouragement_seeking': {
                    'keywords': ['motivation', 'inspiration', 'encourage', 'strength', 'keep going', 'give up'],
                    'level': 'low',
    ...
    def _determine_cultural_context(self, user_context: dict, query: str) -> dict:
        ...
        try:
            ...
            cultural_patterns = {
                'syrian': {
                    'indicators': ['syria', 'syrian', 'damascus', 'aleppo', 'levantine', 'sham', 'bilad al-sham'],
                    'adaptations': ['syrian_context', 'arab_traditions', 'levantine_customs', 'current_situation_awareness', 'diaspora_support']
                },
                'south_asian': {
                    'indicators': ['urdu', 'pakistan', 'india', 'bangladesh', 'desi', 'subcontinental'],
                    'adaptations': ['hanafi_school', 'family_emphasis', 'cultural_traditions']
                },
                'arab': {
                    'indicators': ['arabic', 'saudi', 'egypt', 'lebanon', 'jordan', 'gulf'],
                    'adaptations': ['classical_approach', 'tribal_considerations', 'regional_customs']
                },
                'southeast_asian': {
                    'indicators': ['indonesia', 'malaysia', 'brunei', 'singapore', 'shafi'],
                    'adaptations': ['shafi_school', 'moderate_approach', 'local_customs']
                },
                'african': {
                    'indicators': ['nigeria', 'senegal', 'morocco', 'tunisia', 'algeria', 'mali'],
                    'adaptations': ['maliki_school', 'community_focus', 'sufi_influence']
                },
                'western': {
                    'indicators': ['america', 'europe', 'canada', 'australia', 'convert', 'revert', 'minority'],
                    'adaptations': ['practical_challenges', 'workplace_islam', 'minority_considerations']
                },
                'convert': {
                    'indicators': ['new muslim', 'convert', 'revert', 'recently accepted', 'just became muslim'],
            ...
            for culture, data in cultural_patterns.items():
                indicators_found = [ind for ind in data['indicators'] if ind in query_lower]
                if indicators_found:
