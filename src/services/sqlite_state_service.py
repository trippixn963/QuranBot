...
class SQLiteStateService:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            await self.logger.error("Error during SQLite state service shutdown", {"error": str(e)})
            ...
            if success:
                ...
                try:
                    ...
                    if container:
                        ...
                        if enhanced_webhook and hasattr(enhanced_webhook, "log_database_operation"):
    ...
    async def load_bot_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "total_runtime": stats.get("total_runtime_hours", 0.0) * 3600,  # Convert back to seconds for legacy compatibility
                        ...
                        if enhanced_webhook and hasattr(enhanced_webhook, "log_database_operation"):
                            await enhanced_webhook.log_database_operation(
                                ...
                                context={
                                    ...
                                    "completed_sessions": updates["total_completed_sessions"],
    ...
    async def verify_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if table_counts.get('playback_state', 0) == 0:
                integrity_report["issues"].append("Missing playback state record - bot will use defaults")
                integrity_report["database_healthy"] = False
                
            if table_counts.get('bot_statistics', 0) == 0:
                integrity_report["issues"].append("Missing bot statistics record - analytics unavailable")
                integrity_report["database_healthy"] = False
                
            if table_counts.get('quiz_config', 0) == 0:
                integrity_report["issues"].append("Missing quiz config record - quiz system disabled")
                integrity_report["database_healthy"] = False
            ...
            await self.logger.error("Data integrity verification failed", {"error": str(e)})
            return {
