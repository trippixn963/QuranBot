...
# Transfer web dashboard
log_info "Transferring web dashboard..."
if [ -d "$LOCAL_PROJECT_PATH/vps/web_dashboard" ]; then
    rsync -avz --progress \
        "$LOCAL_PROJECT_PATH/vps/web_dashboard/" "$VPS_USER@$VPS_IP:$VPS_PATH/web_dashboard/"
    log_success "Web dashboard transferred"
else
    log_warning "No web dashboard found to transfer"
fi
...
if [ -f web_dashboard/requirements.txt ]; then
    ...
    echo "No dashboard requirements.txt found"
fi
...
cat > /etc/systemd/system/quranbot-dashboard.service << 'DASHBOARD_SERVICE_EOF'
[Unit]
...
Type=simple
User=root
WorkingDirectory=$VPS_PATH/web_dashboard
Environment=PATH=$VPS_PATH/.venv/bin
ExecStart=$VPS_PATH/.venv/bin/python $VPS_PATH/web_dashboard/app.py
Restart=always
RestartSec=10
...
[Install]
WantedBy=multi-user.target
DASHBOARD_SERVICE_EOF

systemctl daemon-reload
systemctl enable quranbot-dashboard.service

echo "Dashboard service created and enabled"
EOF
...
echo "2. Start dashboard: ssh $VPS_USER@$VPS_IP 'systemctl start quranbot-dashboard'"
echo "3. Check status: ssh $VPS_USER@$VPS_IP 'systemctl status quranbot'"
echo "4. View logs: ssh $VPS_USER@$VPS_IP 'journalctl -u quranbot -f'"
echo ""
echo "🌐 Dashboard will be accessible at: http://$VPS_IP:8080"
